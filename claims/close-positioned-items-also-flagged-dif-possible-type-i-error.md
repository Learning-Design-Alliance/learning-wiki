---
type: claim
title: Some items positioned close together across booklets were also flagged as DIF, possibly due to Type I error from sampling
description: Some items positioned close together across booklets were also flagged as DIF, possibly due to Type I error from sampling
id: close-positioned-items-also-flagged-dif-possible-type-i-error
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
evidence_strength: weak
sources:
  - id: soysal-2021
    resource: "https://doi.org/10.21449/ijate.779963"
    title: "Soysal, S., & Yilmaz Kogar, E. (2021). An Investigation of Item Position Effects by Means of IRT-Based Differential Item Functioning Methods. International Journal of Assessment Tools in Education, 8(2), 239-256. https://doi.org/10.21449/ijate.779963"
    author: "Soysal, S., & Yilmaz Kogar, E."
    q: 2
    i: "?"
---

# Some items positioned close together across booklets were also flagged as DIF, possibly due to Type I error from sampling

> **Claim** · [All claims](index.md)
> **Evidence** · 1 study · `q2` quasi-experiment

## Subclaims
`q2 i?` Items placed at similar positions in different booklets were flagged as DIF in some conditions by at least one method, so not all flagged DIF can be attributed to position. [→ Soysal 2021](#soysal-2021)

## Evidence

### Soysal 2021

Soysal, S., & Yilmaz Kogar, E. (2021). An Investigation of Item Position Effects by Means of IRT-Based Differential Item Functioning Methods. International Journal of Assessment Tools in Education, 8(2), 239-256. https://doi.org/10.21449/ijate.779963

`q2 · i?`

In the TEOG booklet DIF analysis, items 2 and 13 in Booklet A occupy 3rd and 14th positions in Booklet C yet were flagged; the authors state the reason "may not be based on item position" and "may have arisen due to a type 1 error caused by sampling."

> "However, in the present study, the same items positioned close to each other in different booklets were also revealed to flag as DIF in some conditions (with or without purification) in at least one method"

## Discussion


## Related Claims
- [Items flagged as DIF were generally positioned at considerably different places between booklets, suggesting position-related DIF](dif-items-differently-positioned-across-booklets.md) — reports the opposite
- [The polytomous DFIT framework shows Type I error rates close to nominal alpha except when the number of DIF items and DIF magnitude are highest](polytomous-dfit-type-i-error-near-alpha.md) — related
- [An item positioned later in one booklet was found more difficult by that booklet's test takers, attributed to fatigue effect](later-position-item-more-difficult-fatigue-effect.md) — related
