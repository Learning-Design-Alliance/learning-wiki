...
This claim is methodological rather than an effect-size finding: it does not assert that any specific instructional medium is more or less effective than another, but that the *comparison design itself* cannot answer that question validly. The practical implication for instructional designers and researchers is to avoid framing evaluation questions as "does medium A work better than medium B," and instead isolate the specific instructional strategy, media attribute, or learner characteristic actually under investigation — or, when only checking whether a specific local implementation succeeded, to conduct program evaluation rather than present the result as generalizable research.
