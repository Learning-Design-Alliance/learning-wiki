---
type: claim
title: "Across seven kindergarten equivalence lessons, the teacher's sides-of-equation gesture (GSIDES) was coded 38 times, the most of any language or gesture code"
description: "Across seven kindergarten equivalence lessons, the teacher's sides-of-equation gesture (GSIDES) was coded 38 times, the most of any language or gesture code"
id: gsides-gesture-most-frequent-teacher-code-in-kindergarten-equal-sign-lessons
status: draft
generated:
  by: claude/unspecified
  at: 2026-09-24
evidence_strength: weak
sources:
  - id: sung-2022
    resource: "https://eric.ed.gov/?id=ED630361"
    title: "Sung, Y., Stephens, A., Veltri Torres, R., Strachota, S., Blanton, M., Gardiner, A., Stroud, R., & Knuth, E. (2022). Teacher Language and Gesture in an Intervention Focused on Developing Kindergarteners' Understandings of the Equal Sign. Proceedings of the forty-fourth annual meeting of the North American Chapter of the International Group for the Psychology of Mathematics Education. https://eric.ed.gov/?id=ED630361"
    author: "Sung, Y., Stephens, A., Veltri Torres, R., Strachota, S., Blanton, M., Gardiner, A., Stroud, R., & Knuth, E."
    q: 1
    i: "?"
---

# Across seven kindergarten equivalence lessons, the teacher's sides-of-equation gesture (GSIDES) was coded 38 times, the most of any language or gesture code

> **Claim** · [All claims](index.md)
> **Evidence** · 1 study · `q1` argument or single case

## Subclaims
`q1 i?` Qualitative video coding of one teacher across seven lessons recorded 38 GSIDES, 28 ISA, 26 BAL, 17 DEF, 11 GBAL and 6 FORM instances. [→ Sung 2022](#sung-2022)

## Evidence

### Sung 2022

Sung, Y., Stephens, A., Veltri Torres, R., Strachota, S., Blanton, M., Gardiner, A., Stroud, R., & Knuth, E. (2022). Teacher Language and Gesture in an Intervention Focused on Developing Kindergarteners' Understandings of the Equal Sign. Proceedings of the forty-fourth annual meeting of the North American Chapter of the International Group for the Psychology of Mathematics Education. https://eric.ed.gov/?id=ED630361

`q1 · i?`

Qualitative coding of classroom video from one kindergarten classroom's seven equivalence lessons. The article reports "38 instances of the GSIDES code", alongside 28 ISA, 26 BAL, 17 DEF, 11 GBAL and 6 FORM instances. Counts are descriptive; no test is reported.

> "Across the seven lessons that addressed mathematical equivalence, we observed 26 instances of the BAL code, 17 instances of the DEF code, 6 instances of the FORM code, 28 instances of the ISA code, 11 instances of the GBAL code and 38 instances of the GSIDES code (see Table 2)."

## Discussion


## Related Claims
- [Teacher Shifts Equal Sign Language With Kindergarten Lesson Focus](teacher-shifts-equal-sign-language-with-kindergarten-lesson-focus.md)
- [Is The Same As With Sides Gesture Combination In Kindergarten Equation Talk](is-the-same-as-with-sides-gesture-combination-in-kindergarten-equation-talk.md)
