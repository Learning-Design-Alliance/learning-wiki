---
type: claim
title: Critical speculative design pedagogy supports critical consciousness and justice-oriented sensemaking in science
id: csdp-1
status: draft
generated:
  by: claude/unspecified
  at: 2026-08-29
evidence_strength: moderate
sources:
  - id: arad-et-al-2023
    resource: "https://doi.org/10.1080/10508406.2022.2154158"
    title: "Arad, K., Sanchez, A., & Bell, P. (2023). Youth as pattern makers for racial justice: How speculative design pedagogy in science can promote restorative futures through radical care practices. Journal of the Learning Sciences, 32(1), 76-109."
    author: "Arad, K., Sanchez, A., & Bell, P."
---

# Critical speculative design pedagogy supports critical consciousness and justice-oriented sensemaking in science

Middle-school students in a redesigned physics unit centering racial bias in light-related technologies showed increased emotional and critical engagement with anti-racist source material, synthesized readings into systemic (not individual) accounts of injustice, and produced transdisciplinary speculative artifacts — including, in one case, through intergenerational collaboration with a family member.

## Subclaims
`q2 i2` A focal student's annotation complexity and affective engagement increased markedly between an early, disengaged reading (solar-energy workplace inequity) and a later reading on racial bias in AI-driven dermatology, where she named dehumanization by race and asserted a need for diverse research samples. [→ Arad et al. 2023](#arad-et-al-2023)

`q2 i2` A second focal student wove together readings on melanin biology, skin-cancer health disparities, and sunscreen marketing to demand public-health accountability, synthesizing across sources into an account of systemic (not individual) neglect. [→ Arad et al. 2023](#arad-et-al-2023)

`q2 i2` One focal student's culminating "Visioning Board" was produced through intergenerational collaboration with her grandmother, who a teacher reported the student "beamed" about being told was a valid contributor rather than a source of "cheating." [→ Arad et al. 2023](#arad-et-al-2023)

## Evidence

### Arad et al. 2023

Arad, K., Sanchez, A., & Bell, P. (2023). Youth as pattern makers for racial justice: How speculative design pedagogy in science can promote restorative futures through radical care practices. *Journal of the Learning Sciences, 32*(1), 76-109. [https://doi.org/10.1080/10508406.2022.2154158](https://doi.org/10.1080/10508406.2022.2154158)

`q2 · ethnographic case study (158 students tracked, 13 with sufficient data for pathway analysis, 2 focal cases analyzed in depth) using thematic inductive analysis; no comparison group or quantified learning measure; instructional time was reduced mid-study by COVID scheduling changes` · `i2 · clear, well-documented qualitative patterns across multiple data sources (annotations, exit tickets, interviews, artifacts) for the focal cases, not quantified or causally isolated` · `n=158 8th-grade students across two Pacific Northwest middle schools, 65% Black, Brown, or Indigenous`

Two 8th-grade physics classes replaced a standard unit on light waves with [Threading, Weaving, Patternmaking](../patterns/threading-weaving-patternmaking.md), a three-phase sequence built around the driving question "How can and does light serve some communities and not other communities?" using readings on racial bias in solar-energy access, medical treatment, and AI-driven dermatology. The authors document, through annotation analysis, exit tickets, and interviews, two focal students' progression from initial disengagement to explicit critical synthesis and speculative, transdisciplinary artifact production (poetry, a collaboratively designed "Visioning Board"), framed through [Critical Speculative Design Pedagogy](../theories/critical-speculative-design-pedagogy.md).

## Discussion
This is a qualitative, ethnographic design study (q2), not an efficacy trial: there is no comparison condition and no quantified learning-outcome measure, and the most detailed evidence comes from two focal students out of 158 tracked (13 with sufficient data for broader pathway analysis). Implementation was affected by a COVID-driven mid-unit reduction in instructional time (from 75 to 28 minutes per session), which the authors note as a real-world constraint rather than a controlled variable. The pattern of increased engagement with anti-racist, emotionally resonant content is well-documented in the two focal cases but not established as reliable across the full cohort or across other content domains.

## Related Claims
- (none yet linked)
