---
type: claim
title: The global civic activism measurement model was fully valid with no items removed, appearing valid across contexts
description: The global civic activism measurement model was fully valid with no items removed, appearing valid across contexts
id: global-civic-activism-model-fully-valid
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
evidence_strength: moderate
sources:
  - id: nguyen-2021
    resource: "https://doi.org/10.31014/aior.1993.04.03.362"
    title: "Nguyen, M. T. (2021). Validating a Scale for Measuring Students' Perspectives toward Global Citizenship: A Case at a Private University in Vietnam. Education Quarterly Reviews, 4(3), 575-587. https://doi.org/10.31014/aior.1993.04.03.362"
    author: Nguyen, M. T.
    q: 2
    i: 1
  - id: nguyen-2021-2
    resource: "https://doi.org/10.31014/aior.1993.04.03.362"
    title: "Nguyen, M. T. (2021). Validating a Scale for Measuring Students' Perspectives toward Global Citizenship: A Case at a Private University in Vietnam. Education Quarterly Reviews, 4(3), 575-587. https://doi.org/10.31014/aior.1993.04.03.362"
    author: Nguyen, M. T.
    q: 2
    i: 1
---

# The global civic activism measurement model was fully valid with no items removed, appearing valid across contexts

> **Claim** · [All claims](index.md)
> **Evidence** · 2 studies · `q2` quasi-experiment · `i1` small

## Subclaims
`q2 i?` All indicators of the global civic activism model met the outer loading standard, and CR, AVE, cross-loadings, and HTMT criteria were satisfied without any removals. [→ Nguyen 2021](#nguyen-2021)
`q2 i?` The global civic activism model appeared valid across multiple contexts including this study and prior validations by Morais and Ogden (2011) and Roberts and Wilson (2016). [→ Nguyen 2021 (2)](#nguyen-2021-2)

## Evidence

### Nguyen 2021

Nguyen, M. T. (2021). Validating a Scale for Measuring Students' Perspectives toward Global Citizenship: A Case at a Private University in Vietnam. Education Quarterly Reviews, 4(3), 575-587. https://doi.org/10.31014/aior.1993.04.03.362

`q2 · i1`

PLS-SEM assessment of the global civic activism model: ICO (CR 0.902, AVE 0.651), PV (CR 0.879, AVE 0.646), GCA (CR 0.811, AVE 0.590); all HTMT figures under 0.85, so the model was valid.

> "Lastly, for the measurement model of Global Civic Activism, after examining Outer Factor Loading, all the indicators met the standard; therefore, there was no need to remove any indicators."

### Nguyen 2021 (2)

Nguyen, M. T. (2021). Validating a Scale for Measuring Students' Perspectives toward Global Citizenship: A Case at a Private University in Vietnam. Education Quarterly Reviews, 4(3), 575-587. https://doi.org/10.31014/aior.1993.04.03.362

`q2 · i1`

Conclusion-level statement comparing this validation with prior developed-country validations; the article calls this measurement model the most valid one in the study context.

> "the model of measuring Global Civic Activism appeared to be valid within many contexts, such as this study or the studies of Morais and Ogden (2011) and Roberts and Wilson (2016)."

## Discussion


## Related Claims
- [The original global citizenship scale is employable in a developing-country university context after adjustments to social responsibility and global competence](global-citizenship-scale-employable-developing-context.md) — a broader claim this one bears on
- [The social responsibility measurement model required the most item removals, with global justice and personal responsibility items dropped in the Vietnamese context](social-responsibility-model-items-removed-vietnam.md) — related
- [Only two intercultural communication items (IC1, IC2) were removed from the global competence model, attributed to limited intercultural exposure](global-competence-model-ic-items-removed.md) — related
