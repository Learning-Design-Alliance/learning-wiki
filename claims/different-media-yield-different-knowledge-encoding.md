---
type: claim
title: People acquire different knowledge from different media, the closest indirect evidence for symbolic encoding specificity
description: People acquire different knowledge from different media, the closest indirect evidence for symbolic encoding specificity
id: different-media-yield-different-knowledge-encoding
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
evidence_strength: weak
sources:
  - id: shavelson-1986
    resource: "https://eric.ed.gov/?id=ED338680"
    title: "Shavelson, Richard J.; Webb, Noreen M.; Lehman, Penny. (1986). The Role of Symbol Systems in Problem Solving: A Literature Review. CSE Report No. 269. https://eric.ed.gov/?id=ED338680"
    author: Shavelson, Richard J.; Webb, Noreen M.; Lehman, Penny
    q: 2
    i: "?"
---

# People acquire different knowledge from different media, the closest indirect evidence for symbolic encoding specificity

> **Claim** · [All claims](index.md)
> **Evidence** · 1 study · `q2` quasi-experiment

## Subclaims
`q2 i?` Learning from a map yields global spatial-relation images, while learning through navigation yields route knowledge. [→ Shavelson 1986](#shavelson-1986)

## Evidence

### Shavelson 1986

Shavelson, Richard J.; Webb, Noreen M.; Lehman, Penny. (1986). The Role of Symbol Systems in Problem Solving: A Literature Review. CSE Report No. 269. https://eric.ed.gov/?id=ED338680

`q2 · i?`

The review reports, citing Thorndyke and Hayes-Roth's (1982) map-learning study, that people acquire different knowledge from different media, calling this the closest indirect evidence for symbolic encoding specificity; the review states it cannot find research directly related to symbolic encoding specificity.

> "when learning from a map, people encode global spatial relations, images that can be scanned and measured like a physical map; whereas when learning through navigation, people acquire knowledge about the routes"

## Discussion


## Related Claims
-
