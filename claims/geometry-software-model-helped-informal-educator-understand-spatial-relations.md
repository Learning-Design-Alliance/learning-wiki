---
type: claim
title: A dynamic geometry model of the quilt problem helped an informal educator understand its spatial relations
description: A dynamic geometry model of the quilt problem helped an informal educator understand its spatial relations
id: geometry-software-model-helped-informal-educator-understand-spatial-relations
status: draft
generated:
  by: claude/unspecified
  at: 2026-09-24
evidence_strength: weak
sources:
  - id: lili-zhou-2023
    resource: "https://eric.ed.gov/?id=ED658339"
    title: "Lili Zhou. (2023). Humanizing Mathematics through Boundary Crossing Collaboration. Proceedings of the forty-fifth annual meeting of the North American Chapter of the International Group for the Psychology of Mathematics Education (Vol. 2). https://eric.ed.gov/?id=ED658339"
    author: Lili Zhou
---

# A dynamic geometry model of the quilt problem helped an informal educator understand its spatial relations

> **Claim** · [All claims](index.md)

## Subclaims
`q1 i?` After email explanation failed, a geometry-software model of the fabric and quilt was reported by the informal educator as very helpful for understanding the spatial relations. [→ Lili Zhou 2023](#lili-zhou-2023)

## Evidence

### Lili Zhou 2023

Lili Zhou. (2023). Humanizing Mathematics through Boundary Crossing Collaboration. Proceedings of the forty-fifth annual meeting of the North American Chapter of the International Group for the Psychology of Mathematics Education (Vol. 2). https://eric.ed.gov/?id=ED658339

`q1 · i?`

Identification stage of the narrative inquiry; the informal educator's own report after the researcher built a geometry-software model. She "thought the visual model is very helpful for her to understand the spatial relations". Single participant's perception.

> "Laura thought the visual model is very helpful for her to understand the spatial relations."

## Discussion


## Related Claims
-
