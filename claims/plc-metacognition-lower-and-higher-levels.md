---
type: claim
title: Teacher metacognition in PLCs appears at both a lower level (reflecting on what one does) and a higher level (reflecting on how one reflects)
description: Teacher metacognition in PLCs appears at both a lower level (reflecting on what one does) and a higher level (reflecting on how one reflects)
id: plc-metacognition-lower-and-higher-levels
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
evidence_strength: weak
sources:
  - id: prytula-2012
    resource: "https://doi.org/10.5539/ies.v5n4p112"
    title: "Prytula, M. P. (2012). Teacher Metacognition within the Professional Learning Community. International Education Studies. https://doi.org/10.5539/ies.v5n4p112"
    author: Prytula, M. P.
    q: 2
    i: "?"
---

# Teacher metacognition in PLCs appears at both a lower level (reflecting on what one does) and a higher level (reflecting on how one reflects)

> **Claim** · [All claims](index.md)
> **Evidence** · 1 study · `q2` quasi-experiment

## Subclaims
`q2 i?` Participants' accounts showed both lower level metacognition and higher level metacognition, with higher level thinking nurturing metacognition. [→ Prytula 2012](#prytula-2012)

## Evidence

### Prytula 2012

Prytula, M. P. (2012). Teacher Metacognition within the Professional Learning Community. International Education Studies. https://doi.org/10.5539/ies.v5n4p112

`q2 · i?`

The findings apply Von Wright's (1992) situated-metacognition distinction to the three cases: Lisa and Tracey recognized lower-level reflection on classroom processes, while Jacki and Tracey described higher-level reflection on how they reflect. Both levels were evident in the discussion section.

> "In contrast to thinking at a lower level, where one engages in reflection on what one does, thinking at a higher level includes a reflection on  how one reflects."

## Discussion


## Related Claims
-
