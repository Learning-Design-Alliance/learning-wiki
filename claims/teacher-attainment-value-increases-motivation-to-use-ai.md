---
type: claim
title: Teachers who believe AI use will contribute to their professional success and identity (attainment value) may be more motivated to use it (theoretical argument).
description: Teachers who believe AI use will contribute to their professional success and identity (attainment value) may be more motivated to use it (theoretical argument).
id: teacher-attainment-value-increases-motivation-to-use-ai
status: draft
generated:
  by: claude/unspecified
  at: 2026-09-24
evidence_strength: weak
sources:
  - id: yurt-2024
    resource: "https://eric.ed.gov/?id=ED673212"
    title: "Yurt, E. (2024). Theoretical Examination of Teachers' Motivations for Using Artificial Intelligence Applications in Education: Expectancy-Value Theory Perspective. In M. S. Ozturk, A. Kaban, & M. Unal (Eds.), Proceedings of ICSES 2024-- International Conference on Studies in Education and Social Sciences (pp. 149-158), Istanbul, Turkiye. ISTES. https://eric.ed.gov/?id=ED673212"
    author: Yurt, E.
    q: 1
    i: "?"
---

# Teachers who believe AI use will contribute to their professional success and identity (attainment value) may be more motivated to use it (theoretical argument).

> **Claim** · [All claims](index.md)
> **Evidence** · 1 study · `q1` argument or single case

## Subclaims
`q1 i?` The paper argues that when teachers believe AI technologies will significantly contribute to their professional success and identity, they may be more motivated to use them. [→ Yurt 2024](#yurt-2024)

## Evidence

### Yurt 2024

Yurt, E. (2024). Theoretical Examination of Teachers' Motivations for Using Artificial Intelligence Applications in Education: Expectancy-Value Theory Perspective. In M. S. Ozturk, A. Kaban, & M. Unal (Eds.), Proceedings of ICSES 2024-- International Conference on Studies in Education and Social Sciences (pp. 149-158), Istanbul, Turkiye. ISTES. https://eric.ed.gov/?id=ED673212

`q1 · i?`

Theoretical argument in the attainment value section, with no new data: attainment value is tied to teachers' professional identities, and "they may be more motivated to use these technologies" when AI is seen as contributing to professional success.

> "Attainment value is directly related to teachers' professional identities, personal goals, and professional development. When teachers believe that using AI technologies will significantly contribute to their professional success and identity, they may be more motivated to use these technologies."

## Discussion


## Related Claims
- [Teacher Utility Value Increases Motivation To Use Ai](teacher-utility-value-increases-motivation-to-use-ai.md)
