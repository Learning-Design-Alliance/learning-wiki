---
type: claim
title: SEL Programs Improve Academic Achievement
status: draft
generated:
  by: claude/unspecified
  at: 2026-09-25
id: sel-programs-improve-academic-achievement
evidence_strength:
sources:
  - id: durlak-et-al-2011
    resource: "https://doi.org/10.1111/j.1467-8624.2010.01564.x"
    title: "Durlak, J. A., Weissberg, R. P., Dymnicki, A. B., Taylor, R. D., & Schellinger, K. B. (2011). The Impact of Enhancing Students' Social and Emotional Learning: A Meta-Analysis of School-Based Universal Interventions. *Child Development, 82*(1), 405–432. [doi:10.1111/j.1467-8624.2010.01564.x](https://doi.org/10.1111/j.1467-8624.2010.01564.x)"
    author: "Durlak, J. A., Weissberg, R. P., Dymnicki, A. B., Taylor, R. D., & Schellinger, K. B."
    q: 4
    i: 1
    n: 213 studies (270,034 students)
  - id: cipriano-et-al-2023
    resource: "https://doi.org/10.1111/cdev.13968"
    title: "Cipriano, C., Strambler, M. J., Naples, L. H., Ha, C., Kirk, M., Wood, M., Sehgal, K., Zieher, A. K., Eveleigh, A., McCarthy, M., Funaro, M., Ponnock, A., Chow, J. C., & Durlak, J. (2023). The state of evidence for social and emotional learning: A contemporary meta-analysis of universal school-based SEL interventions. *Child Development, 94*(5), 1181–1204. [doi:10.1111/cdev.13968](https://doi.org/10.1111/cdev.13968)"
    author: "Cipriano, C., Strambler, M. J., Naples, L. H., Ha, C., Kirk, M., Wood, M., Sehgal, K., Zieher, A. K., Eveleigh, A., McCarthy, M., Funaro, M., Ponnock, A., Chow, J. C., & Durlak, J."
    q: 4
    i: "?"
    n: 424 studies (575,361 students)
  - id: zhao-and-sang-2025
    resource: "https://doi.org/10.3390/bs15111527"
    title: "Zhao, Y., & Sang, B. (2025). The Effect of Social–Emotional Learning Programs on Elementary and Middle School Students' Academic Achievement: A Meta-Analytic Review. *Behavioral Sciences, 15*(11), 1527. [doi:10.3390/bs15111527](https://doi.org/10.3390/bs15111527)"
    author: "Zhao, Y., & Sang, B."
    q: 3
    i: 0
    n: 22 studies (24,510 students)
---

# SEL Programs Improve Academic Achievement

> **Claim** · [All claims](index.md)
> **Evidence** · 3 studies · `q3`–`q4` · `i0`–`i1`

Social-emotional learning (SEL) programs — structured curricula that teach self-awareness, self-management, social awareness, relationship skills, and responsible decision-making — produce gains in academic achievement alongside their social-emotional outcomes.

## Subclaims

`q4 i1` Across 213 universal school-based SEL programs (K–12), participants outperformed controls on conduct problems, positive social behaviour and academic performance at post-test, with small effects (ES 0.22–0.27), and academic gains were significant only when school staff delivered the program. [→ Durlak et al. 2011](#durlak-et-al-2011)

`q4 i?` A contemporary meta-analysis of 424 studies (2008–2020, 53 countries) reports significantly improved behaviours, school functioning and academic achievement for students in universal school-based SEL interventions; effect sizes were not read here (abstract only). [→ Cipriano et al. 2023](#cipriano-et-al-2023)

`q3 i0` Restricted to RCTs and rigorous quasi-experiments reporting quantitative achievement outcomes (2011–2021), SEL programs' effect on overall academic achievement is positive but negligible (g = 0.08), and smaller still in randomised studies (g = 0.063), which qualifies the size of the achievement claim. [→ Zhao and Sang 2025](#zhao-and-sang-2025)

## Evidence

### Durlak et al. 2011

Durlak, J. A., Weissberg, R. P., Dymnicki, A. B., Taylor, R. D., & Schellinger, K. B. (2011). The Impact of Enhancing Students' Social and Emotional Learning: A Meta-Analysis of School-Based Universal Interventions. *Child Development, 82*(1), 405–432. [doi:10.1111/j.1467-8624.2010.01564.x](https://doi.org/10.1111/j.1467-8624.2010.01564.x)

`q4 · meta-analysis` · `i1 · small effect, ES=0.27 academic performance, ES=0.22 conduct problems` · `n=213 studies (270,034 students)`

A meta-analysis of 213 universal, school-based social and emotional learning programs for kindergarten through high school students, compared with control groups at post-test. All six outcome categories improved significantly: SEL skills (ES 0.57), attitudes (0.23), positive social behaviour (0.24), conduct problems (0.22), emotional distress (0.24) and academic performance (0.27, from 35 interventions), which the authors translate into an 11-percentile-point gain in achievement. Academic performance improved significantly only when school staff ran the program (classroom teacher delivery ES 0.34; non-school personnel ES 0.12, not significant). Effects were moderated by use of four recommended skill-development practices (SAFE) and by reported implementation problems; the 33 studies with follow-up at least six months later still showed significant effects, including academic performance (ES 0.32, k = 8).

### Cipriano et al. 2023

Cipriano, C., Strambler, M. J., Naples, L. H., Ha, C., Kirk, M., Wood, M., Sehgal, K., Zieher, A. K., Eveleigh, A., McCarthy, M., Funaro, M., Ponnock, A., Chow, J. C., & Durlak, J. (2023). The state of evidence for social and emotional learning: A contemporary meta-analysis of universal school-based SEL interventions. *Child Development, 94*(5), 1181–1204. [doi:10.1111/cdev.13968](https://doi.org/10.1111/cdev.13968)

`q4 · systematic review and meta-analysis` · `i? · effect sizes not in the abstract read` · `n=424 studies (575,361 students)`

A systematic review and meta-analysis of universal school-based SEL interventions for kindergarten to 12th-grade students, covering studies from 2008 through 2020: 424 studies from 53 countries, 252 distinct interventions and 575,361 students. Compared with control conditions, participating students showed significantly improved skills, attitudes, behaviours, school climate and safety, peer relationships, school functioning and academic achievement. Substantial heterogeneity in program content, features, context and implementation quality moderated outcomes. Only the abstract was read, so no pooled effect size is recorded here.

### Zhao and Sang 2025

Zhao, Y., & Sang, B. (2025). The Effect of Social–Emotional Learning Programs on Elementary and Middle School Students' Academic Achievement: A Meta-Analytic Review. *Behavioral Sciences, 15*(11), 1527. [doi:10.3390/bs15111527](https://doi.org/10.3390/bs15111527)

`q3 · meta-analysis` · `i0 · negligible effect, g=0.08` · `n=22 studies (24,510 students)`

A meta-analysis of 22 studies (17 universal, 5 targeted programs; 2011–2021) of SEL programs for elementary and middle school students, limited to randomised or rigorous quasi-experimental designs with quantitative achievement outcomes. SEL students showed higher overall academic achievement than controls, but the effect was very small (g = 0.08, 95% CI 0.05 to 0.12), similar across English language arts, mathematics and science, and larger for GPA (g = 0.33, from 6 effect sizes). Quasi-experimental studies showed significantly larger effects (g = 0.27, k = 12) than randomised trials (g = 0.063, k = 64), and 20 of the 22 studies were in elementary schools, so middle-school conclusions are preliminary. This does not contradict the claim's direction, but it indicates the academic benefit in rigorous recent trials is much smaller than the Durlak et al. (2011) estimate.

## Discussion

**Proposed mechanism.** The mechanism most commonly proposed is indirect: SEL programs improve classroom behavior, engagement, and student–teacher relationships, which in turn create conditions for more instructional time and better learning. This pathway parallels the belonging-to-engagement-to-achievement chain described in [Belonging interventions improve outcomes](belonging-interventions-improve-outcomes.md) and is consistent with the motivational account in [Autonomy supports intrinsic motivation](autonomy-supports-intrinsic-motivation.md). Programs that integrate social-emotional skill practice into academic content (rather than delivering it as a stand-alone add-on) are generally argued to show stronger academic spillover, because the skills are practiced in the context where they must be applied.

**Boundary conditions.** Effects on achievement are typically smaller than effects on social-emotional competencies themselves, and depend on implementation quality — dosage, fidelity, and teacher training are recurring moderators in the broader SEL literature. Programs delivered poorly or as brief one-off interventions should not be expected to move achievement. There is also an open question about developmental timing: whether early-childhood, elementary, or secondary implementation yields the largest academic return remains contested.

**Causal direction.** Because many supporting studies are quasi-experimental or rely on school-level adoption, selection effects (schools that adopt SEL programs differ systematically from those that do not) cannot be fully ruled out. Randomized trials with academic outcome measures are needed before the achievement effect can be treated as firmly established.

**Status.** This page currently has no verified Evidence entries, so no effect size or specific study claim should be relied on yet. Studies (ideally randomized trials or meta-analytic syntheses with academic outcome measures) still need to be added before this claim can be treated as supported.

## Related Claims

- [Belonging interventions improve outcomes](belonging-interventions-improve-outcomes.md) — social-emotional factors like belonging are a plausible pathway for SEL's academic effects
- [Autonomy supports intrinsic motivation](autonomy-supports-intrinsic-motivation.md) — motivational climate is a moderator of whether SEL gains transfer to achievement
- [Collaborative learning improves outcomes](collaborative-learning-improves-outcomes.md) — relationship skills taught by SEL programs underpin effective collaboration
- [Assessment for learning improves achievement](assessment-for-learning-improves-achievement.md) — an alternative instructional route to the same achievement outcome
- [Social-emotional learning benefits persist at follow-up](sel-benefits-persist-follow-up.md) — related
- [SEL Programs Improve Behavior And Achievement](sel-programs-improve-behavior-and-achievement.md) — possibly the same claim (merge candidate)
- [SEL programs improve social emotional skills](sel-programs-improve-social-emotional-skills.md) — related