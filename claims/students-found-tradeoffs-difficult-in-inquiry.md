---
type: claim
title: Many students in the digital-images inquiry found identifying engineering tradeoffs difficult despite the budget constraint design
description: Many students in the digital-images inquiry found identifying engineering tradeoffs difficult despite the budget constraint design
id: students-found-tradeoffs-difficult-in-inquiry
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
evidence_strength: weak
sources:
  - id: morzinski-2010
    resource: "https://arxiv.org/abs/1009.3297"
    title: "Morzinski, K. M., Crockett, C. J., and Crossfield, I. J. (2010). Digital image exploration at Maui Community College. ASP Conference Series. https://arxiv.org/abs/1009.3297"
    author: Morzinski, K. M., Crockett, C. J., and Crossfield, I. J.
    q: 1
    i: 1
---

# Many students in the digital-images inquiry found identifying engineering tradeoffs difficult despite the budget constraint design

> **Claim** · [All claims](index.md)
> **Evidence** · 1 study · `q1` argument or single case · `i1` small

## Subclaims
`q1 i?` The course instructor reported that many students still found identifying tradeoffs to be difficult when reviewing the Day 2 write-up, and the designers reflected that the budgetary constraint may have been too complicated. [→ Morzinski 2010](#morzinski-2010)

## Evidence

### Morzinski 2010

Morzinski, K. M., Crockett, C. J., and Crossfield, I. J. (2010). Digital image exploration at Maui Community College. ASP Conference Series. https://arxiv.org/abs/1009.3297

`q1 · i1`

Instructor feedback on student homework reports in the Fall 2008 offering, reported by the design team as their own reflection. The article offers no quantitative measure of tradeoff difficulty.

> "We got feedback from the course instructor Elisabeth Reader, in reviewing the write-up assigned on Day 2, that many students still found identifying tradeoﬀs to be diﬃcult."

## Discussion


## Related Claims
- [Students rated the image decoding, poster sharing, and synthesis lecture components of a digital-images inquiry highest on a five-point feedback scale](digital-images-inquiry-student-feedback-ratings.md) — related
- [Pre-service teachers struggle with instructional design and lesson planning despite existing standards and coursework](pre-service-teachers-struggle-lesson-design.md) — related
