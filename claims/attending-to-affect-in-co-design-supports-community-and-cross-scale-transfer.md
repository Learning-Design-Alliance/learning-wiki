---
type: claim
title: Attending to affect in co-design supports community formation and cross-scale transfer
id: hcd-1
status: draft
generated:
  by: claude/unspecified
  at: 2026-08-29
evidence_strength: moderate
sources:
  - id: potvin-et-al-2024
    resource: "https://doi.org/10.1080/10508406.2024.2318557"
    title: "Potvin, A. S., Teeters, L. P., Penuel, W. R., & Dimidjian, S. (2024). Humanizing Co-design through attention to educators' affective and relational experiences. Journal of the Learning Sciences, 33(1), 41-79."
    author: "Potvin, A. S., Teeters, L. P., Penuel, W. R., & Dimidjian, S."
---

# Attending to affect in co-design supports community formation and cross-scale transfer

When a co-design process deliberately affirms educators' affective and relational experience — through ritualized check-ins, shared contemplative practice, and reflective field-note protocols — educators report a stronger sense of community, apply what they learn across personal, classroom, and systems-level scales, and the design object itself expands from an individual course toward a collective vision for systems change.

## Subclaims
`q2 i1` Educators named 43 distinct affective experiences across 14 co-design meetings (21 negative, 22 positive valence); sharing and discussing negative experiences (frustration, isolation, self-criticism) was explicitly reported by educators as strengthening the group's sense of community rather than weakening it. [→ Potvin et al. 2024](#potvin-et-al-2024)

`q2 i1-i2` Educators reported applying compassion- and dignity-related practices learned in co-design across multiple scales beyond the immediate design task: personal contemplative practice, classroom teaching (e.g., teaching students breathing/intention-setting), and reflections on parenting and colleague relationships. [→ Potvin et al. 2024](#potvin-et-al-2024)

`q2 i1-i2` Over the course of co-design, the group's stated design object expanded from a bounded task (a 4-course digital sequence for educators) to a broader collective vision addressing systems-level inequities (discipline policy, teacher evaluation, trauma support), tracked through meeting transcripts and the final course structure. [→ Potvin et al. 2024](#potvin-et-al-2024)

## Evidence

### Potvin et al. 2024

Potvin, A. S., Teeters, L. P., Penuel, W. R., & Dimidjian, S. (2024). Humanizing Co-design through attention to educators' affective and relational experiences. *Journal of the Learning Sciences, 33*(1), 41-79. [https://doi.org/10.1080/10508406.2024.2318557](https://doi.org/10.1080/10508406.2024.2318557)

`q2 · qualitative case study with systematic inductive/deductive coding (90% inter-rater reliability), single district, no comparison group` · `i1-i2 · clear, well-documented qualitative pattern across multiple data sources, no quantified effect size` · `n=10 educators (5 teachers, 3 counselors, 1 principal, 1 librarian) across 6 schools in one district; 14 co-design meetings, 58 total hours, over roughly 8 months`

Ten educators and a research team completed an 8-week shared contemplative training together before beginning 14 co-design meetings (October 2019-May 2020, transitioning to virtual meetings after March 2020) to design a compassion-and-dignity curriculum for educators, following the [Humanizing Co-Design pattern](../patterns/humanizing-co-design-with-educators.md). Coded meeting transcripts, field notes, artifacts, and post-training interviews showed a roughly even mix of positive and negative named emotions, and educators explicitly credited discussing difficult emotions (isolation, self-criticism, burnout) with building a stronger sense of community and helping them apply concepts beyond the meetings themselves — in their classrooms, with colleagues, and at home. Facilitators' responsive adaptation (e.g., canceling a scheduled meeting for an unstructured check-in when COVID-19 disrupted the district) is documented as one instance of this affect-attentive facilitation in practice.

## Discussion
This is a well-documented single-site qualitative case study (q2) with real methodological rigor (systematic coding, multiple data sources, 90% inter-rater reliability) but no comparison condition and a small, non-random sample from a district with a pre-existing research partnership, which may have made trust-building easier than in a cold-start relationship. The finding that discussing negative affect strengthened rather than weakened community is a genuine and useful counterpoint to designs that treat emotional expression as off-task, but it rests on self-report (interviews, reflections) rather than independent behavioral measurement, and the study coincided with the COVID-19 pandemic, which may have intensified both participants' need for and openness to this kind of support in ways that would not generalize to calmer periods.

## Related Claims
- (none yet linked)
