...
**What "reduced search" predicts.** If the mechanism is correct, the largest benefits should appear (a) for learners with minimal prior knowledge in the domain, (b) for problems with many possible solution paths where means–ends search is most costly, and (c) on measures of cognitive load and study-time efficiency, not only on final test scores. Studies of this claim should therefore report prior-knowledge baselines and load measures, not just outcome scores, to be diagnostic of the mechanism rather than the general example benefit.
