---
type: claim
title: Baseline performance differed by later treatment group for two tasks, with the delayed-reward group giving fewer baseline responses, while no sex or order effects emerged
description: Baseline performance differed by later treatment group for two tasks, with the delayed-reward group giving fewer baseline responses, while no sex or order effects emerged
id: baseline-group-effect-no-sex-order-effects
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-25
evidence_strength: weak
sources:
  - id: ward-1970
    resource: "https://eric.ed.gov/?id=ED045766"
    title: "Ward, William C., Nathan Kogan, and Ethel Pankove. (1970). Motivation and Capacity in Children's Creativity. Research Bulletin. Educational Testing Service. https://eric.ed.gov/?id=ED045766"
    author: Ward, William C., Nathan Kogan, and Ethel Pankove
    q: 3
    i: "?"
  - id: ward-1970-2
    resource: "https://eric.ed.gov/?id=ED045766"
    title: "Ward, William C., Nathan Kogan, and Ethel Pankove. (1970). Motivation and Capacity in Children's Creativity. Research Bulletin. Educational Testing Service. https://eric.ed.gov/?id=ED045766"
    author: Ward, William C., Nathan Kogan, and Ethel Pankove
    q: 3
    i: "?"
---

# Baseline performance differed by later treatment group for two tasks, with the delayed-reward group giving fewer baseline responses, while no sex or order effects emerged

> **Claim** · [All claims](index.md)
> **Evidence** · 2 studies · `q3` peer-reviewed experiment

## Subclaims
`q3 i?` A significant group effect appeared in baseline data for Similarities and Line Drawings, with the Delayed Reward group giving fewer baseline responses. [→ Ward 1970](#ward-1970)
`q3 i?` No sex differences or order effects were found in the baseline data. [→ Ward 1970 (2)](#ward-1970-2)

## Evidence

### Ward 1970

Ward, William C., Nathan Kogan, and Ethel Pankove. (1970). Motivation and Capacity in Children's Creativity. Research Bulletin. Educational Testing Service. https://eric.ed.gov/?id=ED045766

`q3 · i?`

Four 3 x 2 x 2 analyses of variance on baseline data per task found a group effect for Similarities and Line Drawings; "Subjects who received the Delayed Reward treatment in Session 2 gave fewer responses in base line assessment".

> "there was a significant group effect for subjects whose base line assessment required naming similarities and interpreting line drawings (F = 4.14for Similarities and 4.26 for Line Drawings;df = 2,83; p < .025).Subjects who received the Delayed Reward treatment in Session 2 gave fewer responses in base line assessment than did subjects who received the other treatments."

### Ward 1970 (2)

Ward, William C., Nathan Kogan, and Ethel Pankove. (1970). Motivation and Capacity in Children's Creativity. Research Bulletin. Educational Testing Service. https://eric.ed.gov/?id=ED045766

`q3 · i?`

Null results from the same baseline analyses of variance: "No sex differences or order effects were found" across the four tasks.

> "No sex differences or order effects were found, but there was a significant group effect for subjects whose base line assessment required naming similarities and interpreting line drawings"

## Discussion


## Related Claims
-
