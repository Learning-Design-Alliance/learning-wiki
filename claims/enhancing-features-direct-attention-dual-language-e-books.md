---
type: claim
title: "Audio narration with finger-tracking animation directs bilingual preschoolers' attention to the target-language print in dual-language e-books, including the nondominant language"
description: "Audio narration with finger-tracking animation directs bilingual preschoolers' attention to the target-language print in dual-language e-books, including the nondominant language"
id: enhancing-features-direct-attention-dual-language-e-books
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
evidence_strength: moderate
sources:
  - id: yow-2019
    resource: "https://doi.org/10.1177/2332858419878126"
    title: "Yow, W. Q., & Priyashri, S. (2019). Computerized Electronic Features Direct Children's Attention to Print in Single- and Dual-Language e-Books. AERA Open. https://doi.org/10.1177/2332858419878126"
    author: "Yow, W. Q., & Priyashri, S."
    q: 3
    i: 3
  - id: yow-2019-2
    resource: "https://doi.org/10.1177/2332858419878126"
    title: "Yow, W. Q., & Priyashri, S. (2019). Computerized Electronic Features Direct Children's Attention to Print in Single- and Dual-Language e-Books. AERA Open. https://doi.org/10.1177/2332858419878126"
    author: "Yow, W. Q., & Priyashri, S."
    q: 3
    i: 3
---

# Audio narration with finger-tracking animation directs bilingual preschoolers' attention to the target-language print in dual-language e-books, including the nondominant language

> **Claim** · [All claims](index.md)
> **Evidence** · 2 studies · `q3` peer-reviewed experiment · `i3` large

## Subclaims
`q3 i3` When English narration and animation tracked English sentences, children's PLT to English text greatly exceeded PLT to Mandarin text, F(1, 30) = 85.95, p < .001, partial η2 = .74. [→ Yow 2019](#yow-2019)
`q3 i3` When Mandarin narration and animation tracked Mandarin sentences, children looked longer at Mandarin text during animation, t(30) > 5.83, p < .001, d = 2.06, though the main effect of Text Language was only near significant, F(1, 29) = 3.98, p = .056. [→ Yow 2019 (2)](#yow-2019-2)

## Evidence

### Yow 2019

Yow, W. Q., & Priyashri, S. (2019). Computerized Electronic Features Direct Children's Attention to Print in Single- and Dual-Language e-Books. AERA Open. https://doi.org/10.1177/2332858419878126

`q3 · i3`

Study 1 enhanced English dual-language reading condition (eye-tracking, within-subjects). During animation, mean PLT was 0.86 to English versus 0.14 to Mandarin; children's "PLTE was significantly longer than PLTM when a finger-pointing animation tracked the English sentences."

> "Results revealed a significant main effect of Text Language, F(1, 30) = 85.95, p < .001, partial η2 = .74. Children's PLTE was significantly longer than PLTM when a finger-pointing animation tracked the English sentences while a voice read the sentences out loud."

### Yow 2019 (2)

Yow, W. Q., & Priyashri, S. (2019). Computerized Electronic Features Direct Children's Attention to Print in Single- and Dual-Language e-Books. AERA Open. https://doi.org/10.1177/2332858419878126

`q3 · i3`

Study 1 enhanced Mandarin condition: during animation children's "PLT to the target (Mandarin) text was significantly higher than PLT to the English text during animation phase" (d = 2.06), showing multimedia features can override the dominant-language bias for the nondominant language.

> "Further analyses revealed that children's PLT to the target (Mandarin) text was significantly higher than PLT to the English text during animation phase, t (30) > 5.83, p < .001, d = 2.06, but this difference was no longer significant after the animation ended"

## Discussion


## Related Claims
-
