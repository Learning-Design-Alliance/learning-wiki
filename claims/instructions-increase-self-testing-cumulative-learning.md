---
type: claim
title: Instructions increased self-testing and cumulative learning but not study choices or feedback seeking in Experiment 1
description: Instructions increased self-testing and cumulative learning but not study choices or feedback seeking in Experiment 1
id: instructions-increase-self-testing-cumulative-learning
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-25
evidence_strength: moderate
sources:
  - id: ariel-2018
    resource: "https://doi.org/10.1037/xap0000133"
    title: "Ariel, R., & Karpicke, J. D. (2018). Improving Self-Regulated Learning With a Retrieval Practice Intervention. Journal of Experimental Psychology: Applied. https://doi.org/10.1037/xap0000133"
    author: "Ariel, R., & Karpicke, J. D."
    q: 3
    i: 2
  - id: ariel-2018-2
    resource: "https://doi.org/10.1037/xap0000133"
    title: "Ariel, R., & Karpicke, J. D. (2018). Improving Self-Regulated Learning With a Retrieval Practice Intervention. Journal of Experimental Psychology: Applied. https://doi.org/10.1037/xap0000133"
    author: "Ariel, R., & Karpicke, J. D."
    q: 3
    i: 2
---

# Instructions increased self-testing and cumulative learning but not study choices or feedback seeking in Experiment 1

> **Claim** · [All claims](index.md)
> **Evidence** · 2 studies · `q3` peer-reviewed experiment · `i2` medium

## Subclaims
`q3 i2` The instructed group chose to self-test more than the control group; study choice frequency did not differ. [→ Ariel 2018](#ariel-2018)
`q3 i2` The instructed group recalled 90% of items at least once during learning versus 70% in the control group, and attempted retrieval for nearly all items. [→ Ariel 2018 (2)](#ariel-2018-2)

## Evidence

### Ariel 2018

Ariel, R., & Karpicke, J. D. (2018). Improving Self-Regulated Learning With a Retrieval Practice Intervention. Journal of Experimental Psychology: Applied. https://doi.org/10.1037/xap0000133

`q3 · i2`

Experiment 1 analysis of three self-regulated learning measures: study decisions, self-test decisions, and feedback choices per item. Study choices did not differ, t(58) = 0.70, p = .49; feedback choices were at ceiling (96%+ of trials) in both groups and "did not differ much across groups".

> "the retrieval practice instructions group chose to self-test more than did the control group, t(58) /H110053.23, p /H11021.01, d /H110050.83 [0.30, 1.36]"

### Ariel 2018 (2)

Ariel, R., & Karpicke, J. D. (2018). Improving Self-Regulated Learning With a Retrieval Practice Intervention. Journal of Experimental Psychology: Applied. https://doi.org/10.1037/xap0000133

`q3 · i2`

Experiment 1 cumulative learning analysis across test practice blocks (Table 2). The instructed group also used retrieval practice at least once for 99% of items versus 88% in control, t(58) = 2.51, p < .05, d = 0.65.

> "The retrieval practice instructions group recalled 90% of items at least once during learning compared to 70% in the control group, t(58) /H110052.73, p /H11021.01, d /H110050.71 [0.18, 1.22]."

## Discussion


## Related Claims
- [Instructed students allocated more total time to learning, and time was correlated with successful retrieval attempts](instructions-increase-learning-time-allocation.md) — related
- [Learners misjudge which learning strategies are effective](learners-misjudge-effective-learning-strategies.md) — related
- [Effects of instructions on some measures were inconsistent across experiments, and instructed students did not recall all items to criterion](mixed-effects-across-experiments-criterion-failures.md) — related
