---
type: claim
title: Real-time data collection made the momentum lab substantially more efficient, cutting lab duration from two hours to one
description: Real-time data collection made the momentum lab substantially more efficient, cutting lab duration from two hours to one
id: real-time-data-collection-increases-lab-efficiency
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
evidence_strength: weak
sources:
  - id: brewe-2007
    resource: "https://scholar.google.com/scholar?q=Impacts+of+real-time+data+collection+on+introductory+algebra-based+physics"
    title: "Brewe, E., Sawtelle, V., and Pamela, P. (2007). Impacts of real-time data collection on introductory algebra-based physics. https://scholar.google.com/scholar?q=Impacts+of+real-time+data+collection+on+introductory+algebra-based+physics"
    author: Brewe, E., Sawtelle, V., and Pamela, P.
    q: 2
    i: "?"
---

# Real-time data collection made the momentum lab substantially more efficient, cutting lab duration from two hours to one

> **Claim** · [All claims](index.md)
> **Evidence** · 1 study · `q2` quasi-experiment

## Subclaims
`q2 i?` The ethnographer recorded that the Fall 2005 technology lab lasted only an hour whereas the Fall 2004 lab lasted a full two hours, indicating technology significantly increased class efficiency. [→ Brewe 2007](#brewe-2007)

## Evidence

### Brewe 2007

Brewe, E., Sawtelle, V., and Pamela, P. (2007). Impacts of real-time data collection on introductory algebra-based physics. https://scholar.google.com/scholar?q=Impacts+of+real-time+data+collection+on+introductory+algebra-based+physics

`q2 · i?`

Ethnographic observation of the momentum labs in both years by a supporting faculty member. The duration contrast is a qualitative field-note observation, not a controlled timing measurement; students engaged in the same type of activities in both years.

> "Also the ethnographer indicated in her notes that the lab lasted only an hour, whereas during Fall 2004, the lab lasted a full two hours.  This indicates that the technology increased the efficiency of the class significantly even though the students were engaged in the same type of activities."

## Discussion


## Related Claims
- [Students in a technology-enhanced modeling physics class learned significantly more than students in an otherwise identical no-technology class](real-time-data-collection-increases-modeling-physics-learning.md) — related
- [Students who experienced the technology-enhanced class were far more favorable toward technology (76.0%) than no-technology students (20.4%)](student-survey-favorable-toward-lab-technology.md) — related
