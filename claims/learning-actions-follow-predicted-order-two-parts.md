---
type: claim
title: Expansive learning actions in the meeting followed by and large the theory-predicted order, forming a mini-cycle of two main parts with iterations
description: Expansive learning actions in the meeting followed by and large the theory-predicted order, forming a mini-cycle of two main parts with iterations
id: learning-actions-follow-predicted-order-two-parts
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
evidence_strength: moderate
sources:
  - id: rantavuori-2016
    resource: "http://dx.doi.org/10.14786/flr.v4i3.174"
    title: "Rantavuori, J., Engeström, Y., & Lipponen, L. (2016). Learning actions, objects and types of interaction: A methodological analysis of expansive learning among pre-service teachers. Frontline Learning Research. http://dx.doi.org/10.14786/flr.v4i3.174"
    author: "Rantavuori, J., Engeström, Y., & Lipponen, L."
    q: 2
    i: 1
  - id: rantavuori-2016-2
    resource: "http://dx.doi.org/10.14786/flr.v4i3.174"
    title: "Rantavuori, J., Engeström, Y., & Lipponen, L. (2016). Learning actions, objects and types of interaction: A methodological analysis of expansive learning among pre-service teachers. Frontline Learning Research. http://dx.doi.org/10.14786/flr.v4i3.174"
    author: "Rantavuori, J., Engeström, Y., & Lipponen, L."
    q: 2
    i: 1
---

# Expansive learning actions in the meeting followed by and large the theory-predicted order, forming a mini-cycle of two main parts with iterations

> **Claim** · [All claims](index.md)
> **Evidence** · 2 studies · `q2` quasi-experiment · `i1` small

## Subclaims
`q2 i?` The learning actions of the expansive cycle were taken by and large in the order predicted in the theory, though with iterations such as analyzing-modeling-analyzing-modeling. [→ Rantavuori 2016](#rantavuori-2016)
`q2 i?` The mini-cycle was composed of two main parts: working on the problem (turns 66-324) and working on a new model (turns 325-470 and 714-787), with modeling as the turning point between them. [→ Rantavuori 2016 (2)](#rantavuori-2016-2)

## Evidence

### Rantavuori 2016

Rantavuori, J., Engeström, Y., & Lipponen, L. (2016). Learning actions, objects and types of interaction: A methodological analysis of expansive learning among pre-service teachers. Frontline Learning Research. http://dx.doi.org/10.14786/flr.v4i3.174

`q2 · i1`

Sequential analysis of the meeting's learning actions (Table 5). The article states the actions were "taken by and large in the order predicted in the theory" while noting iterations such as analyzing-modeling-analyzing-modeling in turns 282-324.

> "Table 5 shows that the learning actions of the expansive cycle were taken by and large in the order predicted in the theory. To be sure, there were iterations, such as the sequence analyzing –> modeling –> analyzing –> modeling in turns 282–324"

### Rantavuori 2016 (2)

Rantavuori, J., Engeström, Y., & Lipponen, L. (2016). Learning actions, objects and types of interaction: A methodological analysis of expansive learning among pre-service teachers. Frontline Learning Research. http://dx.doi.org/10.14786/flr.v4i3.174

`q2 · i1`

The article's structural analysis of the meeting identifies two main parts: "working on the problem (turns 66 to 324)" and "working on a new model (turns 325 to 470 and turns 714 to 787)", with modeling a new solution as the bridging phase.

> "It seems that the expansive mini-cycle was in this case composed of two main parts. We might call these (1) working on the problem (turns 66 to 324) and (2) working on a new model (turns 325 to 470 and turns 714 to 787)"

## Discussion


## Related Claims
- [A full assessment of a potentially expansive mini-cycle of learning calls for extending the time scale of the analysis beyond a single session](assessing-minicycle-requires-extended-time-scale.md) — related
- [Transitions and disturbances were crucial for the dynamics of expansive learning in the meeting](disturbances-crucial-expansive-learning-dynamics.md) — related
- [A single meeting of self-regulating pre-service teachers contained an almost complete expansive mini-cycle of learning actions, with all expansive actions except consolidating the new practice](single-meeting-almost-complete-expansive-mini-cycle.md) — related
- [Object formation in the meeting was iterative and non-linear: the proposed germ cell of 'making a choice' was encapsulated, not elaborated and expanded](object-formation-iterative-germ-cell-encapsulated.md) — related
- [Expansive learning moved epistemically from questioning to analysis, modeling and implementation while interactionally moving from coordination to cooperation and communication, but with no mechanical correspondence between specific actions and interaction types](no-mechanical-correspondence-actions-interaction.md) — related
