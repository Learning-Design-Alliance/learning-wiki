...
**Mechanism.** Drawing is typically explained as a generative learning strategy: constructing a pictorial representation forces learners to select key ideas, organize them into a coherent structure, and integrate verbal and pictorial representations — consistent with [Dual Coding Theory](../theories/dual-coding-theory.md) and generative activities frameworks. Drawing also imposes [cognitive load](../theories/cognitive-load-theory.md), so its benefit depends on the drawing not overwhelming working memory — the same trade-off that governs [chunking](../claims/chunking-reduces-working-memory-load.md) and other load-management strategies.
