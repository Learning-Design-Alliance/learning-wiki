---
type: claim
title: "Students notice the absence of practice and examples: attitude items about those events are rated lower when the events are missing"
description: "Students notice the absence of practice and examples: attitude items about those events are rated lower when the events are missing"
id: students-notice-missing-practice-and-examples
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
evidence_strength: moderate
sources:
  - id: martin-2003
    resource: "https://eric.ed.gov/?q=Effects+of+Instructional+Events+in+Computer-Based+Instruction"
    title: "Martin, F., Klein, J., & Sullivan, H. (2003). Effects of Instructional Events in Computer-Based Instruction. https://eric.ed.gov/?q=Effects+of+Instructional+Events+in+Computer-Based+Instruction"
    author: "Martin, F., Klein, J., & Sullivan, H."
    q: 3
    i: "?"
---

# Students notice the absence of practice and examples: attitude items about those events are rated lower when the events are missing

> **Claim** · [All claims](index.md)
> **Evidence** · 1 study · `q3` peer-reviewed experiment

## Subclaims
`q3 i?` Groups without practice rated the practice items significantly lower than groups with practice. [→ Martin 2003](#martin-2003)
`q3 i?` Groups without examples gave the lowest ratings on the examples and graphics items. [→ Martin 2003](#martin-2003)

## Evidence

### Martin 2003

Martin, F., Klein, J., & Sullivan, H. (2003). Effects of Instructional Events in Computer-Based Instruction. https://eric.ed.gov/?q=Effects+of+Instructional+Events+in+Computer-Based+Instruction

`q3 · i?`

Scheffe follow-ups on attitude Items 7 and 8 (practice helpfulness and opportunity) showed the two no-practice treatments significantly lower than the four practice-including treatments; on Items 3 and 4 the two no-examples treatments had the lowest ratings.

> "participants in each of the two treatments that did not include practice had significantly lower attitudes than those in each of the four treatments that included practice."

## Discussion


## Related Claims
- [Students may be unaware of the absence of objectives: the no-objectives group gave the highest ratings on objective-related attitude items](no-objectives-group-most-positive-attitudes.md) — related
- [Including practice with feedback in computer-based instruction significantly raises posttest achievement](practice-presence-raises-cbi-posttest-achievement.md) — related
- [Treatment version significantly affects student attitudes, with the lean program most negative and no-practice next](cbi-version-affects-attitudes-lean-most-negative.md) — a broader claim this one bears on
- [Individually removing objectives, examples, or review from a well-designed computer-based lesson did not significantly reduce achievement](single-event-removal-no-achievement-effect.md) — related
