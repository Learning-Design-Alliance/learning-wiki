...
## Subclaims
`q2 i2` Across an 11-lesson genetics unit, 67 of 77 identified student-invoked resources (87%) occurred in "true discussion" sequences involving three or more participants, indicating a co-variation (not claimed as causal by the authors) between student-resource invocation and multi-party participation. [→ Furberg & Silseth 2022](#furberg-silseth-2022)

`q2 i2` In a sequence where the teacher positioned students as accountable "sources" — using open-ended questions, withholding the correct answer, and prompting peers to respond to each other — students provided arguments and counter-arguments to a much greater extent than in two other sequences where the teacher retained authority and quickly supplied canonical explanations. [→ Furberg & Silseth 2022](#furberg-silseth-2022)

## Evidence

### Furberg & Silseth 2022
...
## Discussion
The authors are explicit that the co-variation between student-resource invocation and multi-party discussion is not established as causal ("we cannot ascertain a causal relationship"). The three-sequence contrast is illustrative rather than a systematic experiment — it demonstrates the pattern exists and is analytically well-supported by transcript evidence, but with only one instance of the dialogic pattern documented in this level of detail, its reliability across topics, teachers, and classes is untested.
