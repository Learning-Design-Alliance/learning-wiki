---
type: principle
id: include-aligned-practice-in-cbi
title: Include practice aligned with the assessment in computer-based instruction
description: "The article's central design recommendation is that practice should be built into computer-based instruction because it was the one consistently effective event for achievement and attitudes."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: martin-2003
    resource: "https://eric.ed.gov/?q=Effects+of+Instructional+Events+in+Computer-Based+Instruction"
    title: "Martin, F., Klein, J., & Sullivan, H. (2003). Effects of Instructional Events in Computer-Based Instruction. https://eric.ed.gov/?q=Effects+of+Instructional+Events+in+Computer-Based+Instruction"
    author: "Martin, F., Klein, J., & Sullivan, H"
---

# Include practice aligned with the assessment in computer-based instruction

> **Principle** · [All principles](index.md)

## Description
The article's central design recommendation is that practice should be built into computer-based instruction because it was the one consistently effective event for achievement and attitudes. The authors write that practice "should be included in computer -based instruction especially when students are tested using items aligned with the objectives and practice items." Practice lets learners confirm correct understanding and elicits overt responses the other events do not.

## Design Implications

### Context
#### Requirements
- Practice items must be aligned with the posttest and with the skills and knowledge reflected in the objectives.
#### Constraints
- The null effects of deleting objectives, examples, or review should not be interpreted as license to delete them routinely; effects may vary with learner age, motivation, and subject-matter complexity.

### Target Learners
- undergraduate students in computer-based instruction

### Target Learning Objectives
- factual and conceptual content such as computer input, processing, storage, and output operations

### Claims
- [Practice Presence Raises Cbi Posttest Achievement](../claims/practice-presence-raises-cbi-posttest-achievement.md) [+M]
- [Single Event Removal No Achievement Effect](../claims/single-event-removal-no-achievement-effect.md) [~M]

## Related Principles
- 

## Examples
-

## Key Sources
- Martin, F., Klein, J., & Sullivan, H. (2003). Effects of Instructional Events in Computer-Based Instruction. https://eric.ed.gov/?q=Effects+of+Instructional+Events+in+Computer-Based+Instruction
