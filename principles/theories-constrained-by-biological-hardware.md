---
type: principle
id: theories-constrained-by-biological-hardware
title: Cognitive theories should be constrained by the biological hardware, not merely by abstract machine-independent specification
description: The paper argues that cognitive psychology, unlike artificial intelligence, must account for constraints imposed by the biological hardware that give cognition its human quality.
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-25
sources:
  - id: iran-nejad-1982
    resource: "https://eric.ed.gov/?id=CS006602"
    title: "Iran-Nejad, Asghar & Ortony, Andrew. (1982). Cognition: A Functional View. Center for the Study of Reading, University of Illinois at Urbana-Champaign. https://eric.ed.gov/?id=CS006602"
    author: "Iran-Nejad, Asghar & Ortony, Andrew"
---

# Cognitive theories should be constrained by the biological hardware, not merely by abstract machine-independent specification

> **Principle** · [All principles](index.md)
> **Evidence** · 1 claim (1 for) · 1 study, `q1` · 0 of 1 report an effect size · 1 claim rests on one study

## Description
The paper argues that cognitive psychology, unlike artificial intelligence, must account for constraints imposed by the biological hardware that give cognition its human quality. The authors write that theories in cognitive psychology "need to take-ointoaccountnotonlythe constraintsimposedbypeople'pbehavior, but-also the sort of'constraints w likely -to be imposed by the hiologicalo,hardware". They recommend that the goal of cognitive psychology be specification of the functional properties of the nervous system that make cognition possible.

## Design Implications

### Context
#### Requirements
- Theoretical terms with unclear referential status should eventually be characterized in terms more closely tied to the biological hardware
#### Constraints
- The authors note compatibility with the nervous system is frequently conspicuously absent from psychological theories, citing Schmitt and Gallistel

### Target Learners
- cognitive theorists and researchers designing theories of cognition and learning

### Target Learning Objectives
- building biologically plausible theories of cognition, memory, and comprehension

### Claims
- [Cognition Transient Distributed Functioning](../claims/cognition-transient-distributed-functioning.md) [+M]

## Related Principles
- 

## Examples
-

## Key Sources
- Iran-Nejad, Asghar & Ortony, Andrew. (1982). Cognition: A Functional View. Center for the Study of Reading, University of Illinois at Urbana-Champaign. https://eric.ed.gov/?id=CS006602
