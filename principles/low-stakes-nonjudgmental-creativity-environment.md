---
type: principle
id: low-stakes-nonjudgmental-creativity-environment
title: Run creativity activities in a non-threatening, low-stakes environment free of right-or-wrong evaluation
description: "The article argues that nurturing creativity in STEM requires \"creating a non-threatening, low-stakes learning environment\" in which \"the instructor move away f rom the traditional “right or wrong” evaluation techniqu..."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: kowalski-2013
    resource: "https://arxiv.org/abs/1308.2434"
    title: "Kowalski, F.V., Kowalski, S.E., Kohl, P.B., and Kuo, V.H. (2013). Technology-Enabled Nurturing of Creativity and Innovation: A Specific Illustration from an Undergraduate Engineering Physics Course. https://arxiv.org/abs/1308.2434"
    author: Kowalski, F.V., Kowalski, S.E., Kohl, P.B., and Kuo, V.H
---

# Run creativity activities in a non-threatening, low-stakes environment free of right-or-wrong evaluation

> **Principle** · [All principles](index.md)
> **Evidence** · 1 claim (1 for) · 1 study, `q1` · 1 of 1 report an effect size · 1 claim rests on one study

## Description
The article argues that nurturing creativity in STEM requires "creating a non-threatening, low-stakes learning environment" in which "the instructor move away f rom the traditional “right or wrong” evaluation techniques." Students are made "aware of and become com fortable with the notion that they are free to make errors without any consequences, and that it is active participatio n that increases their learning and adds to their skills." The authors report this helped both students and instructors evaluate their existing heuristics/models and rewarded them with new insights.

## Design Implications

### Context
#### Requirements
- Instructors must refrain from judging ideas during ideation and critical-comment phases, since most STEM students are unfamiliar with this type of pedagogy
#### Constraints
- The authors note it is difficult for instructors to avoid making judgments about ideas, particularly positive and negative critical comments

### Target Learners
- undergraduate engineering and other STEM students

### Target Learning Objectives
- willingness to generate unrestrained ideas
- evaluating existing heuristics and models

### Claims
- [Anonymity Mitigates Evaluation Apprehension](../claims/anonymity-mitigates-evaluation-apprehension.md) [+M]

## Related Principles
- 

## Examples
-

## Key Sources
- Kowalski, F.V., Kowalski, S.E., Kohl, P.B., and Kuo, V.H. (2013). Technology-Enabled Nurturing of Creativity and Innovation: A Specific Illustration from an Undergraduate Engineering Physics Course. https://arxiv.org/abs/1308.2434
