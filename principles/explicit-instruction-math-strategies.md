...
## Implications
Explicit instruction in math strategies works best when learners are shown not only how a strategy works, but when it fits and why it is worth choosing. Comparing methods, explaining decision points, and linking strategies back to problem structure help learners move beyond rote procedure toward flexible use [[claims/self-explanation-improves-conceptual-understanding]] [+S]. The important tradeoff is timing: strategy instruction can reduce unproductive search for learners with some conceptual footing [[claims/worked-examples-example-problem-sequences]] [+S], but if introduced too early or in too much volume, it can overload novices and produce brittle rule-following instead of strategic judgment [[claims/chunking-reduces-working-memory-load]] [+S]. Early strategy instruction is strongest when it emphasizes process and decision criteria, not just correct answers [[claims/process-goals-outperform-outcome-goals-for-novices]] [+M].
