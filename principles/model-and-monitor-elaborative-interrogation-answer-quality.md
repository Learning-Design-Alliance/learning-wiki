---
type: principle
id: model-and-monitor-elaborative-interrogation-answer-quality
title: When using elaborative interrogation, model appropriate answers and monitor answer quality
description: The article proposes that elaborative interrogation benefits learning only when students answer the questions as intended, generating connections among ideas.
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-25
sources:
  - id: clinton-2016
    resource: "https://doi.org/10.1080/00220973.2015.1048847"
    title: "Clinton, V., Alibali, M. W., & Nathan, M. J. (2016). Learning About Posterior Probability: Do Diagrams and Elaborative Interrogation Help? The Journal of Experimental Education, 84(3), 579-599. https://doi.org/10.1080/00220973.2015.1048847"
    author: "Clinton, V., Alibali, M. W., & Nathan, M. J"
---

# When using elaborative interrogation, model appropriate answers and monitor answer quality

> **Principle** · [All principles](index.md)

## Description
The article proposes that elaborative interrogation benefits learning only when students answer the questions as intended, generating connections among ideas. Because answer quality in this study was poor, the authors recommend that "teachers may find it useful to model appropriate answers and monitor answer quality when using elaborative interrogation with their students". This principle is conditional on future work replicating the study's findings.

## Design Implications

### Context
#### Requirements
- Students must provide answers indicative of the connections among ideas the questions are intended to stimulate
#### Constraints
- The authors state it is premature to indicate implications because some findings are inconsistent with previous work; the recommendation holds only if supported by future work with similar findings

### Target Learners
- undergraduate students reading lessons independently

### Target Learning Objectives
- probabilistic reasoning, calculating posterior probability

### Claims
- [Elaborative Interrogation Answer Quality Poor Correlates With Posttest](../claims/elaborative-interrogation-answer-quality-poor-correlates-with-posttest.md) [+M]

## Related Principles
- 

## Examples
-

## Key Sources
- Clinton, V., Alibali, M. W., & Nathan, M. J. (2016). Learning About Posterior Probability: Do Diagrams and Elaborative Interrogation Help? The Journal of Experimental Education, 84(3), 579-599. https://doi.org/10.1080/00220973.2015.1048847
