---
type: principle
id: inquiry-ownership-and-authenticity
title: Design inquiry activities around student ownership and authenticity to real science and engineering practices
description: "The article frames its inquiry design with two stated keys: \"Keys to inquiry are ownership of students over their learning and authenticity of the activity to real-life science and engineering practices\"."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: morzinski-2010
    resource: "https://arxiv.org/abs/1009.3297"
    title: "Morzinski, K. M., Crockett, C. J., and Crossfield, I. J. (2010). Digital image exploration at Maui Community College. ASP Conference Series. https://arxiv.org/abs/1009.3297"
    author: Morzinski, K. M., Crockett, C. J., and Crossfield, I. J
---

# Design inquiry activities around student ownership and authenticity to real science and engineering practices

> **Principle** · [All principles](index.md)
> **Evidence** · 3 claims (3 for) · 3 studies, `q2` · 3 of 3 report an effect size · 3 claims rest on one study

## Description
The article frames its inquiry design with two stated keys: "Keys to inquiry are ownership of students over their learning and authenticity of the activity to real-life science and engineering practices". The digital-images activity operationalizes this through open investigation, question generation, and an engineering design challenge tied to science goals such as mapping sunspots.

## Design Implications

### Context
#### Requirements
- Activities must give students ownership over their learning and be authentic to real-life science and engineering practices, as the article states.
#### Constraints
- 

### Target Learners
- first- and second-year undergraduate engineering technology students

### Target Learning Objectives
- digital image concepts
- engineering design process
- communication skills

### Claims

- [Students rated the image decoding, poster sharing, and synthesis lecture components of a digital-images inquiry highest on a five-point feedback scale](../claims/digital-images-inquiry-student-feedback-ratings.md) [+W]
- [A six-step C/I cycle administered in two engineering physics classes guided a group to select a conveyor-belt power-calibration method via a co-constructed decision metric](../claims/ci-cycle-metric-guided-solution-quarry-problem.md) [+W]
- [Spontaneous responsiveness to real-world events in PBL can deepen student-directed inquiry beyond what designed curriculum achieves](../claims/spontaneous-authenticity-in-pbl-deepens-student-directed-inquiry.md) [+W]

## Related Principles

- [Inquiry-based Learning](inquiry-based-learning.md)

## Examples

- [Make Learning Relevant](../strategies/make_learning_relevant.md)
- [Design Challenges](../strategies/design-challenges.md)
- [Use a budget-constrained image encoding challenge to force explicit tradeoffs between spatial and color resolution](../strategies/budget-constrained-encoding-challenge.md)
- [Use jigsaw-style sharing so every student is responsible for all the material](../strategies/jigsaw-sharing-inquiry-posters.md)

## Key Sources
- Morzinski, K. M., Crockett, C. J., and Crossfield, I. J. (2010). Digital image exploration at Maui Community College. ASP Conference Series. https://arxiv.org/abs/1009.3297
