...
* **Finding**: the Lazuli pipeline reached the same publication rule independently at row grain (`experiments/learning-graph/report/gate.py`). Both gates are needed, at two grains — but the vocabularies **disagree** in four places (`pseudonymised`/`pseudonymous`, `synthetic`/`synthetic-simulation`, `consent_basis`/`permitted_uses`, `age_band`/`age_assurance`). Crosswalk tabulated in `research/SCHEMA.md`, recorded rather than fixed by fiat
