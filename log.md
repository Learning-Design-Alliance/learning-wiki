...
* **Ingest (re-review)**: Ellsworth, *A Survey of Educational Change Models* (EdTech Books, 2018; ERIC Digest ED444597, 2000), earlier rejected, included on the maintainer's decision → [theories/ellsworth-framework-of-educational-change](theories/ellsworth-framework-of-educational-change.md) and [claims/isolated-innovations-are-rejected-by-the-system-they-enter](claims/isolated-innovations-are-rejected-by-the-system-they-enter.md); linked from [theories/fullers-concerns-theory](theories/fullers-concerns-theory.md)
* **Ingest (re-review)**: four sources earlier rejected, re-reviewed under `INCLUSION.md` and included, all CC BY chapters of *Foundations of Learning and Instructional Design Technology* (EdTech Books, 2018), read in full: Osguthorpe et al. → [theories/moral-dimensions-of-instructional-design](theories/moral-dimensions-of-instructional-design.md) and [claims/instruction-has-unintended-moral-effects-that-performance-data-miss](claims/instruction-has-unintended-moral-effects-that-performance-data-miss.md); Christensen & West → [processes/design-based-research](processes/design-based-research.md) and [claims/design-based-research-roles-compete-and-threaten-credible-claims](claims/design-based-research-roles-compete-and-threaten-credible-claims.md); Dousay → [methods/selecting-an-instructional-design-model](methods/selecting-an-instructional-design-model.md) and [claims/instructional-design-models-are-validated-by-adoption-not-testing](claims/instructional-design-models-are-validated-by-adoption-not-testing.md), plus a source on [processes/systematic-instructional-design](processes/systematic-instructional-design.md); Svihla → constraints and a source on [processes/design-thinking](processes/design-thinking.md) and [claims/rapid-prototyping-can-amplify-novice-designers-premature-commitment-to-solutions](claims/rapid-prototyping-can-amplify-novice-designers-premature-commitment-to-solutions.md). No DOIs asserted: none is registered for the chapters, and Crossref has none for the 2003 original
* **Policy**: `INCLUSION.md` now says a research-methods paper is in only when it helps design; the two borderline methods rejections stand
...
* **Citations**: Crossref apply pass (`resolve_citation_metadata.py --apply`) — Rey (2012) title and Bertsch et al. (2007) issue/pages corrected to the registry. **Roediger & Karpicke (2006) carried the wrong DOI on 99 pages**: `10.1111/medu.12141` is Larsen, Butler & Roediger (2013), *Medical Education*. The tool was about to rewrite the correct *Psychological Science* 17(3) 249 into that paper's coordinates; it now reports this shape as `conflict` instead. The DOI was replaced with `10.1111/j.1467-9280.2006.01693.x`, resolved against Crossref (title, both authors, journal, volume, issue and pages match), which 9 pages already carried
...
* **Schema**: `consent.permitted_uses` is a **mapping of closed use classes**, not a list of sentences — forced by the first real protocol's scoping: ToS-only platform telemetry may validate a pipeline and may not be published as research, and as prose that line cannot be enforced. An absent class means `unspecified`, so growing the vocabulary cannot invalidate a frozen file. `secondary_research_use` is refused by name: it is `secondary-research-by-others` said twice
* **Tooling**: three enforcement checks implemented narrowly — a dataset may not exceed its protocol's identifiability ceiling for its access class; `ai_processing: true` requires `consent.ai_processing_disclosed: true` (`unspecified` fails too); and a release requires `permitted_uses['research-publication'] == permitted`, since a release IS a publication. All three verified by mutating the fixture until they fired
...
* **Protocol**: the **first real protocol** — `research/protocols/lazuli-platform-telemetry/1.0.0.yaml`. Records Lazuli platform telemetry governance as it stands, and its central statement is a refusal: `permitted_uses: {research-publication: prohibited}`, because learners accepted terms of service and nobody asked them to consent to research use. Naming it from a release fails validation, verified. Every field established by reading `learning-engine-ai-frontend`; every field nobody has checked says `unspecified` or `NOT ESTABLISHED` — **do not fill those in by assumption**
* **Schema**: writing it forced two changes, both the `permitted_uses` shape again. `data.storage`'s three controls are **tri-state** (a bare boolean offers only "claim true" or "claim false" about an unexamined system), and `participants.age_assurance.{method, jurisdiction}` is required — "18+" does not say whether anyone checked, and a band without a jurisdiction decides nothing
* **Finding**: the Lazuli pipeline reached the same publication rule independently at row grain (`experiments/learning-graph/report/gate.py`). Both gates are needed, at two grains
* **Decision**: the two vocabulary mismatches are **settled, and only one was a collision**. `pseudonymised` was — one concept, two spellings, a silently-breaking join — and **this layer moved to the pipeline's spelling**: ~21 occurrences in unmerged files here against 16 across six files there including a published JSON schema, eight test assertions and compiled Parquet, and GDPR's own term of art is "pseudonymisation" (Art 4(5)). Conceding on your own side is not fixing the other repo by fiat. **Do not rename it back**
* **Correction**: the two `source_type` fields were **never** a collision, and CLAUDE.md said otherwise for an hour. The pipeline's says what produced a ROW; this layer's says what KIND of record it is. Only `research` is shared, and `human` maps to either `runtime-learner-data` or `platform-experiment` depending on whether the release assigned conditions — which a row cannot know. Renaming either side would destroy information
* **Tooling**: the crosswalk is **code, not a table** — `check_research.py --crosswalk` prints it and exits 1 when the other side grows an unmapped value. Where the scales differ the mapping is conservative (toward the more identifiable reading, since it feeds a publication ceiling), and `None` means "the row does not determine this" rather than a guess. A negative test appending a value to the pipeline's list found a real bug first: the resolver raised `KeyError` on exactly the drift case it exists to report
...
* **Rule**: the schema now states its governing rule once, in `observation_lib.py` and `SCHEMA.md` — *put information at the lowest level at which it actually varies, and preserve observations at the grain at which the source actually reports them*. It explains every placement, and it is what settled the two corrections below
* **Correction**: `arms[].role` is **removed**, not renamed. Nothing read it, and it is not a property of an arm — `unenhanced-elaboration` was declared `intervention` while being the *reference* side of `enhanced-vs-unenhanced`, so the role varies by comparison, which is where `index_arm`/`reference_arm` already say it. An arm needs no epistemic type to mark a synthesis either: the source says `meta-analysis` and the arm's own `size: {value: 17, unit: studies}` says it is studies. Replaced by the invariant it was standing in for — **every arm must be named by at least one comparison**, which caught a real orphan (`zingaretti`'s single arm was in no comparison at all)
* **Correction**: a comparison's `index_arm`/`reference_arm` may name **a list**. That is the whole representation of a heterogeneous comparator: Martinengo's behaviour-change estimate pools three studies against two different controls, stays ONE observation because that is the grain the paper reports, and the compiler emits `reference_is_heterogeneous: true` beside the resolved arms. Per-study contribution counts are not recorded — the source gives them, but no use case needs that grain yet
* **Correction**: `outcome.role` is now **required**, with `other` + `role_basis: ambiguous` as the way to decline. A regex intended for arm roles deleted `outcome.role` from all 21 observations and `check_observations` passed, because an optional field cannot tell "nobody classified this" from "an edit ate it" — the same shape as every other silent-corruption bug this log records
...
* **Decision**: the record is NOT in a claim's `sources[]` frontmatter. That block is *derived* — `sync_evidence_codes.py --apply` rebuilds it from the body through `dump_frontmatter`, which emits only `id/resource/title/author/q/i/n`, so anything else nested there is destroyed by a script already in the unattended batch chain; `check_source_entry_keys` also holds a closed key set there on purpose. And a study is cited by many claims, so a per-claim copy is the drift shape this repo has lost weeks to. **No existing claim page was changed**
* **Content**: four studies enriched as fixtures across four claims — `zingaretti-2026` (5 observations, including two nulls and two timepoints of one outcome), `frolli-2023` (2), `mantovani-2026` (2, one with the magnitude explicitly `unreported`), `dorazzi-hajek-2022` (2 qualitative, with learner/teacher `valued_by`). 11 observations in total
* **Correction**: [claims/game-based-practice-outperforms-traditional-l2-vocabulary-instruction](claims/game-based-practice-outperforms-traditional-l2-vocabulary-instruction.md) renders Frolli et al. as finding Italian articles "disproportionately hard for this group". The paper reports articles as an *instructional accommodation* — the one topic needing occasional Russian, because articles are absent from Russian and Ukrainian grammar — and measured no article-specific outcome. Recorded as an implementation note and a `candidate` moderator, never a reported one
* **Tooling**: `scripts/observation_lib.py` (schema + pure validator), `scripts/check_observations.py` (`--summary`, `--stub <claim>`), `scripts/compile_observations.py` (`research_observation` NDJSON, plus `--explain` which answers the acceptance question from the record alone). `lint.py` gained `check_observations`, which starts and stays green and ratchets the claim/anchor join
* **Tooling**: `prompt_versions/v127.txt` adds an `observation` block per evidence entry to the extraction schema. `CURRENT` still points at `v99` — switching it is a deliberate act after the fixtures prove useful, not a side effect of this change
* **CI**: `validate.yml` installs python dependencies *before* lint, and `requirements-docs.txt` pins PyYAML explicitly. A lint step that could not parse the observation files would report OK because it never looked — the same failure this log records for `learner-variables`
...
* **Ingest**: Plass, Homer, Kinzer, Frye & Perlin (2011), *Learning Mechanics and Assessment Mechanics for Games for Learning* (G4LI White Paper #01/2011) — four new pages: [elements/learning-mechanic](elements/learning-mechanic.md), [elements/assessment-mechanic](elements/assessment-mechanic.md), [principles/learning-embedded-in-the-core-mechanic](principles/learning-embedded-in-the-core-mechanic.md), [methods/evidence-centered-design](methods/evidence-centered-design.md). All six asserted DOIs resolved against Crossref and passed `classify_doi` before commit; the manifest entry carries the gate result and six flagged findings
* **Correction**: two errors in the white paper's own reference list were corrected rather than reproduced — it prints Kirschner, Sweller & Clark (2006) at *Educational Psychologist* 46(2) where the registry says 41(2), and gives the Domagk, Schwartz & Plass (2010) title as "Defining interactivity in multimedia learning" where the registry has "Interactivity in multimedia learning: An integrated model" at the same journal, volume and pages
* **Edit**: [principles/game-based-learning](principles/game-based-learning.md), [patterns/epistemic-games](patterns/epistemic-games.md) and [elements/formative-assessment](elements/formative-assessment.md) gained cross-links into the four new pages
* **Correction**: `scripts/sync_evidence_codes.py` — `SOURCES_BLOCK_RE` required a trailing newline on every line of the `sources:` block, but `okf_lib.FRONTMATTER_RE` captures the frontmatter as `(.*?)\n---`, so the block's last line arrives without one. The match stopped a line short and `sub` left that line standing after the replacement: the rendered block plus an orphaned copy of its own last key. 86 claim pages carried a duplicate `author`, `n` or `q` — valid YAML, last-one-wins, invisible to `lint.py`, visible only in a diff. All 86 repaired
* **Correction**: `scripts/okf_lib.py` — `_derive_id_and_author` stripped the period before `(year)` unconditionally, which in APA is the final initial's own period: every author list ending in an initial was recorded one character short (`& Freeman, S` for `& Freeman, S.`). Fixed and restored on 85 entries. The two bugs masked each other — the orphaned duplicate happened to preserve the correct spelling, so the corpus read correctly while both were live
* **Tooling**: `scripts/log_source_review.py` gained `--citations`, which `okf_lib.append_manifest_entry` has accepted since Gate 3 landed but the CLI never exposed — so every manually ingested source wrote a bare `ingested`, which CLAUDE.md is explicit must not be read as "the citations were checked"
