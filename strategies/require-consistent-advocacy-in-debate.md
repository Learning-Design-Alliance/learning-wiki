---
type: strategy
id: require-consistent-advocacy-in-debate
title: "Require consistent advocacy in debate activities: bar or discourage conditional arguments"
description: In debate activities, require students to commit to and defend a consistent set of claims through the round rather than allowing conditional or multiple hypothetical positions that can be dismissed at the last moment.
status: draft
generated:
  by: claude/unspecified
  at: 2026-09-25
sources:
  - id: herbeck-1990
    resource: "https://eric.ed.gov/?id=ED322540"
    title: "Herbeck. (1990). The Case against Conditional Argumentation. https://eric.ed.gov/?id=ED322540"
    author: Herbeck
---

# Require consistent advocacy in debate activities: bar or discourage conditional arguments

> **Strategy** · [All strategies](index.md)
> **Evidence** · no claims cited

## Description
In debate activities, require students to commit to and defend a consistent set of claims through the round rather than allowing conditional or multiple hypothetical positions that can be dismissed at the last moment. Herbeck argues conditional claims should be discouraged because they "fail to satisfy minimum preconditions for meaningful argumentation," encourage shoddy advocacy, and cannot be evaluated meaningfully. He contends that barring them does not unfairly restrict negative options, using a legislator analogy in which an individual advocate can defend only one position at a time.

## Design Implications

### Context
#### Requirements
- Debate formats must require advocates to commit to one consistent position through the round, with conditional claims barred or actively discouraged.
#### Constraints
- The recommendation is argued on theoretical grounds; the article reports no empirical test of the effects of barring conditional argumentation.

### Target Learners
- policy debate students in academic debate programs

### Target Learning Goals
- intellectual accountability in advocacy
- rigorous comparison of policy options

## Related Strategies

- [Eight reforms for improving parliamentary debate as educational debate](parliamentary-debate-reform-recommendations.md)

## Examples
-

## Key Sources
- Herbeck. (1990). The Case against Conditional Argumentation. https://eric.ed.gov/?id=ED322540
