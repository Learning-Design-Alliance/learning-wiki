---
type: strategy
id: gradual-therapeutic-admission-strategy
title: Conduct admissions as a gradual therapeutic process examining needs, forming relationships, involving supports, and celebrating arrival
description: This strategy replaces the brief procedural intake with a phased, therapeutic admission.
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: williams-1995
    resource: "https://eric.ed.gov/?id=ED398498"
    title: "Williams, Paul A. (1995). Developing a Model to Ease Youths' Transitions into Residential Treatment: Integrating Constructivist Therapies and Youth Care Work into a Contextually Relevant Rite of Passage. Nova Southeastern University. https://eric.ed.gov/?id=ED398498"
    author: Williams, Paul A
---

# Conduct admissions as a gradual therapeutic process examining needs, forming relationships, involving supports, and celebrating arrival

> **Strategy** · [All strategies](index.md)
> **Evidence** · no claims cited

## Description
This strategy replaces the brief procedural intake with a phased, therapeutic admission. The article specifies what workers should attempt: "to examine youths' needs; to formrelationships with them; to involve their significant social supports; and to celebrate youths' arrival." It is implemented in the CMR model through distinct transitional gradual admission stages (preparation, initial meeting, follow-up meeting, pre-admission planning, separation, experimentation) and a separate transitional immediate admission sequence for urgent placements.

## Design Implications

### Context
#### Requirements
- Workers trained and time to conduct multi-stage admissions rather than a single intake meeting
#### Constraints
- Emergency admissions require a compressed immediate-admission variant; the article treats gradual and immediate admissions as distinct procedures

### Target Learners
- youth care workers conducting admissions at residential treatment programs

### Target Learning Goals
- easing youths comfortably into residential treatment
- building early worker-youth relationships

### Affordances
- [Cmr Context Metaphor Relationship Model](../theories/cmr-context-metaphor-relationship-model.md)

## Related Strategies
- 

## Examples
-

## Key Sources
- Williams, Paul A. (1995). Developing a Model to Ease Youths' Transitions into Residential Treatment: Integrating Constructivist Therapies and Youth Care Work into a Contextually Relevant Rite of Passage. Nova Southeastern University. https://eric.ed.gov/?id=ED398498
