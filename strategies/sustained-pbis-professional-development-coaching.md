---
type: strategy
id: sustained-pbis-professional-development-coaching
title: Sustained, needs-based PBIS professional development with in-classroom coaching to build teacher capacity
description: The article describes a one-year professional development model in which eight self-contained ED teachers received 20 two-hour training sessions plus four classroom observations, with content guided by a needs assessm...
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: benner-2008
    resource: "https://eric.ed.gov/?q=Impact+of+Intensive+Positive+Behavioral+Supports"
    title: "Benner, G. J., Beaudoin, K. M., Chen, P.-Y., Davis, C., & Ralston, N. (2008). The Impact of Intensive Positive Behavioral Supports on the Behavioral Functioning of Students with Emotional Disturbance: How Much Does Fidelity Matter? JBAIC, Volume 1, No. 1. https://eric.ed.gov/?q=Impact+of+Intensive+Positive+Behavioral+Supports"
    author: "Benner, G. J., Beaudoin, K. M., Chen, P.-Y., Davis, C., & Ralston, N"
---

# Sustained, needs-based PBIS professional development with in-classroom coaching to build teacher capacity

> **Strategy** · [All strategies](index.md)
> **Evidence** · no claims cited

## Description
The article describes a one-year professional development model in which eight self-contained ED teachers received 20 two-hour training sessions plus four classroom observations, with content guided by a needs assessment. Each session combined review, new material, and guided practice. The authors conclude that building capacity "should involve not only workshops, but ongoing coaching and attention to fidelity of implementation."

## Design Implications

### Context
#### Requirements
- Support must continue over an extended period; the authors note teacher support should continue for 12 to 15 months to build capacity in a given area.
#### Constraints
- The study did not include schoolwide measures of PBIS implementation and did not control for child factors such as SES.

### Target Learners
- Special education teachers of students with emotional disturbance in self-contained settings

### Target Learning Goals
- Implementing PBIS with fidelity: functional behavioral assessment, behavior intervention planning, data-based decision making, and positive classroom environment skills

## Related Strategies
- 

## Examples
-

## Key Sources
- Benner, G. J., Beaudoin, K. M., Chen, P.-Y., Davis, C., & Ralston, N. (2008). The Impact of Intensive Positive Behavioral Supports on the Behavioral Functioning of Students with Emotional Disturbance: How Much Does Fidelity Matter? JBAIC, Volume 1, No. 1. https://eric.ed.gov/?q=Impact+of+Intensive+Positive+Behavioral+Supports
