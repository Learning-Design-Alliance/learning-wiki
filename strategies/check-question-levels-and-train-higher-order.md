---
type: strategy
id: check-question-levels-and-train-higher-order
title: Teachers should check question levels with software after writing papers and be trained in preparing higher-order questions
description: The article recommends that after making question papers, teachers verify the cognitive-level balance of their questions and that institutions improve paper-making criteria.
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: anees-2017
    resource: "https://eric.ed.gov/?q=Bloom%27s+Taxonomy+assessment+levels"
    title: "Anees, S. (2017). Analysis of Assessment Levels of Students’ Learning according to Cognitive Domain of Bloom’s Taxonomy. https://eric.ed.gov/?q=Bloom%27s+Taxonomy+assessment+levels"
    author: Anees, S
---

# Teachers should check question levels with software after writing papers and be trained in preparing higher-order questions

> **Strategy** · [All strategies](index.md)

## Description
The article recommends that after making question papers, teachers verify the cognitive-level balance of their questions and that institutions improve paper-making criteria. It states "teachers should use different software to find out their levels of questions after making question papers, to make a balanced question paper which evaluate the whole performance of students", and suggests young teachers be trained in preparing high-order questions. The university should provide good questions' criteria and improve student-teacher interaction.

## Design Implications

### Context
#### Requirements
- Institutional support for question-criteria standards and teacher training in higher-order question preparation
#### Constraints
- The article notes the study did not examine teacher-student interaction, so its impact claim rests on the authors' interpretation rather than measurement

### Target Learners
- university teachers preparing examinations

### Target Learning Goals
- balanced assessment covering all cognitive levels of Bloom's Taxonomy

### Affordances
- [Bloom Cognitive Domain Six Levels](../theories/bloom-cognitive-domain-six-levels.md)

## Related Strategies
- 

## Examples
-

## Key Sources
- Anees, S. (2017). Analysis of Assessment Levels of Students’ Learning according to Cognitive Domain of Bloom’s Taxonomy. https://eric.ed.gov/?q=Bloom%27s+Taxonomy+assessment+levels
