---
type: strategy
id: digital-badges-maker-education-recognition
title: Use digital badges to recognize skills, supplement report cards, and manage equipment training access in maker education
description: The paper describes digital badges as serving to organize, communicate and display accomplishment and progress, with special application to maker-based classes.
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-25
sources:
  - id: lundberg-2018
    resource: "https://eric.ed.gov/?id=EJ1170292"
    title: "Lundberg, M., & Rasmussen, J. (2018). Foundational Principles and Practices to Consider in Assessing Maker Education. i-manager's Journal of Educational Technology, Vol. 14 No. 4, January - March 2018. https://eric.ed.gov/?id=EJ1170292"
    author: "Lundberg, M., & Rasmussen, J"
---

# Use digital badges to recognize skills, supplement report cards, and manage equipment training access in maker education

> **Strategy** · [All strategies](index.md)
> **Evidence** · no claims cited

## Description
The paper describes digital badges as serving to organize, communicate and display accomplishment and progress, with special application to maker-based classes. Badges recognize a skill learned or task accomplished, can supplement report cards, and can act like a credential system: displayed badges let teachers across a building and between grade levels know who is trained in what and who should be given access to what equipment.

## Design Implications

### Context
#### Requirements
- Students complete established requirements and then receive a digital image that can be displayed using some type of online presence
#### Constraints
- Some open badge systems take some technical skill to utilize and can be challenging to manage for classroom use

### Target Learners
- K-12 students in maker education programs using expensive or dangerous equipment

### Target Learning Goals
- Recognition of skills learned, collaboration, and safe independent tool use

## Related Strategies

- [Badges](badges.md)
- [Micro Credentials](micro-credentials.md)

## Examples
-

## Key Sources
- Lundberg, M., & Rasmussen, J. (2018). Foundational Principles and Practices to Consider in Assessing Maker Education. i-manager's Journal of Educational Technology, Vol. 14 No. 4, January - March 2018. https://eric.ed.gov/?id=EJ1170292
