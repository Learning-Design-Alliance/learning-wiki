---
type: strategy
id: question-correct-answers-not-errors
title: Question students about why a correct answer was chosen, rather than why an error was made
description: "In self-instructional kit work, the teacher asks \"Why did you choose c for number two?\" of a CORRECT answer rather than an incorrect one."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: manzo-1977
    resource: "https://eric.ed.gov/?id=ED136190"
    title: "Manzo, Anthony V.; Sherk, John K., Jr. (1977). Reading and \"Languaging in the Content Areas\" A Third Generational Approach. https://eric.ed.gov/?id=ED136190"
    author: Manzo, Anthony V.; Sherk, John K., Jr
---

# Question students about why a correct answer was chosen, rather than why an error was made

> **Strategy** · [All strategies](index.md)

## Description
In self-instructional kit work, the teacher asks "Why did you choose c for number two?" of a CORRECT answer rather than an incorrect one. This lets the teacher "help him to review his first choice and to discover and reinforce the effective... pattern of deliberations which led him to the correct choice". The student comes to view the teacher as a resource person rather than a wandering critic, and conversation with the teacher is not tantamount to public exposure of ignorance.

## Design Implications

### Context
#### Requirements
- Teacher must first identify which student responses were correct before questioning
#### Constraints
- 

### Target Learners
- students using self-instructional reading materials

### Target Learning Goals
- metacognitive awareness of effective reasoning patterns
- willingness to accept criticism of errors

### Affordances
- [Experience With Languaging Activities](../patterns/experience-with-languaging-activities.md)

## Related Strategies
- 

## Examples
-

## Key Sources
- Manzo, Anthony V.; Sherk, John K., Jr. (1977). Reading and "Languaging in the Content Areas" A Third Generational Approach. https://eric.ed.gov/?id=ED136190
