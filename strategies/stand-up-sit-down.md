---
type: strategy
title: Stand Up/Sit Down
description: "The rules are simple: if the teacher gives the number 10, students stand up."
status: review
generated:
  by: "process:wiki-ingest"
  at: 2026-04-06
---

# Stand Up/Sit Down

## Description
The rules are simple: if the teacher gives the number 10, students stand up. Any other number, they sit down. The trick is, the teacher will say things like '7+3' and '14 -5' (pick appropriate sums and differences for your students to solve mentally). This is a great game to try to 'trick' the students by standing up or sitting down on when they should be doing the opposite. There are endless variations, for example: stand when the number is larger than 5; sit if it is 5 or below, stand when the number is even; sit when it is odd, stand if the digit 1 appears on the number; sit otherwise.

## Design Implications

### Context
#### Requirements
- An open space where students can move freely. Teacher needs to be able to call out math problems.
#### Constraints
- Students may get confused by the 'trick' component. Not suitable for students with mobility issues or difficulty following instructions.
#### Implementation Variability
- <!-- TODO -->

### Target Learners
<!-- Link to sub-claims: [Claim Slug](/claims/claim-slug.md) -->
- K-4, adaptable to other grades

### Target Learning Goals
<!-- Link to sub-claims: [Claim Slug](/claims/claim-slug.md) -->
- Reinforce math skills, increase response time, promote active engagement, encourage kinesthetic learning

### Affordances
- Simple rules and variations make it easy to adapt to various learning needs. Incorporates active recall of math facts. Movement refreshes students and provides another take on math concepts.

### Personalization
- Adapt math problems to match the skill levels of the students. Allow students to come up with their own rules.

### Instructions
<!-- Steps with links to elements -->
- [Practice](/elements/practice.md)
- [Provide guidance](/elements/provide-guidance.md)
- [Activation](/elements/activation.md)

## Assessment Evidence
- Observe the speed and accuracy with which students stand up or sit down. Assess students' mental math skills through the responses.

## Impact
- May help students refresh while applying their mental math. Encourages active participation and critical thinking.

## Related Strategies
- <!-- TODO -->

## Related Elements
- <!-- TODO -->

## Tools
- <!-- TODO -->

## Examples
- When the teacher says '6+4' the students stand up, but when the teacher says '9' the students sit down.

## Key Sources
- <!-- TODO -->
