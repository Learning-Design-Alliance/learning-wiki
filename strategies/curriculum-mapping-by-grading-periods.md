---
type: strategy
id: curriculum-mapping-by-grading-periods
title: Map the curriculum by grading periods to eliminate gaps and redundancy and enable interdisciplinary planning
description: The authors use a modified curriculum-mapping process in which subject-area committees identify their purpose in terms of the board design, their standards, and how the area teaches general learning skills, then outli...
status: draft
generated:
  by: claude/unspecified
  at: 2026-09-25
sources:
  - id: wiles-1986
    resource: "https://eric.ed.gov/?id=ED277147"
    title: "Wiles, Jon W.; Bondi, Joseph W. (1986). Making Middle Schools Work. Association for Supervision and Curriculum Development. https://eric.ed.gov/?id=ED277147"
    author: Wiles, Jon W.; Bondi, Joseph W
---

# Map the curriculum by grading periods to eliminate gaps and redundancy and enable interdisciplinary planning

> **Strategy** · [All strategies](index.md)
> **Evidence** · no claims cited

## Description
The authors use a modified curriculum-mapping process in which subject-area committees identify their purpose in terms of the board design, their standards, and how the area teaches general learning skills, then outline content and skills on worksheets arranged by nine-week grading periods. They assert that "If nothing else were done in establishing middle schools, this mapping experience would be worth the effort," because gaps and redundancy are eliminated and teaching and testing are coordinated.

## Design Implications

### Context
#### Requirements
- Subject-area committees working from the board-adopted design
- Worksheets arranging the 6-7-8 time frame by grading periods
#### Constraints
- The authors note participants realize there is more to do in daily instruction but without additional time — the puzzle of the middle school

### Target Learners
- Middle school students in grades 6-8

### Target Learning Goals
- Coordinated, gap-free curriculum coverage across subjects

### Affordances
- [Wiles Bondi Curriculum Management Plan](../theories/wiles-bondi-curriculum-management-plan.md)

## Related Strategies
- 

## Examples
-

## Key Sources
- Wiles, Jon W.; Bondi, Joseph W. (1986). Making Middle Schools Work. Association for Supervision and Curriculum Development. https://eric.ed.gov/?id=ED277147
