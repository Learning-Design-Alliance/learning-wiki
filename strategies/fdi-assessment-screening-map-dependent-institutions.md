---
type: strategy
id: fdi-assessment-screening-map-dependent-institutions
title: Use field-dependence-independence assessment to screen personnel in map-dependent institutions
description: The article recommends that institutions relying on efficient map learning and recall, such as the military and cartographically-oriented agencies, study field-dependence-independence assessment techniques for screeni...
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: shaha-1982
    resource: "https://eric.ed.gov/?id=ED228271"
    title: "Shaha, Steven H. (1982). Cognitive Processes Functional in Spatial Recall. CSE Report No. 193, Center for the Study of Evaluation, UCLA. https://eric.ed.gov/?id=ED228271"
    author: Shaha, Steven H
---

# Use field-dependence-independence assessment to screen personnel in map-dependent institutions

> **Strategy** · [All strategies](index.md)

## Description
The article recommends that institutions relying on efficient map learning and recall, such as the military and cartographically-oriented agencies, study field-dependence-independence assessment techniques for screening or remedial decisions. Assessment could identify personnel with field-dependent deficits that make spatial learning more difficult, who could then be referred for remedial training or channelled into jobs with reduced demands on spatial recall.

## Design Implications

### Context
#### Requirements
- Access to a field-dependence-independence instrument such as the Hidden Patterns Test or embedded figures tasks
#### Constraints
- The article presents this as a potential strategy to study, not one it tested

### Target Learners
- adult personnel in map-dependent institutions

### Target Learning Goals
- spatial learning and recall of map information

### Affordances
- [Part Whole Spatial Recall Model](../theories/part-whole-spatial-recall-model.md)

## Related Strategies
- 

## Examples
-

## Key Sources
- Shaha, Steven H. (1982). Cognitive Processes Functional in Spatial Recall. CSE Report No. 193, Center for the Study of Evaluation, UCLA. https://eric.ed.gov/?id=ED228271
