---
type: strategy
status: draft
last_edited: 2026-04-06
---

# Snowball Discussion

## Description
Learners begin in pairs, responding to a discussion question only with a single partner. After each person has had a chance to share their ideas, the pair joins another pair, creating a group of four. Pairs share their ideas with the pair they just joined. Next, groups of four join together to form groups of eight, and so on, until the whole class is joined up in one large discussion. This structure could simply be used to share ideas on a topic, or learners could be required to reach consensus every time they join up with a new group.

## Design Implications

### Context
#### Requirements
- Requires a physical space where learners can form pairs and small groups, a discussion question, and clear instructions for learners.
#### Constraints
- Can be time-consuming, requires careful facilitation to ensure all learners are engaged and on task, may be difficult for learners who struggle with social interaction or working in large groups.
#### Implementation Variability
- <!-- TODO -->

### Target Learners
<!-- Link to sub-claims -->
- Suitable for all ages and subject areas, particularly effective for brainstorming, problem-solving, and decision-making activities

### Target Learning Goals
<!-- Link to sub-claims -->
- Promote active participation, encourage peer learning and collaboration, enhance communication skills, build consensus

### Affordances
- Encourages active participation, promotes peer learning and collaboration, enhances communication skills, builds consensus, supports gradual increase in group size

### Personalization
- Discussion questions can be tailored to learners' interests and backgrounds. Learners can choose the partners they want to work with.

### Instructions
<!-- Steps with links to elements -->
- [[elements/peer-discussion|Peer Discussion]]
- [[elements/collaborative-learning|Collaborative Learning]]
- [[elements/consensus-discussion|Consensus Discussion]]
- [[elements/knowledge-synthesis|Knowledge Synthesis]]

## Assessment Evidence
- Observe learner participation and engagement in the activity, review the quality of their responses to the discussion question, collect feedback from learners about their experience.

## Impact
- Enhances communication skills, promotes collaboration and peer learning, builds consensus

## Related Strategies
- <!-- TODO -->

## Related Elements
- <!-- TODO -->

## Tools
- <!-- TODO -->

## Examples
- <!-- TODO -->

## Key Sources
- <!-- TODO -->
