---
type: strategy
id: qwwndw-att-question-parking
title: Set aside off-topic student questions as QWWNDW ATTs and tell students in advance that many questions will fall in this category
description: Because starter pictures elicit many questions that do not match the content goals or cannot be answered with the materials, the facilitators sort them into a named holding category.
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: rafelski-2010
    resource: "https://arxiv.org/abs/1009.5404"
    title: "Rafelski, M., Foley, M., Graves, G. J., Kretke, K. A., Mills, E., Nassir, M., & Patel, S. (2010). Teaching Astronomy with an Inquiry Activity on Stellar Populations. ASP Conference Series. https://arxiv.org/abs/1009.5404"
    author: "Rafelski, M., Foley, M., Graves, G. J., Kretke, K. A., Mills, E., Nassir, M., & Patel, S"
---

# Set aside off-topic student questions as QWWNDW ATTs and tell students in advance that many questions will fall in this category

> **Strategy** · [All strategies](index.md)
> **Evidence** · no claims cited

## Description
Because starter pictures elicit many questions that do not match the content goals or cannot be answered with the materials, the facilitators sort them into a named holding category. The article states: "We call these questions QWWNDW ATTs (questions we will not deal with at this time), and make it clear to the students before they start writing questions that a lot of their questions will fall into this category." Easily answered yes/no questions are quickly answered at the start of the next phase without giving away information needed for students' own investigations.

## Design Implications

### Context
#### Requirements
- Questions must be sorted by categories (e.g., colors of stars, colors of clusters, brightness, origins, Gas/Nebula) to facilitate selection
#### Constraints
- Facilitators must avoid giving away information for the students' own investigations when answering quick questions

### Target Learners
- advanced high-school students
- college students

### Target Learning Goals
- question generation
- selecting relevant data from too much data

## Related Strategies
- 

## Examples
-

## Key Sources
- Rafelski, M., Foley, M., Graves, G. J., Kretke, K. A., Mills, E., Nassir, M., & Patel, S. (2010). Teaching Astronomy with an Inquiry Activity on Stellar Populations. ASP Conference Series. https://arxiv.org/abs/1009.5404
