---
type: strategy
status: review
last_edited: 2026-04-06
---

# Encourage Active Learning

## Description
Learning is not a spectator sport. No one learns much just sitting in classes listening to teachers, memorizing prepackaged assignments, and spitting out answers. They must talk about what they are learning, write about it, relate it to past experiences, and apply it to what they feel is important. They must make what they learn a part of themselves. Equally important is the need to make explicit the learning processes that are occurring in the learning environment and why particular strategies are being used.

## Design Implications

### Context
#### Requirements
- Opportunities for discussion and application, tutor modeling of active learning strategies.
#### Constraints
- Requires significant preparation and active facilitation by the tutor, may be challenging for passive learners.
#### Implementation Variability
- <!-- TODO -->

### Target Learners
<!-- Link to sub-claims -->
- Tutees in higher education or adult learning programs

### Target Learning Goals
<!-- Link to sub-claims -->
- Promote active engagement with learning material, encourage reflection and application of knowledge, integrate learning into personal experiences.

### Affordances
- Enhances understanding and retention, promotes critical thinking and problem-solving skills, fosters personal relevance and engagement.

### Personalization
- Give tutees concrete, real-life situations to analyze. Ask tutees to summarize similarities and differences among research findings, artistic works or laboratory results. Model asking questions, listening behaviors, and feedback.

### Instructions
<!-- Steps with links to elements -->
- [[elements/application|Application]]
- [[elements/practice|Practice]]
- [[elements/inquiry-based-exploration|Inquiry-Based Exploration]]

## Assessment Evidence
- Increased participation in discussions, improved quality of written assignments, positive feedback from tutees about engagement.

## Impact
- Research shows that active learning strategies enhance knowledge retention and critical thinking. Chickering and Gamson, 1987, p. 1

## Related Strategies
- <!-- TODO -->

## Related Elements
- <!-- TODO -->

## Tools
- <!-- TODO -->

## Examples
- A tutor gives tutees real-life situations to analyze. The tutor models active listening behaviors.

## Key Sources
- <!-- TODO -->
