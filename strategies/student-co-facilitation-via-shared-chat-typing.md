---
type: strategy
id: student-co-facilitation-via-shared-chat-typing
title: Invite students to co-facilitate tasks by typing content into the shared whiteboard or chat
description: "A concrete move from the article: the teacher designed slides with blanks and asked students \"to voluntarily help me type out the missing words, phrases or information as I explained verbally to the class\"."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: qi-2022
    resource: "https://doi.org/10.29140/jaltcall.v18n1.569"
    title: "Qi, G. Y., & Wang, Y. (2022). Challenges and responses: A Complex Dynamic Systems approach to exploring language teacher agency in a blended classroom. The JALT CALL Journal, 18(1). https://doi.org/10.29140/jaltcall.v18n1.569"
    author: "Qi, G. Y., & Wang, Y"
---

# Invite students to co-facilitate tasks by typing content into the shared whiteboard or chat

> **Strategy** · [All strategies](index.md)
> **Evidence** · no claims cited

## Description
A concrete move from the article: the teacher designed slides with blanks and asked students "to voluntarily help me type out the missing words, phrases or information as I explained verbally to the class". When online students could not hear her, she made a quick decision to have a student type her speech into Chat. These moves positioned students as co-facilitators and kept the online cohort included.

## Design Implications

### Context
#### Requirements
- A shared visible text space (Whiteboard or Text Chat) reachable by both cohorts
- A teacher willing to cede some control of content entry to students
#### Constraints
- Observed in one teacher's blended classroom; the article does not claim generalisability beyond this case

### Target Learners
- third-year undergraduate additional-language learners in a blended classroom

### Target Learning Goals
- interactive language practice
- maintaining online-student inclusion during technical disruptions

## Related Strategies

- [Show Me](show_me.md)
- [Videoconferencing for Collaboration](videoconferencing_for_collaboration.md)
- [Increase Synchronous Teacher Presence](increase_synchronous_teacher_presence.md)

## Examples
-

## Key Sources
- Qi, G. Y., & Wang, Y. (2022). Challenges and responses: A Complex Dynamic Systems approach to exploring language teacher agency in a blended classroom. The JALT CALL Journal, 18(1). https://doi.org/10.29140/jaltcall.v18n1.569
