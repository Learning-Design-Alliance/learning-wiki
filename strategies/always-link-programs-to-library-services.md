---
type: strategy
id: always-link-programs-to-library-services
title: CLASP programming should always create links to books, reading, information use, and library services
description: A key recommendation from the CLASP evaluation is that library outreach programs must explicitly connect their activities back to books, reading, information use, or library services.
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: small-1994
    resource: "https://eric.ed.gov/?id=ED382199"
    title: "Small, Ruth V.; Katzer, Jeffrey; Eisenberg, Michael B.; McClure, Charles R. (1994). Connecting Libraries and Schools Project (CLASP) Evaluation. Report #7. Final Report. Information Management Consultant Services, Inc. https://eric.ed.gov/?id=ED382199"
    author: Small, Ruth V.; Katzer, Jeffrey; Eisenberg, Michael B.; McClure, Charles R
---

# CLASP programming should always create links to books, reading, information use, and library services

> **Strategy** · [All strategies](index.md)

## Description
A key recommendation from the CLASP evaluation is that library outreach programs must explicitly connect their activities back to books, reading, information use, or library services. The report found that while most activities were book-oriented to some degree, presenters did not always make strong connections between activities and reading or library resources. This recommendation addresses the gap between bringing users into the library and ensuring they engage with its core services.

## Design Implications

### Context
#### Requirements
- Program presenters must be briefed on CLASP goals and library connections
- CLASP representation should be present at activities given by outside talent
#### Constraints
- When outside talent presented library programs, there was insufficient connection to CLASP, CLASP staff, and to library or project goals

### Target Learners
- children and families
- underserved populations
- ethnically diverse communities

### Target Learning Goals
- reading motivation
- library use
- information literacy

## Related Strategies
- 

## Examples
-

## Key Sources
- Small, Ruth V.; Katzer, Jeffrey; Eisenberg, Michael B.; McClure, Charles R. (1994). Connecting Libraries and Schools Project (CLASP) Evaluation. Report #7. Final Report. Information Management Consultant Services, Inc. https://eric.ed.gov/?id=ED382199
