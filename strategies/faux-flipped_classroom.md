---
type: strategy
status: review
last_edited: 2026-04-06
---

# Faux-Flipped Classroom

## Description
This is a specific model targeting young learners. The aim is to replace the homework with instructional lecture-videos and other resources. When students come back to the classroom, one-to-one guidance and support are provided by the teacher.

## Design Implications

### Context
#### Requirements
- Requires creation of engaging video lectures and resources, sufficient teacher time for individualized support.
#### Constraints
- May require more teacher resources, dependent on learners' engagement with pre-class material.
#### Implementation Variability
- <!-- TODO -->

### Target Learners
<!-- Link to sub-claims -->
- Specifically designed for young learners who may benefit from reduced homework and increased teacher support.

### Target Learning Goals
<!-- Link to sub-claims -->
- Replace traditional homework, provide individualized support, reinforce understanding through one-to-one interaction.

### Affordances
- Reduces homework burden, provides personalized attention and support.

### Personalization
- One-to-one guidance can be tailored to each learner's needs.

### Instructions
<!-- Steps with links to elements -->
- [[elements/lectures|Lectures]]
- [[elements/provide-guidance|Provide Guidance]]
- [[elements/individual-reflection|Individual Reflection]]
- [[elements/provide-feedback|Provide Feedback]]

## Assessment Evidence
- Learner engagement with videos, participation in one-to-one sessions, academic performance.

## Impact
- Can improve academic outcomes and reduce homework-related stress for young learners.

## Related Strategies
- <!-- TODO -->

## Related Elements
- <!-- TODO -->

## Tools
- <!-- TODO -->

## Examples
- Elementary school classes using videos to explain math concepts followed by individual support.

## Key Sources
- <!-- TODO -->
