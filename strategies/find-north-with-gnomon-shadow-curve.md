---
type: strategy
id: find-north-with-gnomon-shadow-curve
title: Ubicar el norte marcando la sombra de la punta de un gnomon sobre una circunferencia, en lugar de usar una brújula
description: "El recuadro del artículo propone un método de orientación sin brújula: se dibuja una circunferencia con un gnomon vertical en su centro y se marcan los puntos que recorre la sombra de la punta, obteniéndose una curva..."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: gangui-2009
    resource: "https://cms.iafe.uba.ar/gangui"
    title: "Gangui, Alejandro; Iglesias, María C.; Quinteros, Cynthia P. (2009). El movimiento de las sombras: Una propuesta de trabajo para la escuela secundaria. Ciencia Hoy. https://cms.iafe.uba.ar/gangui"
    author: Gangui, Alejandro; Iglesias, María C.; Quinteros, Cynthia P
---

# Ubicar el norte marcando la sombra de la punta de un gnomon sobre una circunferencia, en lugar de usar una brújula

> **Strategy** · [All strategies](index.md)

## Description
El recuadro del artículo propone un método de orientación sin brújula: se dibuja una circunferencia con un gnomon vertical en su centro y se marcan los puntos que recorre la sombra de la punta, obteniéndose una curva que corta la circunferencia en dos puntos equidistantes del mediodía solar; la recta que los une da la dirección este-oeste y su perpendicular, la norte-sur.

## Design Implications

### Context
#### Requirements
- Un gnomon vertical, un papel con una circunferencia dibujada y un lápiz para marcar la sombra a lo largo del día
#### Constraints
- El artículo señala razones didácticas que hacen a la brújula poco recomendable, pues su empleo fortalece ideas previas sobre la supuesta relación entre el campo magnético terrestre y la rotación del planeta

### Target Learners
- estudiantes de escuela secundaria

### Target Learning Goals
- orientación con las sombras
- puntos cardinales

## Related Strategies
- 

## Examples
-

## Key Sources
- Gangui, Alejandro; Iglesias, María C.; Quinteros, Cynthia P. (2009). El movimiento de las sombras: Una propuesta de trabajo para la escuela secundaria. Ciencia Hoy. https://cms.iafe.uba.ar/gangui
