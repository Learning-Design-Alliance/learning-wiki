---
type: strategy
id: technology-in-community-based-lifelong-learning-research
title: "Investigate technology's role in community-based lifelong learning delivery as a research priority"
description: The paper identifies the use of technology as a major focus for advancing community-based lifelong learning, arguing that applications of new technologies to improve program effectiveness and learning are imperative f...
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-25
sources:
  - id: galbraith-1995
    resource: "https://eric.ed.gov/?id=ED385253"
    title: "Galbraith, M. W. (1995). Community-Based Organizations and the Delivery of Lifelong Learning Opportunities. https://eric.ed.gov/?id=ED385253"
    author: Galbraith, M. W
---

# Investigate technology's role in community-based lifelong learning delivery as a research priority

> **Strategy** · [All strategies](index.md)
> **Evidence** · no claims cited

## Description
The paper identifies the use of technology as a major focus for advancing community-based lifelong learning, arguing that applications of new technologies to improve program effectiveness and learning are imperative for a futuristic perspective. It raises open questions about program delivery, how learning is facilitated through diverse technologies, how adults learn through technology, and how to reduce the phobias attached to technology use.

## Design Implications

### Context
#### Requirements
- Communities must develop new perspectives about where, how, when, and why they engage in learning to provide technological providership
#### Constraints
- The paper frames technology as raising many unanswered questions rather than reporting demonstrated benefits

### Target Learners
- adult learners in urban and rural communities, including technology-phobic users

### Target Learning Goals
- lifelong learning access beyond local boundaries
- effective learning through diverse technologies

## Related Strategies
- 

## Examples
-

## Key Sources
- Galbraith, M. W. (1995). Community-Based Organizations and the Delivery of Lifelong Learning Opportunities. https://eric.ed.gov/?id=ED385253
