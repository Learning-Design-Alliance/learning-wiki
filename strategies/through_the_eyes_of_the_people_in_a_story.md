---
type: strategy
title: Through the Eyes of the People in a Story
description: Learners pick a character from a short story or book and relive the main events from that character's point of view.
status: review
generated:
  by: "claude/unspecified"
  at: 2026-08-29
---

# Through the Eyes of the People in a Story

> **Strategy** · [All strategies](index.md)

## Description
Learners select a character from a short story or novel and retell or re-enact the main events from that character's point of view, articulating the character's thoughts, emotions, and perceptions of other characters and events. The strategy combines close reading, reflective writing, and role-play or presentation, converting narrative comprehension into perspective-taking work. It is carried out through individual reflection on the chosen character, structured discussion with peers who adopted other characters, and optional in-character performance.

## Design Implications

Perspective-taking tasks deepen narrative comprehension by requiring learners to build a causal model of a character's motivations rather than merely tracking plot events, and reading literary fiction has been linked to improved theory-of-mind performance [Kidd & Castano's literary fiction finding.](../claims/reading-literary-fiction-improves-theory-of-mind.md) [+M]. The strategy also functions as an empathy intervention: sustained engagement with characters' inner lives supports empathy and perspective-taking gains, particularly when learners must articulate rather than merely infer those inner states [Fiction reading is associated with empathy and perspective-taking.](../claims/fiction-reading-improves-empathy.md) [+M]. Because learners must justify their interpretation from textual evidence, the activity doubles as a [Self-Explanation](../elements/self-explanation.md) task [Self-explanation improves conceptual understanding.](../claims/self-explanation-improves-conceptual-understanding.md) [+S].

### Context
#### Requirements
- A short story or book rich enough to support multiple defensible character perspectives (multiple viewpoints, internal conflict, or an unreliable or peripheral narrator)
- Individual reflection time with a written or spoken artifact (monologue, diary entry, letter) that makes the character's reasoning explicit
- A discussion structure in which learners who chose *different* characters confront the same events from conflicting viewpoints ([Class Discussion](../elements/class-discussion.md))
- Instructor guidance distinguishing text-grounded inference from projection of the learner's own biases

#### Constraints
- Learners frequently impose their own values and biases on the character, producing projection rather than perspective-taking; without evidence-based prompts the exercise can reinforce rather than challenge existing assumptions [~M]
- Weak readers may lack the comprehension baseline needed to model a character's mental states, causing the task to collapse into plot summary [-M]
- Empathy gains from a single session are fragile; effects on dispositional empathy require repeated engagement across texts [~W]
- If all learners choose the same protagonist, the contrasting-perspectives discussion — the mechanism driving abstraction about the story — is lost [Multiple contrasting cases support abstraction.](../claims/multiple-contrasting-cases-support-abstraction.md) [~M]

#### Implementation Variability
- Written form: in-character diary, letter, or interior monologue (lower-risk, suitable for introverts and large classes)
- Performance form: hot-seating, monologue, or in-character debate ([Act It Out](../strategies/act_it_out.md))
- Comparative form: two learners relive the *same* scene from opposing characters, then reconcile the accounts
- Extension: relive events from a minor or silent character (a servant, a bystander), which maximizes inferential demand

### Target Learners
- Adolescent and adult literature students who can sustain inference about unspoken mental states [~M]
- Learners developing social-emotional competencies; fiction-based perspective work shows the strongest theory-of-mind effects for readers of literary (character-driven) rather than plot-driven texts [Reading literary fiction improves theory of mind.](../claims/reading-literary-fiction-improves-theory-of-mind.md) [+M]
- Emerging readers need scaffolding (character trait charts, evidence prompts) before the perspective task is tractable [-M]

### Target Learning Goals
- Narrative comprehension: causal modeling of character motivation and theme
- Perspective-taking and empathy: adopting and articulating another's viewpoint [Fiction reading is associated with empathy and perspective-taking.](../claims/fiction-reading-improves-empathy.md) [+M]
- Textual evidence use: grounding interpretation in specific passages [Self-explanation improves conceptual understanding.](../claims/self-explanation-improves-conceptual-understanding.md) [+S]
- Oral communication and discussion skills

### Instructions
1. Assign the text and let learners choose (or draw) a character; deliberately distribute choices across minor and major characters ([Assigned Readings](../elements/assigned-readings.md), [Choice Boards](../elements/choice-boards.md)).
2. Learners re-read key scenes marking every passage that reveals what their character knows, wants, or feels ([Annotating](../principles/annotating.md)).
3. Learners produce a first-person artifact — monologue, diary entry, or letter — reliving the main events and explicitly stating the character's reasoning.
4. Convene a mixed-character discussion of a shared pivotal scene; each learner speaks and answers questions in character ([Class Discussion](../elements/class-discussion.md)).
5. Debrief out of character: compare accounts, identify where perspectives conflicted, and evaluate which interpretations the text best supports.

## Related Strategies
- [Act It Out](../strategies/act_it_out.md) — the embodied performance variant of the same perspective-taking mechanism
- [Activating Prior Knowledge](../strategies/activating-prior-knowledge.md) — pre-reading work on characters' social worlds improves the quality of perspective models

## Related Elements
- [Assigned Readings](../elements/assigned-readings.md) — supplies the shared text
- [Class Discussion](../elements/class-discussion.md) — the contrasting-perspectives forum where the strategy's main payoff occurs
- [Choice Boards](../elements/choice-boards.md) — structures character selection with autonomy
- [Self-Explanation](../elements/self-explanation.md) — the evidence-based justification step
- [Act](../elements/act.md) — in-character performance option

## Patterns That Use This Strategy
- [Case-Based Learning](../patterns/case-based-learning.md) — shares the structure of adopting a stakeholder position within a rich scenario
- [Discussion-Based Learning](../patterns/discussion-based-learning.md) — the mixed-character debate enacts this pattern with narrative material

## Examples
- Students read *To Kill a Mockingbird* and relive the trial from the perspective of Scout Finch, Atticus Finch, or Tom Robinson, then debate the testimony in character.
- Students read *The Great Gatsby* and reconstruct the same party scene as Nick Carraway, Jay Gatsby, or Daisy Buchanan, comparing how each account assigns blame and motive.
- Facing History and Ourselves ([https://www.facinghistory.org](https://www.facinghistory.org)) uses identity-centered perspective-taking routines on historical narratives, pairing close reading with in-character reflection and discussion.

## Key Sources
- Mar, R. A., Oatley, K., & Peterson, J. B. (2009). Exploring the link between reading fiction and empathy: Ruling out individual differences and examining outcomes. *COMM, 34*(4), 407-428. [doi:10.1515/comm.2009.025](https://doi.org/10.1515/comm.2009.025)
- Kidd, D. C., & Castano, E. (2013). Reading literary fiction improves theory of mind. *Science, 342*(6156), 377–380. [doi:10.1126/science.1239918](https://doi.org/10.1126/science.1239918)
- Chi, M. T. H., de Leeuw, N., Chiu, M.-H., & LaVancher, C. (1994). Eliciting self-explanations improves understanding. *Cognitive Science, 18*(3), 439–477. [doi:10.1207/s15516709cog1803_3](https://doi.org/10.1207/s15516709cog1803_3)
- Dodell-Feder, D., & Tamir, D. I. (2018). Fiction reading has a small positive impact on social cognition: A meta-analysis. *Journal of Experimental Psychology: General, 147*(11), 1713–1727. [doi:10.1037/xge0000395](https://doi.org/10.1037/xge0000395)
---