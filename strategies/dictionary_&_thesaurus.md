---
type: strategy
status: draft
last_edited: 2026-04-06
---

# Dictionary & Thesaurus

## Description
Incorporating web dictionaries and thesauruses into vocabulary lessons allows learners to quickly search and understand unfamiliar words. When learners look up challenging words while reading, they monitor their comprehension and activate cognitive processes that support increased recall and consolidation to long-term memory. Features such as voice search, audio pronunciation, and embedded quizzes can further support vocabulary building and practice.

## Design Implications

### Context
#### Requirements
- Access to web dictionaries and thesauruses, such as Merriam-Webster Dictionary app, Oxford Dictionaries or Dictionary.com.
#### Constraints
- Requires learners to actively search for and look up words, which may be time-consuming. Reliance on dictionary definitions may not always provide sufficient context for understanding.
#### Implementation Variability
- <!-- TODO -->

### Target Learners
<!-- Link to sub-claims -->
- All learners, particularly those in vocabulary lessons or those who encounter unfamiliar words while reading

### Target Learning Goals
<!-- Link to sub-claims -->
- Improve vocabulary acquisition, enhance reading comprehension, promote active learning, support self-directed learning

### Affordances
- Provides quick access to definitions and synonyms, supports auditory learners with voice search and audio pronunciation, promotes active reading and comprehension monitoring.

### Personalization
- Learners can choose between text-based and audio-based definitions, and can use quizzes for self-assessment and practice.

### Instructions
<!-- Steps with links to elements -->
- [[elements/provide-guidance|Provide guidance]]
- [[elements/practice|Practice]]
- [[elements/provide-feedback|Provide feedback]]

## Assessment Evidence
- Observe learners' use of dictionaries and thesauruses while reading. Review digital notes (vocabulary definitions or explanations). Score each definition according to a Vocabulary Rubric. Identify common misconceptions, misunderstandings, or lack of knowledge.

## Impact
- Enhanced vocabulary acquisition: Quick access to definitions and synonyms helps learners understand and retain new words. Improved reading comprehension: Looking up unfamiliar words promotes active reading and comprehension monitoring.

## Related Strategies
- <!-- TODO -->

## Related Elements
- <!-- TODO -->

## Tools
- <!-- TODO -->

## Examples
- <!-- TODO -->

## Key Sources
- <!-- TODO -->
