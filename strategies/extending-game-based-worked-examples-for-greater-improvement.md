---
type: strategy
id: extending-game-based-worked-examples-for-greater-improvement
title: Extending game-based worked examples with practice problems, fading, self-explanation, verbal instruction and subgoals
description: "The authors' forward-looking recommendation, prompted by a gain they call small."
status: draft
generated:
  by: claude/unspecified
  at: 2026-09-24
sources:
  - id: shen-2006
    resource: "https://eric.ed.gov/?id=ED491961"
    title: "Shen, C.-Y., & O’Neil, H. (2006). The Effectiveness of Worked Examples in a Game-Based Learning Environment. Paper presented at the annual meeting of the American Educational Research Association (AERA), San Francisco, CA. https://eric.ed.gov/?id=ED491961"
    author: "Shen, C.-Y., & O’Neil, H"
---

# Extending game-based worked examples with practice problems, fading, self-explanation, verbal instruction and subgoals

> **Strategy** · [All strategies](index.md)

## Description
The authors' forward-looking recommendation, prompted by a gain they call small. "In order to obtain greater improvement, the worked example instruction could add: (a) practice problems", followed by (b) fading procedure, (c) self-explanations, (d) verbal instruction, and (e) subgoals, each tied to earlier worked example research. These are proposals for future study; the present experiment did not test them.

## Design Implications

### Context
#### Requirements
- An existing worked example intervention for a game-based problem-solving task, to which the five additions are made
#### Constraints
- Untested in this study: the authors propose the additions for future study
- Motivated by the observation that the improvement was small compared to expert performance

### Target Learners
- Adults: undergraduate or graduate students playing a computer puzzle game (SafeCracker)

### Target Learning Goals
- Greater improvement in content understanding and problem-solving strategies in game-based tasks

## Related Strategies
- [Faded Worked Examples](faded-worked-examples.md)
- [Worked Examples For Game Based Puzzle Problem Solving](../patterns/worked-examples-for-game-based-puzzle-problem-solving.md)

## Examples
-

## Key Sources
- Shen, C.-Y., & O’Neil, H. (2006). The Effectiveness of Worked Examples in a Game-Based Learning Environment. Paper presented at the annual meeting of the American Educational Research Association (AERA), San Francisco, CA. https://eric.ed.gov/?id=ED491961
