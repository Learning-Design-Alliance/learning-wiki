---
type: strategy
status: draft
last_edited: 2026-04-06
---

# Virtual Flipped Classroom

## Description
Classroom time for tutoring is completely eliminated in this model. Educators, like university professors, share all the resources and allocate time for individual sessions during office hours. Assignments are collected online through learning management systems.

## Design Implications

### Context
#### Requirements
- Requires comprehensive online resources, availability for individual sessions, learning management system.
#### Constraints
- Dependent on learners' self-motivation, may not suit all learning styles.
#### Implementation Variability
- <!-- TODO -->

### Target Learners
<!-- Link to sub-claims -->
- Suitable for independent learners who can manage their time and seek assistance when needed.

### Target Learning Goals
<!-- Link to sub-claims -->
- Provide flexibility, support self-directed learning, offer personalized assistance during office hours.

### Affordances
- Offers maximum flexibility, allows learners to progress at their own pace.

### Personalization
- Individual sessions can be tailored to address specific learning needs.

### Instructions
<!-- Steps with links to elements -->
- [[elements/lectures|Lectures]]
- [[elements/individual-reflection|Individual Reflection]]
- [[elements/provide-feedback|Provide Feedback]]
- [[elements/peer-discussion|Peer Discussion]]

## Assessment Evidence
- Completion rates of online assignments, utilization of office hours, overall academic performance.

## Impact
- Can provide a flexible, personalized learning experience for self-directed learners.

## Related Strategies
- <!-- TODO -->

## Related Elements
- <!-- TODO -->

## Tools
- <!-- TODO -->

## Examples
- Online university courses using videos, readings, and virtual office hours.

## Key Sources
- <!-- TODO -->
