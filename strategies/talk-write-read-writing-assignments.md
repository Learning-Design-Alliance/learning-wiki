---
type: strategy
id: talk-write-read-writing-assignments
title: Writing assignments that have students talk, write and read through the writing process
description: "The second recommendation is to re-evaluate writing assignments, reconsidering the one-hour in-class essay and \"developing instead assignments that encourage writers to talk, write, and read\" while collecting informat..."
status: draft
generated:
  by: claude/unspecified
  at: 2026-09-24
sources:
  - id: deciccio-1988
    resource: "https://eric.ed.gov/?id=ED294201"
    title: "DeCiccio, Albert C. (1988). Social Constructionism and Collaborative Learning: Recommendations for Teaching Writing. Paper presented at the Annual Meeting of the Conference on College Composition and Communication. https://eric.ed.gov/?id=ED294201"
    author: DeCiccio, Albert C
---

# Writing assignments that have students talk, write and read through the writing process

> **Strategy** · [All strategies](index.md)

## Description
The second recommendation is to re-evaluate writing assignments, reconsidering the one-hour in-class essay and "developing instead assignments that encourage writers to talk, write, and read" while collecting information, focusing, designing, ordering, drafting and clarifying. A paper on a societal problem, researched by talking with authorities, is offered as an example.

## Design Implications

### Context
#### Requirements
- Assignments encourage writers to talk, write and read as they proceed through collecting information, focusing, designing, ordering, drafting and clarifying the text for a reader.
#### Constraints
- 

### Target Learners
- College composition students

### Target Learning Goals
- Generating consequential discourse through a dialectical writing process

### Affordances
- [Social Constructionism In Composition](../theories/social-constructionism-in-composition.md)

## Related Strategies
- [Authentic Writing Tasks](authentic-writing-tasks.md)
- [Process Writing](process-writing.md)

## Examples
-

## Key Sources
- DeCiccio, Albert C. (1988). Social Constructionism and Collaborative Learning: Recommendations for Teaching Writing. Paper presented at the Annual Meeting of the Conference on College Composition and Communication. https://eric.ed.gov/?id=ED294201
