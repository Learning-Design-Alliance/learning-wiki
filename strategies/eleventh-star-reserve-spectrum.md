---
type: strategy
id: eleventh-star-reserve-spectrum
title: Keep an extra unassigned spectrum (a red giant) in reserve to challenge students who finish early or reach an incorrect answer
description: Besides the ten spectra given to students, facilitators hold back an eleventh spectrum of a red giant.
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: rafelski-2010
    resource: "https://arxiv.org/abs/1009.5404"
    title: "Rafelski, M., Foley, M., Graves, G. J., Kretke, K. A., Mills, E., Nassir, M., & Patel, S. (2010). Teaching Astronomy with an Inquiry Activity on Stellar Populations. ASP Conference Series. https://arxiv.org/abs/1009.5404"
    author: "Rafelski, M., Foley, M., Graves, G. J., Kretke, K. A., Mills, E., Nassir, M., & Patel, S"
---

# Keep an extra unassigned spectrum (a red giant) in reserve to challenge students who finish early or reach an incorrect answer

> **Strategy** · [All strategies](index.md)

## Description
Besides the ten spectra given to students, facilitators hold back an eleventh spectrum of a red giant. The article explains this star "can be used in a myriad of situations," most commonly when students finish early, to ask them to verify their explanation also works with this star, or as evidence against an incorrect theory. Facilitators are cautioned against using it with very confused students, as it may confuse them more.

## Design Implications

### Context
#### Requirements
- Facilitators must judge when use is appropriate
#### Constraints
- The article cautions against using the 11th star if the students are very confused, as it may just confuse them more

### Target Learners
- advanced high-school students
- college students

### Target Learning Goals
- evidence-based critical thinking
- testing hypotheses against new data

## Related Strategies
- 

## Examples
-

## Key Sources
- Rafelski, M., Foley, M., Graves, G. J., Kretke, K. A., Mills, E., Nassir, M., & Patel, S. (2010). Teaching Astronomy with an Inquiry Activity on Stellar Populations. ASP Conference Series. https://arxiv.org/abs/1009.5404
