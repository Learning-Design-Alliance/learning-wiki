---
type: strategy
id: email-exchanges-target-language-peers
title: Use structured e-mail exchanges with target-language peers to build cross-cultural interest and motivation
description: The activity has students correspond by e-mail with peers in the target language, chosen via a penpal website where they can select partner country, age, and sex.
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: murray-2001
    resource: "https://teslcanadajournal.ca"
    title: "Murray, G. L., & Bollinger, D. J. (2001). Developing Cross-Cultural Awareness: Learning Through the Experiences of Others. TESL Canada Journal, 19(1). https://teslcanadajournal.ca"
    author: "Murray, G. L., & Bollinger, D. J"
---

# Use structured e-mail exchanges with target-language peers to build cross-cultural interest and motivation

> **Strategy** · [All strategies](index.md)

## Description
The activity has students correspond by e-mail with peers in the target language, chosen via a penpal website where they can select partner country, age, and sex. It proceeds through preliminary tasks (e-mailing classmates and the instructor), partner selection, letter writing, and Internet research on the partner's country shared in presentations. The authors state that "we encourage students to dialogue through e-mail with their target language peers" to foster cultural awareness and motivate communication in English.

## Design Implications

### Context
#### Requirements
- Students should first be accustomed to using computers, English software, and writing English e-mail messages through preliminary tasks.
#### Constraints
- Designed for beginner to intermediate Japanese university learners with little or no direct contact with other cultures.

### Target Learners
- Japanese university students of English, beginner to intermediate level

### Target Learning Goals
- cross-cultural awareness
- motivation to communicate in English

## Related Strategies
- 

## Examples
-

## Key Sources
- Murray, G. L., & Bollinger, D. J. (2001). Developing Cross-Cultural Awareness: Learning Through the Experiences of Others. TESL Canada Journal, 19(1). https://teslcanadajournal.ca
