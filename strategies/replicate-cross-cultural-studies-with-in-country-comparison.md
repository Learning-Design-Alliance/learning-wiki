---
type: strategy
id: replicate-cross-cultural-studies-with-in-country-comparison
title: Replicate cross-cultural psychosocial studies with additional samples and in-country comparison groups
description: "The authors recommend that cross-cultural findings warrant replication with additional sample groups of first-generation Asian immigrants, and that administering \"the same test on Koreans in Korea would be helpful to..."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-25
sources:
  - id: kim-1998
    resource: "https://eric.ed.gov/?id=ED420007"
    title: "Kim, Saekyung; Gaa, John; Swank, Paul; Liberman, Dov. (1998). Identity Transformation of Korean Immigrants. https://eric.ed.gov/?id=ED420007"
    author: Kim, Saekyung; Gaa, John; Swank, Paul; Liberman, Dov
---

# Replicate cross-cultural psychosocial studies with additional samples and in-country comparison groups

> **Strategy** · [All strategies](index.md)

## Description
The authors recommend that cross-cultural findings warrant replication with additional sample groups of first-generation Asian immigrants, and that administering "the same test on Koreans in Korea would be helpful to determine the effect of immigration", which could also produce a Korean norm for the MPD. They further recommend comparing groups transferred between extremely different sociocultural environments and studying second-generation immigrants living in a dual culture.

## Design Implications

### Context
#### Requirements
- Requires administering the same instrument to a comparison group in the country of origin, since pre-immigration testing of the same subjects was impossible
#### Constraints
- The study could not determine whether results reflect immigration or cross-cultural differences between the normative and immigrant groups

### Target Learners
- adult first-generation immigrants
- second-generation immigrants

### Target Learning Goals
- psychosocial identity development across cultures

## Related Strategies
- 

## Examples
-

## Key Sources
- Kim, Saekyung; Gaa, John; Swank, Paul; Liberman, Dov. (1998). Identity Transformation of Korean Immigrants. https://eric.ed.gov/?id=ED420007
