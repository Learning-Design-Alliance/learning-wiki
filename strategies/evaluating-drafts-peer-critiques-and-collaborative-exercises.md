---
type: strategy
id: evaluating-drafts-peer-critiques-and-collaborative-exercises
title: Evaluating drafts, peer critiques and collaborative exercises alongside finished writing
description: The final recommendation is to consider alternate ways of evaluating student writing.
status: draft
generated:
  by: claude/unspecified
  at: 2026-09-24
sources:
  - id: deciccio-1988
    resource: "https://eric.ed.gov/?id=ED294201"
    title: "DeCiccio, Albert C. (1988). Social Constructionism and Collaborative Learning: Recommendations for Teaching Writing. Paper presented at the Annual Meeting of the Conference on College Composition and Communication. https://eric.ed.gov/?id=ED294201"
    author: DeCiccio, Albert C
---

# Evaluating drafts, peer critiques and collaborative exercises alongside finished writing

> **Strategy** · [All strategies](index.md)
> **Evidence** · no claims cited

## Description
The final recommendation is to consider alternate ways of evaluating student writing. Besides grading finished texts, and not on a quantifiable scale, teachers "might evaluate all of the drafts for those texts", the peer critiques students prepare for one another, readings of early and final versions, and the preparatory collaborative exercises.

## Design Implications

### Context
#### Requirements
- Evaluation covers the drafts, the peer critiques students prepare for one another, readings of the early and final versions, and the collaborative exercises that prepare students for the texts, in addition to the finished texts.
#### Constraints
- 

### Target Learners
- College composition students

### Target Learning Goals
- Valuing the collaborative writing process as well as the finished product

### Affordances
- [Social Constructionism In Composition](../theories/social-constructionism-in-composition.md)

## Related Strategies
- [Writing Process Portfolios](writing-process-portfolios.md)
- [Portfolio Assessment](portfolio-assessment.md)
- [Drafting And Feedback Cycles](drafting-and-feedback-cycles.md)

## Examples
-

## Key Sources
- DeCiccio, Albert C. (1988). Social Constructionism and Collaborative Learning: Recommendations for Teaching Writing. Paper presented at the Annual Meeting of the Conference on College Composition and Communication. https://eric.ed.gov/?id=ED294201
