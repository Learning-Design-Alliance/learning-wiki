---
type: strategy
title: Analyze Self-Talk Messages
description: Ask students to identify patterns or themes in their self-talk.
status: review
generated:
  by: "process:wiki-ingest"
  at: 2026-04-06
---

# Analyze Self-Talk Messages

## Description
Ask students to identify patterns or themes in their self-talk. For example, at 10:00 each day, right before we dive into science, Juanita might notice that she begins to feel lousy. She dreads coming back from PE because she knows it will be time to start the science lab that she doesn’t understand. Her voice says things like, “Hopefully my partner, Berta, understands this stuff because I guess I can just copy from her.” Identifying these patterns will be the first step towards combatting negative self-talk.

## Design Implications

### Context
#### Requirements
- Recorded self-talk messages, analytical skills, time for reflection.
#### Constraints
- Requires learners to be honest and open about their thoughts and feelings. May be challenging to identify subtle patterns.
#### Implementation Variability
- <!-- TODO -->

### Target Learners
<!-- Link to sub-claims -->
- Elementary to adult learners

### Target Learning Goals
<!-- Link to sub-claims -->
- Identify patterns and themes in self-talk.

### Affordances
- Helps learners recognize recurring thoughts and feelings related to specific situations or activities.

### Personalization
- Can be adapted by providing structured questions or prompts to guide the analysis.

### Instructions
<!-- Steps with links to elements -->
- [Individual Reflection](/elements/individual-reflection.md)

## Assessment Evidence
- Review analysis of self-talk messages for identified patterns and themes.

## Impact
- Greater understanding of the triggers and contexts for negative self-talk (Practitioner-based evidence).

## Related Strategies
- <!-- TODO -->

## Related Elements
- <!-- TODO -->

## Tools
- <!-- TODO -->

## Examples
- <!-- TODO -->

## Key Sources
- <!-- TODO -->
