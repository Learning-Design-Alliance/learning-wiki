---
type: strategy
status: draft
last_edited: 2026-04-06
---

# Maintaining Sensitivity to Language Concerns

## Description
In traditional classrooms, students who are not native English speakers often feel marginalized, lost, and pressured into discarding their original language in favor of English. In a culturally responsive classroom, diversity of language is celebrated, and the level of instructional materials provided to non-native speakers are tailored to their level of English fluency. Accompanying materials should be provided in the student’s primary language, and the student should be encouraged to master English.

## Design Implications

### Context
#### Requirements
- Access to resources in multiple languages, training in second language acquisition, and a supportive classroom environment.
#### Constraints
- May require additional time and resources to implement. Some students may be reluctant to use their primary language in the classroom.
#### Implementation Variability
- <!-- TODO -->

### Target Learners
<!-- Link to sub-claims: [[claims/claim-slug]] -->
- Teachers with English language learners in their classrooms (K-12, adult education)

### Target Learning Goals
<!-- Link to sub-claims: [[claims/claim-slug]] -->
- Support language acquisition, promote linguistic diversity, and create an inclusive classroom environment for non-native English speakers.

### Affordances
- Reduces feelings of marginalization and isolation. Supports language development in both English and the student's primary language.

### Personalization
- Instructional materials and support can be tailored to the student's level of English fluency and language background.

### Instructions
<!-- Steps with links to elements -->
- [[elements/assigned-readings|Assigned Readings]]
- [[elements/provide-guidance|Provide Guidance]]
- [[elements/direct-instruction|Direct instruction]]

## Assessment Evidence
- Increased student participation and engagement, improved language proficiency, and greater confidence in using both English and their primary language.

## Impact
- Creates a more linguistically diverse and inclusive classroom environment where all students feel valued and supported.

## Related Strategies
- <!-- TODO -->

## Related Elements
- <!-- TODO -->

## Tools
- <!-- TODO -->

## Examples
- <!-- TODO -->

## Key Sources
- <!-- TODO -->
