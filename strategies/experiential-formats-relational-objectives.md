---
type: strategy
id: experiential-formats-relational-objectives
title: Use direct-experience formats — simulation games, extracurricular activities, action learning, peer teaching — to pursue relational-domain objectives
description: The article recommends learning environments that let learners explore social relationships through direct experience, in simulated or real-world formats, on the premise that one leads by leading rather than by readin...
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: brandhorst-1976
    resource: "https://eric.ed.gov/?id=ED134505"
    title: "Brandhorst, Allan R. (1976). Toward a Taxonomy of Educational Objectives in the Relational Domain. https://eric.ed.gov/?id=ED134505"
    author: Brandhorst, Allan R
---

# Use direct-experience formats — simulation games, extracurricular activities, action learning, peer teaching — to pursue relational-domain objectives

> **Strategy** · [All strategies](index.md)
> **Evidence** · no claims cited

## Description
The article recommends learning environments that let learners explore social relationships through direct experience, in simulated or real-world formats, on the premise that one leads by leading rather than by reading about leading. It invokes Anderson and Moore's three qualities of effective formats for interdependence training: "a safe environment; safety to make mistakes; a playful atmosphere; (2) an intrinsic reward system; builtin goals and source of motivation; (3) guidance in mastery of relevant techniques." Simulation games, school extracurricular activities, community action learning, peer teaching, and Junior Achievement are named as strong matches, especially when paired with structured conceptual and evaluational activities.

## Design Implications

### Context
#### Requirements
- Structured conceptual and evaluational activities aligned with the taxonomy's categories must be added so formats also provide guidance in mastery of relevant interpersonal techniques
#### Constraints
- The recommendation rests on a stated assumption about the interactive qualities of social relationships, not on comparative data reported in the paper

### Target Learners
- social studies students
- youth in school and community programs

### Target Learning Goals
- interpersonal relations skills
- leadership and followership capabilities

## Related Strategies
- 

## Examples
-

## Key Sources
- Brandhorst, Allan R. (1976). Toward a Taxonomy of Educational Objectives in the Relational Domain. https://eric.ed.gov/?id=ED134505
