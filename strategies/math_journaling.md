---
type: strategy
title: Math Journaling
description: "In their math journals, students can write about things they are having trouble with or things that they've figured out."
status: review
generated:
  by: "process:wiki-ingest"
  at: 2026-04-06
---

# Math Journaling

## Description
In their math journals, students can write about things they are having trouble with or things that they've figured out. Putting these thoughts into words can help students get a more concrete handle on the logic of their ideas.

## Design Implications

### Context
#### Requirements
- Math journals or notebooks, prompts for reflection (optional).
#### Constraints
- Requires regular time for journaling and may require encouragement for learners to express their thoughts openly.
#### Implementation Variability
- <!-- TODO -->

### Target Learners
<!-- Link to sub-claims -->
- Suitable for all grade levels.

### Target Learning Goals
<!-- Link to sub-claims -->
- Help learners reflect on their understanding, clarify their thinking, and develop a more concrete handle on mathematical concepts.

### Affordances
- Provides a space for learners to explore their struggles and successes, promoting deeper understanding and self-reflection.

### Personalization
- Learners can write about topics that are most relevant to their own learning and understanding.

### Instructions
<!-- Steps with links to elements -->
- [Individual Reflection](../elements/individual-reflection.md)
- [Practice](../elements/practice.md)

## Assessment Evidence
- Review journal entries to assess learners' understanding and identify areas of struggle. Look for evidence of self-reflection and improved clarity of thinking.

## Impact
- Promotes deeper understanding, self-reflection, and the ability to articulate mathematical ideas.

## Related Strategies
- <!-- TODO -->

## Related Elements
- <!-- TODO -->

## Tools
- <!-- TODO -->

## Examples
- Have students write about concepts they are struggling with or explaining solutions they have discovered.

## Key Sources
- <!-- TODO -->
