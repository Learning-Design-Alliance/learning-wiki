---
type: strategy
id: geocentric-heliocentric-model-debate
title: Debate entre grupos asignados a los modelos geocéntrico y heliocéntrico para explorar explicaciones alternativas de los mismos fenómenos
description: Tras leer textos que presentan las diferentes cosmovisiones sostenidas a lo largo de la historia, cada grupo de alumnos recibe uno de los dos modelos teóricos y confecciona una lista con los argumentos, explicaciones...
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: gangui-2009
    resource: "https://cms.iafe.uba.ar/gangui"
    title: "Gangui, Alejandro; Iglesias, María C.; Quinteros, Cynthia P. (2009). El movimiento de las sombras: Una propuesta de trabajo para la escuela secundaria. Ciencia Hoy. https://cms.iafe.uba.ar/gangui"
    author: Gangui, Alejandro; Iglesias, María C.; Quinteros, Cynthia P
---

# Debate entre grupos asignados a los modelos geocéntrico y heliocéntrico para explorar explicaciones alternativas de los mismos fenómenos

> **Strategy** · [All strategies](index.md)

## Description
Tras leer textos que presentan las diferentes cosmovisiones sostenidas a lo largo de la historia, cada grupo de alumnos recibe uno de los dos modelos teóricos y confecciona una lista con los argumentos, explicaciones y evidencias que sostienen su postura, anticipando y refutando los argumentos contrarios. Los alumnos recrean los modelos con la esfera de telgopor, su eje de 16 cm y una lámpara incandescente.

## Design Implications

### Context
#### Requirements
- Textos que solo definan algunos elementos o hipótesis de cada modelo, de manera que los alumnos deban realizar sus aportes al analizarlos
- materiales para recrear los modelos
#### Constraints
- No es necesario aún concluir cuál modelo del cosmos es científicamente más válido; se trata de presentar modelos racionales alternativos

### Target Learners
- estudiantes de escuela secundaria

### Target Learning Goals
- naturaleza de los modelos científicos
- ciclo día-noche y estaciones
- argumentación científica

## Related Strategies
- 

## Examples
-

## Key Sources
- Gangui, Alejandro; Iglesias, María C.; Quinteros, Cynthia P. (2009). El movimiento de las sombras: Una propuesta de trabajo para la escuela secundaria. Ciencia Hoy. https://cms.iafe.uba.ar/gangui
