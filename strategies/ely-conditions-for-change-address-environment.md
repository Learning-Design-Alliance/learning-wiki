---
type: strategy
id: ely-conditions-for-change-address-environment
title: "Address environmental deficiencies using Ely's Conditions for Change, such as leadership commitment and professional development"
description: "The digest recommends that when obstacles arise from the environment in which change is implemented, Ely's Conditions for Change can help address those deficiencies."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-25
sources:
  - id: ellsworth-2000
    resource: "https://eric.ed.gov/?id=ED444597"
    title: "Ellsworth, James B. (2000). A Survey of Educational Change Models. ERIC Digest. https://eric.ed.gov/?id=ED444597"
    author: Ellsworth, James B
---

# Address environmental deficiencies using Ely's Conditions for Change, such as leadership commitment and professional development

> **Strategy** · [All strategies](index.md)
> **Evidence** · no claims cited

## Description
The digest recommends that when obstacles arise from the environment in which change is implemented, Ely's Conditions for Change can help address those deficiencies. Examples it gives include a clearer statement of commitment by top leaders or more evident leadership by example, and more opportunity for professional development to help stakeholders learn to use their new tools.

## Design Implications

### Context
#### Requirements
- Requires assessing which environmental conditions for change are deficient before intervening
#### Constraints
- This strategy addresses obstacles arising from the environment in which change is implemented, not from the innovation's attributes themselves

### Target Learners
- educational leaders and change agents implementing technology innovations

### Target Learning Goals
- creating conditions that facilitate implementation of educational technology innovations

## Related Strategies
- 

## Examples
-

## Key Sources
- Ellsworth, James B. (2000). A Survey of Educational Change Models. ERIC Digest. https://eric.ed.gov/?id=ED444597
