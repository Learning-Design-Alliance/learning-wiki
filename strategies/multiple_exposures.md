---
type: strategy
title: Multiple Exposures
description: Multiple exposures provide students with multiple opportunities to encounter, engage with, and elaborate on new knowledge and skills.
status: review
generated:
  by: "process:wiki-ingest"
  at: 2026-04-06
---

# Multiple Exposures

## Description
Multiple exposures provide students with multiple opportunities to encounter, engage with, and elaborate on new knowledge and skills. Research demonstrates deep learning develops over time via multiple, spaced interactions with new knowledge and concepts. This may require spacing practice over several days, and using different activities to vary the interactions learners have with new knowledge.

## Design Implications

### Context
#### Requirements
- Requires careful planning to ensure spaced repetition and varied interactions.
#### Constraints
- May be time-consuming to implement effectively.
#### Implementation Variability
- <!-- TODO -->

### Target Learners
<!-- Link to sub-claims -->
- All learner levels

### Target Learning Goals
<!-- Link to sub-claims -->
- Reinforce learning through repeated and varied interactions with new content.

### Affordances
- Enhances retention and transfer of knowledge through spaced and varied practice.

### Personalization
- Activities can be varied to suit different learning preferences and needs.

### Instructions
<!-- Steps with links to elements -->
- [Enhance retention and transfer](/elements/enhance-retention-and-transfer.md)

## Assessment Evidence
- Retention over time, ability to apply knowledge in different contexts.

## Impact
- Multiple exposures facilitate deep learning by reinforcing concepts and skills through repeated and varied interactions.

## Related Strategies
- <!-- TODO -->

## Related Elements
- <!-- TODO -->

## Tools
- <!-- TODO -->

## Examples
- <!-- TODO -->

## Key Sources
- <!-- TODO -->
