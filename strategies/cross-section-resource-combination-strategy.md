---
type: strategy
id: cross-section-resource-combination-strategy
title: Combine resources across guide sections for the most fruitful results
description: "The guide's concluding comments urge users to employ both a critical and a creative approach and to combine resources from different sections."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: alspektor-1979
    resource: "https://eric.ed.gov/?id=ED020373"
    title: "Alspektor, Rose Ann, and Wirtenberg, Jeana. (1979). Fair Textbooks: A Resource Guide. U.S. Commission on Civil Rights, Clearinghouse Publication 61. https://eric.ed.gov/?id=ED020373"
    author: Alspektor, Rose Ann, and Wirtenberg, Jeana
---

# Combine resources across guide sections for the most fruitful results

> **Strategy** · [All strategies](index.md)
> **Evidence** · no claims cited

## Description
The guide's concluding comments urge users to employ both a critical and a creative approach and to combine resources from different sections. For example, "The textbook guidekv jrom Section 7) can be apPIied tcrthe-resources (from Sec- tiansand 6) by all persons who wish'to determine the exte'nt of sex, race/ethnict and social fairness in their childrens' books." Other suggested combinations include comparing state statutes across all 50 states for a national perspective, comparing a state's procedures with administrative guidelines, applying research content analyses to curriculum guidelines and publisher catalogs, and contacting publishers and state departments via the directories.

## Design Implications

### Context
#### Requirements
- Users must consult multiple sections and their directories; state-listed items marked with an asterisk require referral to the Section 12 state listing for addresses
#### Constraints
- 

### Target Learners
- Parents, educators, administrators, publishers, and textbook selection committees

### Target Learning Goals
- Assessing and ameliorating sex, race/ethnic, and social fairness bias in textbooks and curricula

## Related Strategies
- 

## Examples
-

## Key Sources
- Alspektor, Rose Ann, and Wirtenberg, Jeana. (1979). Fair Textbooks: A Resource Guide. U.S. Commission on Civil Rights, Clearinghouse Publication 61. https://eric.ed.gov/?id=ED020373
