---
type: strategy
id: arcs-relevance-strategies
title: "ARCS relevance strategies: goal orientation, motive matching and familiarity"
description: "Relevance is the second ARCS component, described as strategies \"that link to learners' needs, interests, and motives\"."
status: draft
generated:
  by: claude/unspecified
  at: 2026-09-24
sources:
  - id: small-1997
    resource: "https://eric.ed.gov/?id=ED409895"
    title: "Small, Ruth V. (1997). Motivation in Instructional Design. ERIC Digest. ERIC Clearinghouse on Information and Technology. https://eric.ed.gov/?id=ED409895"
    author: Small, Ruth V
---

# ARCS relevance strategies: goal orientation, motive matching and familiarity

> **Strategy** · [All strategies](index.md)
> **Evidence** · no claims cited

## Description
Relevance is the second ARCS component, described as strategies "that link to learners' needs, interests, and motives". The digest lists three sub-components: goal orientation (present the objectives and useful purpose of the instruction), motive matching (match objectives to student needs and motives), and familiarity (present content related to the learners' experience and values), each with a teacher example.

## Design Implications

### Context
#### Requirements
- Goal orientation: present the objectives, useful purpose and specific methods for successful achievement
- Motive matching: match objectives to student needs and motives
- Familiarity: present content in understandable ways related to learners' experience and values
#### Constraints
- 

### Target Learners
- Students in classroom instruction and users of instructional materials, as described in the digest

### Target Learning Goals
- Perceiving instruction as linked to one's own needs, interests and motives

### Affordances
- [Arcs Model](../theories/arcs-model.md)
- [Arcs Strategy Subcomponent Taxonomy](../theories/arcs-strategy-subcomponent-taxonomy.md)

## Related Strategies
- [Arcs Model](../theories/arcs-model.md)
- [State Objectives](../elements/state-objectives.md)
- [Learning Objectives](../elements/learning-objectives.md)

## Examples
-

## Key Sources
- Small, Ruth V. (1997). Motivation in Instructional Design. ERIC Digest. ERIC Clearinghouse on Information and Technology. https://eric.ed.gov/?id=ED409895
