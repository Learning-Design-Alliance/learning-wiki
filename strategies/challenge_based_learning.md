---
type: strategy
status: draft
last_edited: 2026-04-06
---

# Challenge Based Learning

## Description
Challenge Based Learning (CBL) provides an efficient and effective framework for learning while solving real-world challenges. The framework fuels collaboration to identify big ideas, ask thoughtful questions, and identify, investigate and solve challenges. CBL helps learners gain deep subject area knowledge and develop the skills necessary to thrive in an ever-changing world.

## Design Implications

### Context
#### Requirements
- Structured framework, collaboration tools, access to real-world resources.
#### Constraints
- Requires careful planning and facilitation, may be challenging to align with curriculum, success depends on learner engagement and collaboration.
#### Implementation Variability
- <!-- TODO -->

### Target Learners
<!-- Link to sub-claims -->
- All learner levels

### Target Learning Goals
<!-- Link to sub-claims -->
- Solve real-world problems, foster collaboration and communication, promote lifelong learning skills, develop sustainable solutions.

### Affordances
- Real-world relevance, collaborative environment, hands-on learning, fosters lifelong learning skills, promotes sustainable solutions.

### Personalization
- Challenges can be adapted to learner interests and skill levels.

### Instructions
<!-- Steps with links to elements -->
- [[elements/engage|Engage]]
- [[elements/investigate|Investigate]]
- [[elements/act|Act]]
- [[elements/challenge-identification|Challenge identification]]
- [[elements/solution-development|Solution development]]
- [[elements/collaboration|Collaboration]]
- [[elements/problem-solving|Problem-Solving]]

## Assessment Evidence
- Quality of solutions developed, demonstration of collaboration and communication skills, application of knowledge to real-world problems.

## Impact
- Provides an efficient and effective framework for learning while solving real-world challenges (The Challenge Institute).

## Related Strategies
- <!-- TODO -->

## Related Elements
- <!-- TODO -->

## Tools
- <!-- TODO -->

## Examples
- <!-- TODO -->

## Key Sources
- <!-- TODO -->
