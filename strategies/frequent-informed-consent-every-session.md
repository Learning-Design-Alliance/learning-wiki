---
type: strategy
id: frequent-informed-consent-every-session
title: Give informed consent frequently at every peer counselling session to manage confidentiality dilemmas
description: "To tackle confidentiality dilemmas, the author recommends that peer counsellors be proactive in self-securing \"by frequently giving informed consent at every meeting of the sessions\"."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: zakaria-2007
    resource: "https://eric.ed.gov/?id=ED499793"
    title: "Zakaria, N. S. (2007). Peer Counselling Empowerment and Ethical Considerations. https://eric.ed.gov/?id=ED499793"
    author: Zakaria, N. S
---

# Give informed consent frequently at every peer counselling session to manage confidentiality dilemmas

> **Strategy** · [All strategies](index.md)
> **Evidence** · no claims cited

## Description
To tackle confidentiality dilemmas, the author recommends that peer counsellors be proactive in self-securing "by frequently giving informed consent at every meeting of the sessions". Peer counsellors must also be aware of circumstances that are not subject to confidentiality obligation, and should not scrutinize information that would be detrimental to clients or other people. The strategy operationalizes the client's right to privileged communication, which the author notes is not absolute.

## Design Implications

### Context
#### Requirements
- Peer counsellors must be able to grasp and analyze the possible ramifications of privileged communication made by the client
#### Constraints
- The privilege of confidential communication is not absolute; certain limitations apply that will not implicate the client or other people

### Target Learners
- peer counsellors

### Target Learning Goals
- ethical practice in confidentiality and privileged communication

## Related Strategies
- 

## Examples
-

## Key Sources
- Zakaria, N. S. (2007). Peer Counselling Empowerment and Ethical Considerations. https://eric.ed.gov/?id=ED499793
