---
type: strategy
id: assess-concerns-three-methods
title: "Use three methods to assess concerns: questionnaire, open-ended statements, and informal conversation"
description: "The paper describes three practical ways to determine individuals' or groups' concerns about an innovation."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-25
sources:
  - id: hall-1983
    resource: "https://eric.ed.gov/?id=ED251728"
    title: "Hall, Gene E.; Rutherford, William L. (1983). Client Concerns: A Guide to Facilitating Institutional Change. Research and Development Center for Teacher Education, The University of Texas at Austin. https://eric.ed.gov/?id=ED251728"
    author: Hall, Gene E.; Rutherford, William L
---

# Use three methods to assess concerns: questionnaire, open-ended statements, and informal conversation

> **Strategy** · [All strategies](index.md)
> **Evidence** · no claims cited

## Description
The paper describes three practical ways to determine individuals' or groups' concerns about an innovation. The most accurate is administering the 35-item SoC Questionnaire, which yields a graphic profile showing which stages are highest and lowest. A second method solicits written responses to the prompt "WHEN YOU THINK ABOUT THE INNOVATION, WHAT ARE YOU CONCERNED ABOUT?" with statements then analyzed against the seven stages. A third is informal conversation by a perceptive individual familiar with Stages of Concern, suited to counselors and others with non-threatening peer relationships with staff.

## Design Implications

### Context
#### Requirements
- For open-ended assessment, respondents are encouraged to provide at least three statements in complete sentences
#### Constraints
- The questionnaire is described as the most appropriate procedure for research and evaluation studies, implying the other methods suit practice settings

### Target Learners
- teachers and staff being assessed for innovation-related concerns

### Target Learning Goals
- identifying which Stages of Concern are most intense for individuals or groups

### Affordances
- [Stages Of Concern Seven Stage Taxonomy](../theories/stages-of-concern-seven-stage-taxonomy.md)

## Related Strategies
- 

## Examples
-

## Key Sources
- Hall, Gene E.; Rutherford, William L. (1983). Client Concerns: A Guide to Facilitating Institutional Change. Research and Development Center for Teacher Education, The University of Texas at Austin. https://eric.ed.gov/?id=ED251728
