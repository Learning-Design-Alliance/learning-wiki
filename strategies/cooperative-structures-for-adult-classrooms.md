---
type: strategy
id: cooperative-structures-for-adult-classrooms
title: Use named cooperative learning structures adapted to adult classrooms, including Student Team Learning, Think-Pair-Share, Jigsaw, Teaming Together, and Group Investigation
description: The handbook catalogs the most commonly used cooperative learning structures and advises teachers that knowing the concept matters more than the exact name.
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-25
sources:
  - id: you-can-be-in-a-group-and-still-not-cooperate-collaborative-approaches-and-cooperative-learning-activities-for-adult-learners-1993
    resource: "https://eric.ed.gov/?id=ED361492"
    title: "You Can Be in a Group and Still Not Cooperate: Collaborative Approaches and Cooperative Learning Activities for Adult Learners. (1993). Parma City School District. https://eric.ed.gov/?id=ED361492"
    author: "You Can Be in a Group and Still Not Cooperate: Collaborative Approaches and Cooperative Learning Activities for Adult Learners"
---

# Use named cooperative learning structures adapted to adult classrooms, including Student Team Learning, Think-Pair-Share, Jigsaw, Teaming Together, and Group Investigation

> **Strategy** · [All strategies](index.md)

## Description
The handbook catalogs the most commonly used cooperative learning structures and advises teachers that knowing the concept matters more than the exact name. Think-Pair-Share lets individuals think alone, pair to discuss, then share with the whole group, and "allows the shy person to gain confidenceby just sharing with one other person instead of a wholegroup." Jigsaw, originally designed by Elliot Aronson, uses home teams and expert representatives; Student Team Learning adds team goals and inter-team competition with individual scores based on improvement; Teaming Together suits group essays and problem solving; Group Investigation has small groups divide subtopics into individual tasks.

## Design Implications

### Context
#### Requirements
- Jigsaw is not advisable to use until the teacher has some experience with cooperative learning
- Teachers should know the concept of the structure rather than the exact name
#### Constraints
- The handbook is unsure about the effectiveness of Student Team Learning in adult learning situations because of the competition component and the uncertainty of adults' attendance in ABLE programs
- Group Investigation is effective where an entire class studies the same topic, which is often not the case in ABLE programs

### Target Learners
- Adult basic education learners
- Adult ESL learners
- GED students

### Target Learning Goals
- Participation of shy learners
- Efficient dissemination of information
- Group problem solving and content learning

## Related Strategies
- 

## Examples
-

## Key Sources
- You Can Be in a Group and Still Not Cooperate: Collaborative Approaches and Cooperative Learning Activities for Adult Learners. (1993). Parma City School District. https://eric.ed.gov/?id=ED361492
