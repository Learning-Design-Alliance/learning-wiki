---
type: strategy
status: draft
last_edited: 2026-04-06
---

# Teach Academic Vocabulary

## Description
Provide explicit instruction on academic vocabulary that will be encountered during the project. Knowing these words ahead of time can help you plan instruction for teaching them to students inside the project. While all students will need this scaffolding, ELL students may require more targeted instruction.

## Design Implications

### Context
#### Requirements
- Identification of key vocabulary and explicit instructional strategies.
#### Constraints
- May be time-consuming and require careful selection of relevant terms.
#### Implementation Variability
- <!-- TODO -->

### Target Learners
<!-- Link to sub-claims -->
- All students, particularly ELL students.

### Target Learning Goals
<!-- Link to sub-claims -->
- Improve vocabulary, promote academic language acquisition, and enhance content understanding.

### Affordances
- Enhances comprehension of project materials and improves the ability to communicate effectively.

### Personalization
- Offer tiered vocabulary instruction, providing more support for ELLs and advanced challenges for native speakers.

### Instructions
<!-- Steps with links to elements -->
- [[elements/direct-instruction|Direct instruction]]
- [[elements/summarization-and-synthesis|Summarization and Synthesis]]

## Assessment Evidence
- Improved vocabulary test scores, increased use of academic language in projects, and better content comprehension.

## Impact
- Helps plan instruction for teaching vocabulary

## Related Strategies
- <!-- TODO -->

## Related Elements
- <!-- TODO -->

## Tools
- <!-- TODO -->

## Examples
- <!-- TODO -->

## Key Sources
- <!-- TODO -->
