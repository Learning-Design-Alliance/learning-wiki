---
type: strategy
status: draft
last_edited: 2026-04-06
---

# Formative Assessment to Gauge Student Progress and Guide Instruction

## Description
Assessing student work on a regular basis provides both the teacher and the student with a sense of the student’s progress, indicates strengths and areas for improvement, and helps the teacher plan appropriately to meet the student’s needs. This formative assessment is part of the process of a learning sequence.

## Design Implications

### Context
#### Requirements
- Regularly scheduled formative assessments, tools for data collection and feedback (e.g., Flip, Padlet, Google Sites, weekly surveys).
#### Constraints
- Requires time for data collection and feedback, may require adaptation to different learning environments and student needs.
#### Implementation Variability
- <!-- TODO -->

### Target Learners
<!-- Link to sub-claims: [[claims/claim-slug]] -->
- All adult learners in online or blended learning environments.

### Target Learning Goals
<!-- Link to sub-claims: [[claims/claim-slug]] -->
- Provide diagnostic feedback, help educators set standards, evaluate progress, relate to a student’s progress, support student self-evaluation, and support teacher self-evaluation.

### Affordances
- Provides regular feedback, identifies strengths and weaknesses, helps students gauge their progress, and supports instructional planning.

### Personalization
- Uses rubrics, quizzes, observation protocols, quick comprehension check questions, or exit tickets to structure the assessment.

### Instructions
<!-- Steps with links to elements -->
- [[elements/provide-feedback|Provide Feedback]]
- [[elements/assess-performance|Assess Performance]]
- [[elements/practice|Practice]]

## Assessment Evidence
- Student work samples, reflection videos, photos/screenshots of progress, weekly surveys, and progress trackers.

## Impact
- Provides a clear sense of student progress, identifies areas for improvement, and supports instructional planning to meet the student’s needs.

## Related Strategies
- <!-- TODO -->

## Related Elements
- <!-- TODO -->

## Tools
- <!-- TODO -->

## Examples
- <!-- TODO -->

## Key Sources
- <!-- TODO -->
