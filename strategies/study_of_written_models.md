---
type: strategy
title: Study of Written Models
description: Direct, guided practice was found to be an effective instructional strategy, especially for students with low skills.
status: review
generated:
  by: "process:wiki-ingest"
  at: 2026-04-06
---

# Study of Written Models

## Description
Direct, guided practice was found to be an effective instructional strategy, especially for students with low skills. Many adult education students are not familiar with different types of written genres; the explicit study of formats, styles, tones, vocabularies, sentence structures, etc., can provide new frames and words for their own work.

## Design Implications

### Context
#### Requirements
- Written models, guided practice, feedback on imitation
#### Constraints
- Requires careful selection of models, may limit creativity
#### Implementation Variability
- <!-- TODO -->

### Target Learners
<!-- Link to sub-claims -->
- All writers, especially students with low skills

### Target Learning Goals
<!-- Link to sub-claims -->
- Improve writing skills, enhance genre awareness, promote effective writing

### Affordances
- Provides models for writing, enhances genre awareness, promotes effective writing

### Personalization
- Models can be tailored to different genres and skill levels.

### Instructions
<!-- Steps with links to elements -->
- [Demonstration](../elements/demonstration.md)
- [Practice](../elements/practice.md)
- [Provide feedback](../elements/provide-feedback.md)

## Assessment Evidence
- Improved writing quality, increased genre awareness, greater writing effectiveness

## Impact
- The explicit study of formats, styles, tones, vocabularies, sentence structures, etc., can provide new frames and words for their own work.

## Related Strategies
- <!-- TODO -->

## Related Elements
- <!-- TODO -->

## Tools
- <!-- TODO -->

## Examples
- <!-- TODO -->

## Key Sources
- <!-- TODO -->
