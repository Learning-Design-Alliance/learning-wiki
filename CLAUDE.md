...
`reverse-index.json` answers "which pages point at this one". It is generated by
`scripts/build_reverse_index.py` and regenerated by `build_indexes.py`, so anything that
adds or relinks a page already refreshes it; `--check` fails when the committed copy is
stale.

It exists because **the forward links from a principle are thin and the reverse links to it
are not**:

| edge | forward | reverse |
|---|---|---|
| principle ↔ strategy | 43 / 198 (22%) | **1752 / 2557 (69%)** |
| element ↔ strategy | — | **2390 / 2557 (93%)** |
| claim ↔ strategy | — | **2324 / 2557 (91%)** |

Three times the coverage, and the more trustworthy direction: the author of a strategy knew
which principle they were applying, while the author of a principle page is guessing at
which strategies might exemplify it. **So do not hand-populate `## Examples` on 198
principle pages** — one pass over the corpus gets more, and gets it right.

Claim edges carry the citation's `polarity` and `strength` from the inline marker, which is
what lets the design side weight evidence per citation rather than per claim.

It is committed on purpose. CI on the design-spec side has no wiki checkout, so an index
that only exists after someone runs a script is an index that side cannot use.
...
for evidence quality. It beats the frontmatter `evidence_strength:` in a way worth naming:
strength attaches to the **citation**, not the claim, so a claim cited `[+S]` on one page
and `[~W]` on another records a disagreement one number per claim would flatten. Polarity
carries as much as strength — a principle citing `[-S]` cites strong evidence *against*,
which is a constraint on the principle, not support for it.

`python3 scripts/check_evidence_markers.py` reports citations without one: **244 across 196
pages**, out of 11,697 outside `claims/` — 97.9% already carry one. Links from `claims/` are
excluded, because a claim linking a claim is a `## Related Claims` relation rather than a
citation (247 of the 491 raw unmarked links are that).

**It is a script, not a lint check, and deliberately so.** Adding it to `lint.py` would take
it from 0 to permanently red, destroying the property this file leans on — that a count of
zero means the work is done rather than the check being broken. `find_title_duplicates.py`
sets the precedent. And the backlog cannot be cleared mechanically: a marker asserts a
polarity and a strength about how *this page* uses *this claim*, so deriving it from the
claim's frontmatter, or copying the majority marker other pages used, would manufacture
exactly the kind of plausible unverified detail this repo already lost weeks to on DOIs.

**Do not clean up `evidence_strength:`.** 19 distinct values across 422 claims, 117 blank,
nine of them spellings of "not yet". The inline notation supersedes it and the design side
does not read it.
