---
type: theory
title: Cognitive theory of multimedia learning with the attention-guiding principle as a framework for e-book design
description: "The article applies the cognitive theory of multimedia learning (Mayer & Moreno) together with Betrancourt's attention-guiding principle to explain how e-book features shape young bilingual readers' attention."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: yow-2019
    resource: "https://doi.org/10.1177/2332858419878126"
    title: "Yow, W. Q., & Priyashri, S. (2019). Computerized Electronic Features Direct Children's Attention to Print in Single- and Dual-Language e-Books. AERA Open. https://doi.org/10.1177/2332858419878126"
    author: "Yow, W. Q., & Priyashri, S"
---

# Cognitive theory of multimedia learning with the attention-guiding principle as a framework for e-book design

> **Theory** · [All theories](index.md)

## Description
The article applies the cognitive theory of multimedia learning (Mayer & Moreno) together with Betrancourt's attention-guiding principle to explain how e-book features shape young bilingual readers' attention. Multimodal inputs "can provide important connections between the visual and auditory channels," while cueing such as finger-tracking animation guides attention to relevant details and reduces extraneous cognitive load. The framework predicts that matched dual-channel inputs plus attention-guiding cues direct children's attention to target-language print, which the two studies support.

## Design Implications

### Context
#### Requirements
- Multimedia inputs must be semantically related and synchronized across visual and audio channels so children can process information in separate channels without overload
#### Constraints
- The article did not test single input channels alone, so whether one channel alone suffices, or the synergistic two-modality display is key, remains open

### Target Learners
- Bilingual preschoolers aged 4 to 6 developing emergent literacy

### Target Learning Objectives
- Attention to print, word recognition, and emergent biliteracy development

### Claims
- [Enhancing Features Direct Attention Dual Language E Books](../claims/enhancing-features-direct-attention-dual-language-e-books.md) [+M]
- [Enhancing Features Increase Attention Single Language E Books](../claims/enhancing-features-increase-attention-single-language-e-books.md) [+M]

## Related Theories
- 

## Examples
-

## Key Sources
- Yow, W. Q., & Priyashri, S. (2019). Computerized Electronic Features Direct Children's Attention to Print in Single- and Dual-Language e-Books. AERA Open. https://doi.org/10.1177/2332858419878126
