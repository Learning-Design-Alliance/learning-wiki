---
type: theory
title: "BICS/CALP distinction: conversational and academic language proficiency as conceptually distinct dimensions with different developmental patterns"
description: The BICS/CALP distinction separates basic interpersonal communicative skills (conversational language) from cognitive academic language proficiency.
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-25
sources:
  - id: cummins-1999
    resource: "https://eric.ed.gov/?id=ED438551"
    title: "Cummins, J. (1999). BICS and CALP: Clarifying the Distinction. ERIC Document ED 438 551. https://eric.ed.gov/?id=ED438551"
    author: Cummins, J
---

# BICS/CALP distinction: conversational and academic language proficiency as conceptually distinct dimensions with different developmental patterns

> **Theory** · [All theories](index.md)

## Description
The BICS/CALP distinction separates basic interpersonal communicative skills (conversational language) from cognitive academic language proficiency. Cummins originally proposed it in 1979 as a qualification to Oller's single-factor global language proficiency claim, and this paper clarifies that the two dimensions "follow different developmental patterns": phonology and basic fluency plateau early, while CALP continues developing throughout schooling. It is a specific conceptual distinction with policy and practice implications, not an overall theory of language.

## Design Implications

### Context
#### Requirements
- Assessed in relation to the issues it attempts to address; the article states it does not address sociolinguistics, discourse styles, or the appropriate time to introduce English reading instruction.
#### Constraints
- The sequential BICS-before-CALP acquisition pattern was suggested as typical for immigrant children learning a second language, not as an absolute order applying in every situation.

### Target Learners
- bilingual and ESL students in elementary education

### Target Learning Objectives
- academic language proficiency (CALP) development in a second language

### Claims
- [Bics Acquired Faster Than Calc Immigrant Children](../claims/bics-acquired-faster-than-calc-immigrant-children.md) [+M]
- [Bics Calp Intersect Not Identical](../claims/bics-calp-intersect-not-identical.md) [+M]
- [Calp Can Precede Bics In Certain Situations](../claims/calp-can-precede-bics-in-certain-situations.md) [+M]

## Related Theories
- 

## Examples
-

## Key Sources
- Cummins, J. (1999). BICS and CALP: Clarifying the Distinction. ERIC Document ED 438 551. https://eric.ed.gov/?id=ED438551
