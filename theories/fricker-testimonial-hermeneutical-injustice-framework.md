---
type: theory
title: "Fricker's two-subset framework of epistemic injustice: testimonial and hermeneutical"
description: "The article adopts Miranda Fricker's account of epistemic injustice, which focuses on representation and participation in knowledge production and how unjust epistemic practices silence people and deprive society of e..."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: haarman-2021
    resource: "https://www.ovpes.org/"
    title: "Haarman, Susan. (2021). The Data Should Not Speak for Itself: Epistemic Injustice and Data as Rhetoric. Philosophical Studies in Education 52. https://www.ovpes.org/"
    author: Haarman, Susan
---

# Fricker's two-subset framework of epistemic injustice: testimonial and hermeneutical

> **Theory** · [All theories](index.md)

## Description
The article adopts Miranda Fricker's account of epistemic injustice, which focuses on representation and participation in knowledge production and how unjust epistemic practices silence people and deprive society of epistemic resources. It specifies two subsets: testimonial injustice, where an individual is treated unjustly in their capacity as a knower, and hermeneutical injustice, where a significant area of social experience is obscured from collective understanding owing to hermeneutical marginalization. The author uses this framework to analyze how datafication can challenge or exacerbate each type. The article stresses that hermeneutical injustice is connected to social power and privilege, not merely a gap in language.

## Design Implications

### Context
#### Requirements
- Understanding the framework requires attending to how power and privilege shape what is recognized as valid knowledge, since the article says epistemic cultures are the nexus where power, privilege, and oppression intersect with understandings of truth.
#### Constraints
- 

### Target Learners
- students in datafied learning environments
- marginalized communities subject to educational data collection

### Target Learning Objectives
- recognizing and redressing epistemic injustice in knowledge production and data practices

### Claims
- [Big Data Assumed Authenticity Amplifies Suppressed Testimony](../claims/big-data-assumed-authenticity-amplifies-suppressed-testimony.md) [+M]
- [Data Construction Embeds Privilege In Data](../claims/data-construction-embeds-privilege-in-data.md) [+M]
- [Big Data Variety Aids Hermeneutical Injustice Redress](../claims/big-data-variety-aids-hermeneutical-injustice-redress.md) [+M]

## Related Theories
- 

## Examples
-

## Key Sources
- Haarman, Susan. (2021). The Data Should Not Speak for Itself: Epistemic Injustice and Data as Rhetoric. Philosophical Studies in Education 52. https://www.ovpes.org/
