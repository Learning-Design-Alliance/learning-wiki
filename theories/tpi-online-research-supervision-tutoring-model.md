---
type: theory
title: Research project supervision (TPI) as a special case of tutoring with identified organizational and process elements
description: "The article presents a model for online research project tutoring (TPI) in which supervision is treated as \"a special case of tutoring that offers the student guidance and orientation in aspects linked to the acquisit..."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: darder-2015
    resource: "https://doi.org/10.7821/naer.2015.4.110"
    title: "Darder, A.; Pérez Garcias, A. (2015). Online tutoring procedure for research project supervision: management, organization and key elements. New Approaches in Educational Research 4(2). https://doi.org/10.7821/naer.2015.4.110"
    author: Darder, A.; Pérez Garcias, A
---

# Research project supervision (TPI) as a special case of tutoring with identified organizational and process elements

> **Theory** · [All theories](index.md)

## Description
The article presents a model for online research project tutoring (TPI) in which supervision is treated as "a special case of tutoring that offers the student guidance and orientation in aspects linked to the acquisition of research skills, motivation and personal orientation, administrative advice, and technological as well as professional issues." The model is represented as a conceptual map identifying TPI elements: the tutor (skills, supervision styles, functions), the trainee researcher and his/her needs, and the support and interaction between them and other partners, plus elements influencing project success and virtual-environment requirements. It was built iteratively through literature review, collaborative work groups, discussion groups, consultations, and implementation.

## Design Implications

### Context
#### Requirements
- Iterative design and development stages including consultation with tutors and trainee researchers and implementation with a tutor-trainee pair
#### Constraints
- Developed within the context of education sciences; the authors note adaptations may be needed for other fields of knowledge

### Target Learners
- trainee researchers in online post-graduate programs

### Target Learning Objectives
- research skills acquisition
- research project completion
- supervisor-trainee relationship quality

### Claims
- 

## Related Theories
- 

## Examples
-

## Key Sources
- Darder, A.; Pérez Garcias, A. (2015). Online tutoring procedure for research project supervision: management, organization and key elements. New Approaches in Educational Research 4(2). https://doi.org/10.7821/naer.2015.4.110
