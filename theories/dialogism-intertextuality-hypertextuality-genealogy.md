---
type: theory
title: "Dialogism, intertextuality, and hypertextuality: the language-centered genealogy leading to connectivism"
description: "The article describes a chain of language-centered theories within architectonic thought: Bakhtin's dialogism, in which \"languages do not exclude each other, but rather intersect with each other in many different ways..."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: dennis-2020
    resource: "https://www.irrodl.org/"
    title: "Dennis, J. (2020). Languaging Network Learning: The Emergence of Connectivism in Architectonic Thought. International Review of Research in Open and Distributed Learning, 21(3). https://www.irrodl.org/"
    author: Dennis, J
---

# Dialogism, intertextuality, and hypertextuality: the language-centered genealogy leading to connectivism

> **Theory** · [All theories](index.md)
> **Evidence** · no claims cited

## Description
The article describes a chain of language-centered theories within architectonic thought: Bakhtin's dialogism, in which "languages do not exclude each other, but rather intersect with each other in many different ways"; Kristeva's intertextuality, where "each word (text) is an intersection of word (text) where at least one other word (text) can be read"; and Nelson's hypertextuality, non-sequential electronic writing with branches and links. On this foundation, the article argues, the contemporary idea of connectivism emerges in architectonic thought, with dialogue and texts modeling the conceptual system of relations.

## Design Implications

### Context
#### Requirements
- Reading this genealogy requires connecting rather than siloing the perspectives of Kant, Peirce, Bakhtin, Kristeva, and Nelson
#### Constraints
- Kristeva's reading of Bakhtin is described by the article as controversial

### Target Learners
- higher education educators and scholars of learning theory

### Target Learning Objectives
- understanding how dialogic and textual theories underpin network learning

### Claims
- 

## Related Theories
- 

## Examples
-

## Key Sources
- Dennis, J. (2020). Languaging Network Learning: The Emergence of Connectivism in Architectonic Thought. International Review of Research in Open and Distributed Learning, 21(3). https://www.irrodl.org/
