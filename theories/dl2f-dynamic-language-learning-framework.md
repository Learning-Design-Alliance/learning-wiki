---
type: theory
title: "Dynamic Language Learning Framework (DL2F): a communication-centric, learner-centered instructional model for language education"
description: DL2F is a progressive pedagogical approach for language education that positions effective communication as the primary goal of learning.
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-25
sources:
  - id: merueña-2023
    resource: "https://doi.org/10.36713/epra2016"
    title: "Merueña, J. A. A. (2023). Dynamic Language Learning Framework (DL2F) Instructional Model. EPRA International Journal of Research and Development (IJRD). https://doi.org/10.36713/epra2016"
    author: Merueña, J. A. A
---

# Dynamic Language Learning Framework (DL2F): a communication-centric, learner-centered instructional model for language education

> **Theory** · [All theories](index.md)

## Description
DL2F is a progressive pedagogical approach for language education that positions effective communication as the primary goal of learning. The article states it is 'grounded in a communication-centric philosophy, positioning effective communication as the primary goal of language learning', prioritizing real-life language use, authentic contexts, adaptability to diverse learners, collaboration, cultural integration, and continuous improvement through feedback, self-assessment, and revision. Implementation relies on authentic materials, diverse tasks, and technology integration.

## Design Implications

### Context
#### Requirements
- Authentic materials, diverse tasks, and technology integration as key implementation components
#### Constraints
- Conceived for language education contexts; the article details adaptation specifically to Philippine secondary schools

### Target Learners
- secondary school language learners in the Philippines

### Target Learning Objectives
- communicative competence in authentic contexts
- cultural awareness
- collaborative language use

### Claims
- 

## Related Theories
- 

## Examples
-

## Key Sources
- Merueña, J. A. A. (2023). Dynamic Language Learning Framework (DL2F) Instructional Model. EPRA International Journal of Research and Development (IJRD). https://doi.org/10.36713/epra2016
