---
type: theory
title: Six areas of values in evaluation conflicts
description: A second organizing axis of the taxonomy is the set of value areas in which evaluation conflicts arise.
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: patry-2004
    resource: "https://eric.ed.gov/?id=EJ874233"
    title: "Patry, Jean-Luc, & Gastager, Angela. (2004). Kokybės vertinimo dilemos – Dilemmas in quality assessment. The Quality of Higher Education 2004/1. https://eric.ed.gov/?id=EJ874233"
    author: "Patry, Jean-Luc, & Gastager, Angela"
---

# Six areas of values in evaluation conflicts

> **Theory** · [All theories](index.md)

## Description
A second organizing axis of the taxonomy is the set of value areas in which evaluation conflicts arise. The abstract states: "Six areas of values are discussed: the ethical, methodical, social and interactive, legal, economic, and personal values." Conflicts between values from these areas form the cells of the taxonomy that is then applied to concrete evaluation situations.

## Design Implications

### Context
#### Requirements
- Analyzing a conflict requires locating the values involved within one or more of the six named areas.
#### Constraints
- 

### Target Learners
- evaluators and quality assessors in higher education

### Target Learning Objectives
- classifying evaluation conflicts by value area

### Claims
- 

## Related Theories
- 

## Examples
-

## Key Sources
- Patry, Jean-Luc, & Gastager, Angela. (2004). Kokybės vertinimo dilemos – Dilemmas in quality assessment. The Quality of Higher Education 2004/1. https://eric.ed.gov/?id=EJ874233
