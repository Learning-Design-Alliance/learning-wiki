---
type: theory
title: Strength and Retrieval Effort Account (New Theory of Disuse and Bifurcation)
description: "This account holds that effortful retrieval strengthens knowledge and that \"The degree to which knowledge is strengthened is assumed to be proportional to the amount of effort involved in retrieval\"."
status: draft
generated:
  by: claude/unspecified
  at: 2026-09-24
sources:
  - id: karpicke-2017
    resource: "https://doi.org/10.1016/B978-0-12-809324-5.21055-9"
    title: "Karpicke, J. D. (2017). Retrieval-Based Learning: A Decade of Progress. Learning and Memory: A Comprehensive Reference, 2nd edition, Volume 2. https://doi.org/10.1016/B978-0-12-809324-5.21055-9"
    author: Karpicke, J. D
---

# Strength and Retrieval Effort Account (New Theory of Disuse and Bifurcation)

> **Theory** · [All theories](index.md)
> **Evidence** · 4 claims (2 mixed, 2 unmarked) · 4 studies, `q2`–`q4` · 3 of 4 report an effect size · 2 claims rest on one study

## Description
This account holds that effortful retrieval strengthens knowledge and that "The degree to which knowledge is strengthened is assumed to be proportional to the amount of effort involved in retrieval". Bjork and Bjork's new theory of disuse distinguishes storage and retrieval strength; the bifurcation account of Kornell et al. (2011) adds that only retrieved items are strengthened, explaining delay-dependent effects.

## Design Implications

### Context
#### Requirements
- Retrieval must involve effort, for example retrieving items with low retrieval strength.
#### Constraints
- The chapter states the proposal that retrieved items become strengthened is essentially a redescription of the phenomenon rather than an explanation, amounting to a heuristic description.

### Target Learners
- Learners in educational settings
- Researchers comparing theories of retrieval-based learning

### Target Learning Objectives
- Explain why effortful retrieval conditions confer greater long-term benefits than easier ones

### Claims
- [Retrieval Practice Effects More Robust When Initial Retrieval Success Exceeds 75 Percent](../claims/retrieval-practice-effects-more-robust-when-initial-retrieval-success-exceeds-75-percent.md) [~M]
- [Repeated Retrieval Alters The Rate Of Forgetting](../claims/repeated-retrieval-alters-the-rate-of-forgetting.md) [~W]

## Related Theories
- [Desirable Difficulties Enhance Learning](../claims/desirable-difficulties-enhance-learning.md)
- [Desirable Difficulties Improve Retention](../claims/desirable-difficulties-improve-retention.md)

## Examples
-

## Key Sources
- Karpicke, J. D. (2017). Retrieval-Based Learning: A Decade of Progress. Learning and Memory: A Comprehensive Reference, 2nd edition, Volume 2. https://doi.org/10.1016/B978-0-12-809324-5.21055-9
