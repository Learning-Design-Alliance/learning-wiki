---
type: theory
title: Contrastive Rhetoric Model of L2 writing
description: "The Contrastive Rhetoric Model explains L2 writing through the influence of a writer's first language and culture."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-25
sources:
  - id: chmarkh-2025
    resource: "https://eric.ed.gov/?q=Chmarkh+contrastive+rhetoric+cognitive+sociocultural+L2+writing"
    title: "Chmarkh, M. (2025). Evaluating the Contrastive Rhetoric Model, the Cognitive Model, and the Sociocultural Model to L2 Writing. https://eric.ed.gov/?q=Chmarkh+contrastive+rhetoric+cognitive+sociocultural+L2+writing"
    author: Chmarkh, M
---

# Contrastive Rhetoric Model of L2 writing

> **Theory** · [All theories](index.md)
> **Evidence** · 1 claim (1 for) · 1 study, `q1` · 0 of 1 report an effect size · 1 claim rests on one study

## Description
The Contrastive Rhetoric Model explains L2 writing through the influence of a writer's first language and culture. The article states that it "focuses on the influence of the L1 and one's own culture on the process of writing in a second language", building on Kaplan's claim that linguistic communities share collective thought patterns reflected in their writing. It has focused primarily on international student L2 writers in ESL contexts in American higher education.

## Design Implications

### Context
#### Requirements
- Writers' L1 and cultural background must be treated as relevant factors in understanding their L2 writing
#### Constraints
- The article notes the model focuses primarily on international student L2 writers in ESL contexts in American higher education institutions

### Target Learners
- International student L2 writers in higher education

### Target Learning Objectives
- Second language writing development

### Claims
- [Contrastive Rhetoric Overgeneralizes Culture](../claims/contrastive-rhetoric-overgeneralizes-culture.md) [+M]

## Related Theories
- 

## Examples
-

## Key Sources
- Chmarkh, M. (2025). Evaluating the Contrastive Rhetoric Model, the Cognitive Model, and the Sociocultural Model to L2 Writing. https://eric.ed.gov/?q=Chmarkh+contrastive+rhetoric+cognitive+sociocultural+L2+writing
