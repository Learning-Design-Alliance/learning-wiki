---
type: theory
title: Three patterns of instructional adaptation to individual differences, each with distinct measurement requirements
description: Drawing on Cronbach (1967), the paper identifies three patterns of adapting instruction to individual differences.
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-25
sources:
  - id: nitko-1970
    resource: "https://eric.ed.gov/?id=ED047010"
    title: "Nitko, Anthony J. (1970). Criterion-Referenced Testing in the Context of Instruction. https://eric.ed.gov/?id=ED047010"
    author: Nitko, Anthony J
---

# Three patterns of instructional adaptation to individual differences, each with distinct measurement requirements

> **Theory** · [All theories](index.md)

## Description
Drawing on Cronbach (1967), the paper identifies three patterns of adapting instruction to individual differences. In the first, goals and methods are fixed and differences are accommodated by dropping students along the way. In the second, a student's future role determines an appropriate curriculum with optional objectives but fixed procedures within each option. In the third, "instructional procedures are varied to accommodate the differences in each student", via branching to remedial work or tailoring a course to assessed learning habits, achievements, and cognitive style. Each pattern, the paper argues, requires different kinds of measurement.

## Design Implications

### Context
#### Requirements
- The testing program must be designed into the instructional process so required information is easily obtained and available in usable form when a decision needs to be made.
#### Constraints
- The paper states the merits of any testing approach cannot be judged without knowing the instructional pattern and decision requirements involved.

### Target Learners
- students across ability levels in school systems adapting to individual differences

### Target Learning Objectives
- matching instruction and measurement to individual student needs and goals

### Claims
- 

## Related Theories
- 

## Examples
-

## Key Sources
- Nitko, Anthony J. (1970). Criterion-Referenced Testing in the Context of Instruction. https://eric.ed.gov/?id=ED047010
