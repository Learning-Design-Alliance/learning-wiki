---
type: theory
title: "Guided practice framework: five interacting elements for analyzing close-to-the-classroom work with teachers"
description: The authors define guided practice as face-to-face, close-to-the-classroom work on teaching and learning to teach, carried out under labels such as clinical supervision, coaching, advising and mentoring.
status: draft
generated:
  by: claude/unspecified
  at: 2026-09-25
sources:
  - id: feiman-nemser-1994
    resource: "https://eric.ed.gov/?id=ED377169"
    title: "Feiman-Nemser, Sharon, et al. (1994). Guiding Teacher Learning: Insider Studies of Classroom-Based Work with Teachers. Craft Paper 94-1. National Center for Research on Teacher Learning. https://eric.ed.gov/?id=ED377169"
    author: Feiman-Nemser, Sharon, et al
---

# Guided practice framework: five interacting elements for analyzing close-to-the-classroom work with teachers

> **Theory** · [All theories](index.md)
> **Evidence** · no claims cited

## Description
The authors define guided practice as face-to-face, close-to-the-classroom work on teaching and learning to teach, carried out under labels such as clinical supervision, coaching, advising and mentoring. Rather than promote a specific model, they offer analytic tools: "These considerations led us to identify five interacting elements: (1) participants and their relationship, (2) goals, (3) practices, (4) context and (5) conceptual underpinnings." Practices sit at the center of the framework, and each element is analyzed descriptively and normatively. The framework is used to compare traditions and to study particular enactments of guided practice.

## Design Implications

### Context
#### Requirements
- Analysis must consider all five elements and their interactions, since goals, relationship, practices, context and beliefs shape one another
#### Constraints
- The framework is an analytic tool for appraising approaches, not a prescribed model of effective guided practice

### Target Learners
- prospective and practicing K-12 teachers

### Target Learning Objectives
- learning to teach
- learning from teaching
- instructional improvement

### Claims
- 

## Related Theories
- [Guided Practice Four Purposes](../patterns/guided-practice-four-purposes.md)

## Examples
-

## Key Sources
- Feiman-Nemser, Sharon, et al. (1994). Guiding Teacher Learning: Insider Studies of Classroom-Based Work with Teachers. Craft Paper 94-1. National Center for Research on Teacher Learning. https://eric.ed.gov/?id=ED377169
