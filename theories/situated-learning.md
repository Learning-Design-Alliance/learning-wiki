---
type: theory
title: Situated Learning
description: Situated learning holds that knowledge is inseparable from the social, material, and cultural contexts in which it is used.
status: review
generated:
  by: claude/unspecified
  at: 2026-08-29
---

# Situated Learning

## Description
Situated learning holds that knowledge is inseparable from the social, material, and cultural contexts in which it is used. Learning is strengthened when it happens through participation in authentic practices rather than only through abstract decontextualized instruction.

## Core Idea
- Cognition is shaped by activity, tools, roles, and communities, not just by the individual mind in isolation.

## Implications
- Supports authentic tasks, apprenticeship, communities of practice, and scenario-based learning.
- Qualifies theories that treat learning only as internal information processing.

## Claims
- [Teachers actively synthesize professional-development practices with their own context rather than transferring them intact](../claims/teachers-synthesize-pd-practices-with-context-not-just-transfer-them.md) [+M]

## Related Theories
- [Constructivism](constructivism.md)
- [Cognitive Apprenticeship](cognitive-apprenticeship.md)
- [Boundary Crossing Learning](boundary-crossing-learning.md) — extends situated learning to the specific interpretive work required when a learner moves between two distinct authentic practices, not just into one
- [Sociocultural Theory](sociocultural-theory.md) — shares the view that knowledge and cognition are inseparable from social and cultural context
- [Connectivism](connectivism.md) — also locates learning outside the individual mind, but in digital networks rather than embodied social practice

## Examples
- [Epistemic Games](../patterns/epistemic-games.md)

## Key Sources
- Lave, J., & Wenger, E. (1991). *Situated learning: Legitimate peripheral participation*. Cambridge University Press.
