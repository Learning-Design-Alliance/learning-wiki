---
type: theory
title: "Värdegrunden: the läroplan as consensual steering document of constitutive democratic values"
description: "The Swedish läroplan is \"a consensually constructed document and a symbol of broad political agreement\" and a steering document identifying the steering values in education."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: norberg-2003
    resource: "http://www.ed.psu.edu/uceacsle/"
    title: "Norberg, K. (2003). Constitutive Values and Daily Practice in a Swedish School. Values and Ethics in Educational Administration, 1(3). http://www.ed.psu.edu/uceacsle/"
    author: Norberg, K
---

# Värdegrunden: the läroplan as consensual steering document of constitutive democratic values

> **Theory** · [All theories](index.md)
> **Evidence** · no claims cited

## Description
The Swedish läroplan is "a consensually constructed document and a symbol of broad political agreement" and a steering document identifying the steering values in education. When the 1994 läroplan was introduced, a new label for the constitutive values of the school system was invented: "värdegrunden". The revised Lpo94 opened with norms and values rather than knowledge, symbolically emphasising that schooling can prioritise values over knowledge, and each sector document begins by identifying democratic values all Swedish schools should represent and impart.

## Design Implications

### Context
#### Requirements
- Values should saturate all activities and be shared by all working within a school
#### Constraints
- There is a perpetual struggle to reconcile the constitutive values of the national läroplan with outside values, personal values and school practices

### Target Learners
- All adults and children in Swedish pre-school, compulsory and non-compulsory schooling

### Target Learning Objectives
- Fostering a shared value system and preparing children for active participation in civic life through democratic organisation of daily work

### Claims
- 

## Related Theories
- 

## Examples
-

## Key Sources
- Norberg, K. (2003). Constitutive Values and Daily Practice in a Swedish School. Values and Ethics in Educational Administration, 1(3). http://www.ed.psu.edu/uceacsle/
