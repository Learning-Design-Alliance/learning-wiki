---
type: theory
title: "Learning cycle instructional model: exploration, concept introduction, concept application"
description: "The guide grounds its activities in the learning cycle, a sequence science educators have identified as \"an effective means for learning concepts and processes of science\" that is \"also..."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: reilly-2008
    resource: "https://www.blm.gov/education"
    title: "Reilly, K., Wooster, B., Andrews, E., Bourdeau, V., Conley, R., & Thompson, M. (2008). Holding onto the GREEN Zone Leader Guide: A Youth Program for the Study and Stewardship of Community Riparian Areas. Bureau of Land Management. https://www.blm.gov/education"
    author: "Reilly, K., Wooster, B., Andrews, E., Bourdeau, V., Conley, R., & Thompson, M"
---

# Learning cycle instructional model: exploration, concept introduction, concept application

> **Theory** · [All theories](index.md)

## Description
The guide grounds its activities in the learning cycle, a sequence science educators have identified as "an effective means for learning concepts and processes of science" that is "also... effective for developing reasoning abilities and for reducing scientific misconceptions." Three phases are emphasized: Exploration, where learners develop their own questions and frame of reference with minimal guidance; Concept Introduction, where riparian science concepts are introduced through activities applying new patterns of thinking; and Concept Application, where learners apply newfound knowledge to develop scientific thinking and decision-making skills.

## Design Implications

### Context
#### Requirements
- Educators provide only minimal guidance during the exploration phase to maximize its benefits
#### Constraints
- The guide attributes the model's effectiveness to prior science-education work rather than testing it in this curriculum

### Target Learners
- youth in grades 5-8 learning Earth Science

### Target Learning Objectives
- science concept learning
- scientific reasoning abilities
- practical science inquiry methods and interpretation

### Claims
- 

## Related Theories
- 

## Examples
-

## Key Sources
- Reilly, K., Wooster, B., Andrews, E., Bourdeau, V., Conley, R., & Thompson, M. (2008). Holding onto the GREEN Zone Leader Guide: A Youth Program for the Study and Stewardship of Community Riparian Areas. Bureau of Land Management. https://www.blm.gov/education
