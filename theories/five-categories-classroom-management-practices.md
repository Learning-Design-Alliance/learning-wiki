---
type: theory
title: Five broad categories of evidence-based classroom management practices (Simonsen et al. taxonomy)
description: "The article organizes evidence-based classroom management practices, drawing on a prior systematic review of 81 studies, into five broad categories: \"(1) maximize structure and predictability, (2) post, teach, review,..."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-25
sources:
  - id: oliver-2011
    resource: "https://www.sree.org"
    title: "Oliver, R. M., Wehby, J. H., & Reschly, D. J. (2011). Teacher Classroom Management Practices: Effects on Disruptive or Aggressive Student Behavior. 2011 SREE Conference. https://www.sree.org"
    author: "Oliver, R. M., Wehby, J. H., & Reschly, D. J"
---

# Five broad categories of evidence-based classroom management practices (Simonsen et al. taxonomy)

> **Theory** · [All theories](index.md)

## Description
The article organizes evidence-based classroom management practices, drawing on a prior systematic review of 81 studies, into five broad categories: "(1) maximize structure and predictability, (2) post, teach, review, and provide feedback on expectations, (3) actively engage students in observable ways, (4) use a continuum of strategies to acknowledge appropriate behavior, and (5) use a continuum of strategies to respond to inappropriate behavior". This taxonomy frames the multi-component universal programs the meta-analysis evaluates.

## Design Implications

### Context
#### Requirements
- Practices must be actions performed by the classroom teacher in the context of the classroom, implemented universally with the entire class
#### Constraints
- The taxonomy is attributed to a prior review (Simonsen et al., 2008) rather than derived in this article

### Target Learners
- School-aged children in kindergarten through grade 12, ages 5-18

### Target Learning Objectives
- Supporting prosocial behavior and preventing and reducing inappropriate classroom behavior

### Claims
- [Universal Classroom Management Reduces Problem Behavior](../claims/universal-classroom-management-reduces-problem-behavior.md) [+M]

## Related Theories
- 

## Examples
-

## Key Sources
- Oliver, R. M., Wehby, J. H., & Reschly, D. J. (2011). Teacher Classroom Management Practices: Effects on Disruptive or Aggressive Student Behavior. 2011 SREE Conference. https://www.sree.org. [doi:10.4073/csr.2011.4](https://doi.org/10.4073/csr.2011.4)
