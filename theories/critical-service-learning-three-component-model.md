---
type: theory
title: "Critical service-learning model with three components: social change orientation, power redistribution, and authentic relationships"
description: "Critical service-learning is a model that adopts a social justice framework, as opposed to a more \"apolitical helper\" model of service-learning, and demands an analysis of power structures and social change (Mitchell,..."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: harkins-2021
    resource: "https://doi.org/10.3998/mjcsloa.3239521.0026.203"
    title: "Harkins, D. A., Grenier, L. I., Irizarry, C., Robinson, E., Ray, S., & Shea, L.-M. (2021). Building relationships for critical service-learning. Michigan Journal of Community Service Learning, 26(2), 21–38. https://doi.org/10.3998/mjcsloa.3239521.0026.203"
    author: "Harkins, D. A., Grenier, L. I., Irizarry, C., Robinson, E., Ray, S., & Shea, L.-M"
---

# Critical service-learning model with three components: social change orientation, power redistribution, and authentic relationships

> **Theory** · [All theories](index.md)

## Description
Critical service-learning is a model that adopts a social justice framework, as opposed to a more "apolitical helper" model of service-learning, and demands an analysis of power structures and social change (Mitchell, 2008). The article identifies three components: "(a) providing a social change orientation; (b) working to redistribute power; and (c) developing authentic relationships." Authentic relationships challenge the self-other binary, emphasizing interdependence and reciprocity. This study operationalizes the authentic relationships component by examining professor, community partner, and peer mentor relationships.

## Design Implications

### Context
#### Requirements
- Adopts a social justice framework rather than an apolitical helper model
- Demands analysis of power structures and social change
#### Constraints
- Service-learning has not fully realized its original civic purpose without moving toward justice-oriented pedagogy

### Target Learners
- Undergraduate students in service-learning courses

### Target Learning Objectives
- Civic engagement attitudes including civic action, political awareness, social justice attitudes, and diversity attitudes

### Claims
- [Peer Mentoring Increases Civic Action Service Learning](../claims/peer-mentoring-increases-civic-action-service-learning.md) [+M]
- [Slam Relationship Quality Correlates Civic Action](../claims/slam-relationship-quality-correlates-civic-action.md) [+M]
- [Professor Relationship Quality Civic Engagement Bidirectional](../claims/professor-relationship-quality-civic-engagement-bidirectional.md) [+M]
- [Community Partner Conflict Negatively Correlates Social Justice](../claims/community-partner-conflict-negatively-correlates-social-justice.md) [+M]

## Related Theories
- 

## Examples
-

## Key Sources
- Harkins, D. A., Grenier, L. I., Irizarry, C., Robinson, E., Ray, S., & Shea, L.-M. (2021). Building relationships for critical service-learning. Michigan Journal of Community Service Learning, 26(2), 21–38. https://doi.org/10.3998/mjcsloa.3239521.0026.203
