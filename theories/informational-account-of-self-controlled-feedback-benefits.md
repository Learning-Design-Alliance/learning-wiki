---
type: theory
title: Informational (error-estimation) account of self-controlled feedback benefits
description: The article contrasts two explanations for why self-controlled knowledge of results (KR) aids motor learning.
status: draft
generated:
  by: claude/unspecified
  at: 2026-09-24
sources:
  - id: carter-2014
    resource: "https://doi.org/10.3389/fpsyg.2014.01325"
    title: "Carter, M. J., Carlsen, A. N., & Ste-Marie, D. M. (2014). Self-controlled feedback is effective if it is based on the learner’s performance: a replication and extension of Chiviacowsky and Wulf (2005). Frontiers in Psychology, 5, 1325. https://doi.org/10.3389/fpsyg.2014.01325"
    author: "Carter, M. J., Carlsen, A. N., & Ste-Marie, D. M"
---

# Informational (error-estimation) account of self-controlled feedback benefits

> **Theory** · [All theories](index.md)
> **Evidence** · 6 claims (4 for, 2 mixed) · 1 study, `q3` · 1 of 1 report an effect size · 6 claims rest on one study

## Description
The article contrasts two explanations for why self-controlled knowledge of results (KR) aids motor learning. The motivational explanation says self-control "satisfies the basic psychological needs of autonomy and competence"; the information processing perspective says benefits are driven by "performance-dependent KR strategies". The authors conclude that error estimation, available when the KR decision follows the movement, is the more critical mechanism.

## Design Implications

### Context
#### Requirements
- The article states the learner must be able to base the KR decision on the performance just completed: self-controlled learning benefits "depend on the option of making the KR decision after completing one’s motor response".
#### Constraints
- The study collected no autonomy or motivation measure regarding choice over the KR schedule, so the motivational side was assumed rather than measured.
- The authors state that an immediate retention test would be required to assess offline learning, so the consolidation explanation is offered as initial support only.

### Target Learners
- Young adult volunteers learning a novel motor skill (university sample in this study)

### Target Learning Objectives
- Retention and transfer of a motor skill
- Accurate error detection and correction (estimating one’s own movement outcome)

### Claims
- [Self Controlled Kr Decided After Trial Improves Motor Retention](../claims/self-controlled-kr-decided-after-trial-improves-motor-retention.md) [+M]
- [Self Controlled Kr Decided After Trial Improves Motor Transfer](../claims/self-controlled-kr-decided-after-trial-improves-motor-transfer.md) [+M]
- [Self Controlled Kr Decided Before Trial Shows No Benefit Over Yoked](../claims/self-controlled-kr-decided-before-trial-shows-no-benefit-over-yoked.md) [+M]
- [Self Controlled Kr Decided After Trial Improves Error Estimation Accuracy](../claims/self-controlled-kr-decided-after-trial-improves-error-estimation-accuracy.md) [+M]
- [Self Both Kr Decision Shows No Additive Motivational Informational Benefit](../claims/self-both-kr-decision-shows-no-additive-motivational-informational-benefit.md) [~M]
- [Self Controlled Learners Do Not Show Lower Error On Requested Kr Trials](../claims/self-controlled-learners-do-not-show-lower-error-on-requested-kr-trials.md) [~M]

## Related Theories
- [Self Determination Theory](self-determination-theory.md)

## Examples
-

## Key Sources
- Carter, M. J., Carlsen, A. N., & Ste-Marie, D. M. (2014). Self-controlled feedback is effective if it is based on the learner’s performance: a replication and extension of Chiviacowsky and Wulf (2005). Frontiers in Psychology, 5, 1325. https://doi.org/10.3389/fpsyg.2014.01325
