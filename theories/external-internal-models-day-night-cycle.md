---
type: theory
title: Modelos externos e internos (vivenciales) como dos representaciones complementarias del ciclo día-noche
description: "El artículo distingue dos representaciones del movimiento aparente del Sol: el modelo externo, en el que un observador fuera de la Tierra ve el planeta rotar con el Sol fijo, y el modelo interno o vivencial, en el que..."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: gangui-2009
    resource: "https://cms.iafe.uba.ar/gangui"
    title: "Gangui, Alejandro; Iglesias, María C.; Quinteros, Cynthia P. (2009). El movimiento de las sombras: Una propuesta de trabajo para la escuela secundaria. Ciencia Hoy. https://cms.iafe.uba.ar/gangui"
    author: Gangui, Alejandro; Iglesias, María C.; Quinteros, Cynthia P
---

# Modelos externos e internos (vivenciales) como dos representaciones complementarias del ciclo día-noche

> **Theory** · [All theories](index.md)

## Description
El artículo distingue dos representaciones del movimiento aparente del Sol: el modelo externo, en el que un observador fuera de la Tierra ve el planeta rotar con el Sol fijo, y el modelo interno o vivencial, en el que "la Tierra permanece inalterada y es el Sol el que aparentemente se mueve por el cielo". La secuencia guía a los alumnos en una transición lenta y reflexiva entre ambos modelos, usando la esfera de telgopor iluminada y luego la maqueta.

## Design Implications

### Context
#### Requirements
- Guía del docente para la transición entre representaciones
- esfera de telgopor con gnomon (escarbadiente) y fuente de luz
#### Constraints
- El modelo interno sigue siendo una buena representación del ciclo día-noche y de las estaciones siempre desde el punto de vista geocéntrico de la observación

### Target Learners
- estudiantes de escuela secundaria

### Target Learning Objectives
- ciclo día-noche
- marco de referencia del observador
- conceptos observacionales de la astronomía

### Claims
- 

## Related Theories
- 

## Examples
-

## Key Sources
- Gangui, Alejandro; Iglesias, María C.; Quinteros, Cynthia P. (2009). El movimiento de las sombras: Una propuesta de trabajo para la escuela secundaria. Ciencia Hoy. https://cms.iafe.uba.ar/gangui
