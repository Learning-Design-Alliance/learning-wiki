---
type: theory
title: "Krashen's Monitor Model: five hypotheses distinguishing subconscious acquisition from conscious language learning"
description: "The article presents Krashen's theory of second language acquisition as an explanatory framework built on five hypotheses: the Acquisition-Learning distinction, the Natural Order Hypothesis, the Monitor Hypothesis, th..."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: diaz-1988
    resource: "https://eric.ed.gov/?id=ED294203"
    title: "Diaz, Diana M. (1988). First Language/Second Language: Acquisition, Writing, and Cognitive Development. https://eric.ed.gov/?id=ED294203"
    author: Diaz, Diana M
---

# Krashen's Monitor Model: five hypotheses distinguishing subconscious acquisition from conscious language learning

> **Theory** · [All theories](index.md)

## Description
The article presents Krashen's theory of second language acquisition as an explanatory framework built on five hypotheses: the Acquisition-Learning distinction, the Natural Order Hypothesis, the Monitor Hypothesis, the Input Hypothesis, and the Affective Filter Hypothesis. The article states that "Krashen's theory (1983) of L2 acquisition, based on research from applied linguistics, consists of five hypotheses." Learning is conscious rule knowledge, while acquisition is a subconscious process driven by comprehensible input and shaped by motivation, self-confidence, and anxiety.

## Design Implications

### Context
#### Requirements
- Acquisition requires comprehensible input occurring in the context of natural language use, per the Input Hypothesis
#### Constraints
- The article notes current L2 methodologies focused on grammar, error correction, and product have been diverted from this informing research

### Target Learners
- second language (L2) students

### Target Learning Objectives
- second language acquisition
- writing ability development

### Claims
- 

## Related Theories
- 

## Examples
-

## Key Sources
- Diaz, Diana M. (1988). First Language/Second Language: Acquisition, Writing, and Cognitive Development. https://eric.ed.gov/?id=ED294203
