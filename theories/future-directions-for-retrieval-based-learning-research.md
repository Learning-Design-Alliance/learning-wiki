---
type: theory
title: Future Directions for Retrieval-Based Learning Research
description: "The chapter closes by \"highlighting four possible avenues\": deepening understanding of mechanisms and linking them to global models of memory; resolving how to balance retrieval success and effort; adopting a contextu..."
status: draft
generated:
  by: claude/unspecified
  at: 2026-09-24
sources:
  - id: karpicke-2017
    resource: "https://doi.org/10.1016/B978-0-12-809324-5.21055-9"
    title: "Karpicke, J. D. (2017). Retrieval-Based Learning: A Decade of Progress. Learning and Memory: A Comprehensive Reference, 2nd edition, Volume 2. https://doi.org/10.1016/B978-0-12-809324-5.21055-9"
    author: Karpicke, J. D
---

# Future Directions for Retrieval-Based Learning Research

> **Theory** · [All theories](index.md)
> **Evidence** · 2 claims (2 mixed) · 1 study, `q2` · 1 of 1 report an effect size · 2 claims rest on one study

## Description
The chapter closes by "highlighting four possible avenues": deepening understanding of mechanisms and linking them to global models of memory; resolving how to balance retrieval success and effort; adopting a contextual perspective that tests whether effects interact with learners, materials, and assessments; and integrating retrieval practice into educational activities beyond classroom quizzing.

## Design Implications

### Context
#### Requirements
- Manipulate features of materials or aspects of final assessments to test interactions rather than only generalizing effects.
#### Constraints
- The chapter states that generality across learner characteristics is established to a lesser extent than across materials and final assessments.

### Target Learners
- Researchers and educators working on retrieval practice

### Target Learning Objectives
- Identify mechanisms and boundary conditions of retrieval-based learning
- Develop new educational activities that incorporate retrieval

### Claims
- [Short Answer Versus Multiple Choice Retrieval Practice Evidence Is Mixed](../claims/short-answer-versus-multiple-choice-retrieval-practice-evidence-is-mixed.md) [~W]
- [Expanding Retrieval Schedules Are Not Superior To Equal Or Contracting Schedules](../claims/expanding-retrieval-schedules-are-not-superior-to-equal-or-contracting-schedules.md) [~W]

## Related Theories
- 

## Examples
-

## Key Sources
- Karpicke, J. D. (2017). Retrieval-Based Learning: A Decade of Progress. Learning and Memory: A Comprehensive Reference, 2nd edition, Volume 2. https://doi.org/10.1016/B978-0-12-809324-5.21055-9
