---
type: theory
title: Cognitive Model of L2 writing
description: The Cognitive Model treats L2 writing as a complex mental activity involving strategies and problem-solving skills, emphasizing the mental processes occurring during composing.
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-25
sources:
  - id: chmarkh-2025
    resource: "https://eric.ed.gov/?q=Chmarkh+contrastive+rhetoric+cognitive+sociocultural+L2+writing"
    title: "Chmarkh, M. (2025). Evaluating the Contrastive Rhetoric Model, the Cognitive Model, and the Sociocultural Model to L2 Writing. https://eric.ed.gov/?q=Chmarkh+contrastive+rhetoric+cognitive+sociocultural+L2+writing"
    author: Chmarkh, M
---

# Cognitive Model of L2 writing

> **Theory** · [All theories](index.md)
> **Evidence** · 1 claim (1 for) · 1 study, `q1` · 0 of 1 report an effect size · 1 claim rests on one study

## Description
The Cognitive Model treats L2 writing as a complex mental activity involving strategies and problem-solving skills, emphasizing the mental processes occurring during composing. The article notes it "underscores the importance of the interrelated nature of the environment, the long-term memory and the cognitive processes involved in writing", with topic, purpose and audience prompting strategies such as planning, generating ideas, editing and monitoring. It has been the predominant theoretical frame in L2 writing research.

## Design Implications

### Context
#### Requirements
- Attention to the writer's long-term memory, including topic knowledge and writing history, and to the writing environment of topic, purpose and audience
#### Constraints
- The article states L2 writers may not be able to verbalize their thinking processes and that the model does not address the social aspect of writing

### Target Learners
- Second language writers

### Target Learning Objectives
- Writing processes and composing strategies in a second language

### Claims
- [Cognitive Model Omits Social Aspect Of Writing](../claims/cognitive-model-omits-social-aspect-of-writing.md) [+M]

## Related Theories
- 

## Examples
-

## Key Sources
- Chmarkh, M. (2025). Evaluating the Contrastive Rhetoric Model, the Cognitive Model, and the Sociocultural Model to L2 Writing. https://eric.ed.gov/?q=Chmarkh+contrastive+rhetoric+cognitive+sociocultural+L2+writing
