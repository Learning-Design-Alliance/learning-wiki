---
type: theory
title: The Wiles-Bondi Curriculum Management Plan (CMP) for middle school change
description: The CMP is a management framework for implementing and sustaining middle school programs, drawing on Tyler and Taba and the accreditation process format.
status: draft
generated:
  by: claude/unspecified
  at: 2026-09-25
sources:
  - id: wiles-1986
    resource: "https://eric.ed.gov/?id=ED277147"
    title: "Wiles, Jon W.; Bondi, Joseph W. (1986). Making Middle Schools Work. Association for Supervision and Curriculum Development. https://eric.ed.gov/?id=ED277147"
    author: Wiles, Jon W.; Bondi, Joseph W
---

# The Wiles-Bondi Curriculum Management Plan (CMP) for middle school change

> **Theory** · [All theories](index.md)
> **Evidence** · no claims cited

## Description
The CMP is a management framework for implementing and sustaining middle school programs, drawing on Tyler and Taba and the accreditation process format. As the authors state, "the CMP attempts to introduce regularity into the process of change" by superimposing a management schemata over the curriculum development framework and forcing values clarification and commitment at each step. It rests on four key points: involving affected people, directing change from the top, basing decisions on hard data, and driving change by evaluation and accountability.

## Design Implications

### Context
#### Requirements
- A management team of district administrators with authority to guide curriculum development
- A committee structure enabling citizen and professional input
- A needs assessment informing the board of the current program's status
#### Constraints
- The authors state that conversion from a junior high design takes "some three to five years" in most districts
- The model assumes change must be directed from the top in a bureaucratic district environment

### Target Learners
- Preadolescent students ages 10-14 in middle grades

### Target Learning Objectives
- Balanced, comprehensive curriculum addressing physical, social, emotional, and intellectual development

### Claims
- 

## Related Theories
- 

## Examples
-

## Key Sources
- Wiles, Jon W.; Bondi, Joseph W. (1986). Making Middle Schools Work. Association for Supervision and Curriculum Development. https://eric.ed.gov/?id=ED277147
