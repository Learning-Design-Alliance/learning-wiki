---
type: theory
title: "The developmental interdependence hypothesis: L2 competence depends partly on L1 competence at the onset of intensive L2 exposure"
description: "The developmental interdependence hypothesis is the paper's central explanatory account of how first-language (L1) and second-language (L2) skills relate across development."
status: draft
generated:
  by: claude/unspecified
  at: 2026-09-25
sources:
  - id: cummins-1979
    resource: "https://eric.ed.gov/?id=ED257312"
    title: "Cummins, James. (1979). Linguistic Interdependence and the Educational Development of Bilingual Children. Bilingual Education Paper Series, Vol. 3 No. 2. https://eric.ed.gov/?id=ED257312"
    author: Cummins, James
---

# The developmental interdependence hypothesis: L2 competence depends partly on L1 competence at the onset of intensive L2 exposure

> **Theory** · [All theories](index.md)

## Description
The developmental interdependence hypothesis is the paper's central explanatory account of how first-language (L1) and second-language (L2) skills relate across development. It proposes that "the development of competence in a second language (L2) is partially a function of the type of competence already developed in Ll at the time when intensive exposure to L2 begins." When a child's L1 is well developed before intensive L2 exposure, high L2 competence can develop at no cost to L1; when L1 is less developed, early intensive L2 exposure impedes continued L1 growth, which in turn limits L2 development. The hypothesis underpins the paper's argument that instruction through the minority child's L1 supports, rather than hinders, L2 acquisition.

## Design Implications

### Context
#### Requirements
- The child's L1 must have developed certain functions (vocabulary-concept knowledge, literacy prerequisites) prior to intensive L2 exposure for transfer to support L2 learning
#### Constraints
- The hypothesis addresses functional interdependence between L1 and L2 skill development; the paper states it does not specify which school programs promote additive versus subtractive bilingualism under all conditions

### Target Learners
- Bilingual children, including minority-language and majority-language children in bilingual or immersion programs

### Target Learning Objectives
- Second-language competence
- First-language maintenance and development
- Academic achievement in bilingual education programs

### Claims
- [L1 Maintenance Supports L2 Without Cost](../claims/l1-maintenance-supports-l2-without-cost.md) [+M]
- [Mother Tongue Preservation Predicts L2 Achievement](../claims/mother-tongue-preservation-predicts-l2-achievement.md) [+M]

## Related Theories
- 

## Examples
-

## Key Sources
- Cummins, James. (1979). Linguistic Interdependence and the Educational Development of Bilingual Children. Bilingual Education Paper Series, Vol. 3 No. 2. https://eric.ed.gov/?id=ED257312. [doi:10.3102/00346543049002222](https://doi.org/10.3102/00346543049002222)
