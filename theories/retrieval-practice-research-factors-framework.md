---
type: theory
title: Retrieval Practice Experiment Architecture and Factors
description: "The chapter's organizing framework: learners study materials, a retrieval practice condition completes initial retrieval activities while a control does not, and all take a final criterial assessment."
status: draft
generated:
  by: claude/unspecified
  at: 2026-09-24
sources:
  - id: karpicke-2017
    resource: "https://doi.org/10.1016/B978-0-12-809324-5.21055-9"
    title: "Karpicke, J. D. (2017). Retrieval-Based Learning: A Decade of Progress. Learning and Memory: A Comprehensive Reference, 2nd edition, Volume 2. https://doi.org/10.1016/B978-0-12-809324-5.21055-9"
    author: Karpicke, J. D
---

# Retrieval Practice Experiment Architecture and Factors

> **Theory** · [All theories](index.md)

## Description
The chapter's organizing framework: learners study materials, a retrieval practice condition completes initial retrieval activities while a control does not, and all take a final criterial assessment. The factors explored include "aspects of the initial retrieval practice conditions, the conditions of the ﬁnal criterial assessment, characteristics of the learners, the nature of the materials, and the setting of the study", which structure the chapter's sections.

## Design Implications

### Context
#### Requirements
- A control condition, such as no additional activity, extra restudy, or an elaborative study activity.
- A final criterial assessment, given anywhere from a few minutes to several months after initial activities.
#### Constraints
- The chapter notes comparisons with restudy are biased toward restudy because retrieval conditions rarely reexpose all material.

### Target Learners
- Researchers studying retrieval-based learning

### Target Learning Objectives
- Design and interpret retrieval practice studies across initial conditions, final assessments, learners, materials, and settings

### Claims
- [Retrieval Practice Outperforms Repeated Study Rowland Meta Analysis](../claims/retrieval-practice-outperforms-repeated-study-rowland-meta-analysis.md) [+S]

## Related Theories
- 

## Examples
-

## Key Sources
- Karpicke, J. D. (2017). Retrieval-Based Learning: A Decade of Progress. Learning and Memory: A Comprehensive Reference, 2nd edition, Volume 2. https://doi.org/10.1016/B978-0-12-809324-5.21055-9
