---
type: theory
title: Dynamic assessment and the zone of proximal development as a framework for mediator-learner interaction
description: "The forum presents Poehner's (2014) account of dynamic assessment (DA) within Vygotsky's sociocultural theory."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: dean-2014
    resource: "https://eric.ed.gov/?id=EJ1176847"
    title: "Dean, A. C. (2014). The Interactional Dimension of LOA: Within and Beyond the Classroom. Teachers College, Columbia University Working Papers in TESOL & Applied Linguistics. https://eric.ed.gov/?id=EJ1176847"
    author: Dean, A. C
---

# Dynamic assessment and the zone of proximal development as a framework for mediator-learner interaction

> **Theory** · [All theories](index.md)

## Description
The forum presents Poehner's (2014) account of dynamic assessment (DA) within Vygotsky's sociocultural theory. DA aims to bridge the zone of proximal development, "the gap between what learners can achieve through mediation and what they can achieve independently." The mediator-learner dialectic presupposes joint engagement, and mediator involvement should push the student toward self-regulation while maintaining "a delicate balance of explicitness and scaffolding in the interaction."

## Design Implications

### Context
#### Requirements
- Mediator intervention must be calibrated so that its necessity decreases as students progress towards self-regulation
#### Constraints
- The mediator's involvement should not exceed its purpose, in the interest of promoting learner agency

### Target Learners
- second language learners

### Target Learning Objectives
- second language development through mediated interaction

### Claims
- 

## Related Theories
- 

## Examples
-

## Key Sources
- Dean, A. C. (2014). The Interactional Dimension of LOA: Within and Beyond the Classroom. Teachers College, Columbia University Working Papers in TESOL & Applied Linguistics. https://eric.ed.gov/?id=EJ1176847
