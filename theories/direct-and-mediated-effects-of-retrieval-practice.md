---
type: theory
title: Direct versus Mediated Effects of Retrieval Practice
description: A distinction the chapter uses to organize retrieval research.
status: draft
generated:
  by: claude/unspecified
  at: 2026-09-24
sources:
  - id: karpicke-2017
    resource: "https://doi.org/10.1016/B978-0-12-809324-5.21055-9"
    title: "Karpicke, J. D. (2017). Retrieval-Based Learning: A Decade of Progress. Learning and Memory: A Comprehensive Reference, 2nd edition, Volume 2. https://doi.org/10.1016/B978-0-12-809324-5.21055-9"
    author: Karpicke, J. D
---

# Direct versus Mediated Effects of Retrieval Practice

> **Theory** · [All theories](index.md)

## Description
A distinction the chapter uses to organize retrieval research. Mediated effects arise when a test guides later study, informs teachers, or motivates effort; in contrast, "Any gains in learning from practicing retrieval, without restudy or feedback, represent direct effects of retrieval processes on learning". The chapter credits the mediated-effects idea to Roediger and Karpicke (2006a).

## Design Implications

### Context
#### Requirements
- Assessing direct effects requires that learners practice retrieval without restudying or receiving feedback afterward.
#### Constraints
- The chapter states that feedback introduces mediated effects and prohibits assessing the direct mnemonic effects of retrieval, even though feedback is beneficial in educational settings.

### Target Learners
- Researchers designing retrieval practice experiments
- Educators using tests and quizzes

### Target Learning Objectives
- Distinguish learning caused by the act of retrieval from learning caused by feedback, restudy, or motivation

### Claims
- [Repeated Recall Without Feedback Beats Repeated Study At One Week But Not Five Minutes](../claims/repeated-recall-without-feedback-beats-repeated-study-at-one-week-but-not-five-minutes.md) [+W]

## Related Theories
- [Feedback Enhances Retrieval Practice](../claims/feedback-enhances-retrieval-practice.md)

## Examples
-

## Key Sources
- Karpicke, J. D. (2017). Retrieval-Based Learning: A Decade of Progress. Learning and Memory: A Comprehensive Reference, 2nd edition, Volume 2. https://doi.org/10.1016/B978-0-12-809324-5.21055-9
