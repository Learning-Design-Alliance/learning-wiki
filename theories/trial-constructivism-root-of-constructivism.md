---
type: theory
title: "Trial constructivism as the root of constructivism (Piaget's personal constructivism)"
description: "The article presents trial constructivism, attributed to von Glasersfeld's account of Piaget, as the origin of constructivism, also called personal constructivism."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-25
sources:
  - id: liu-2010
    resource: "https://www.cluteinstitute.com/ojs/index.php/cier"
    title: "Liu, C. C., & Chen, I. J. (2010). Evolution Of Constructivism. Contemporary Issues In Education Research, 3(4). https://www.cluteinstitute.com/ojs/index.php/cier"
    author: "Liu, C. C., & Chen, I. J"
---

# Trial constructivism as the root of constructivism (Piaget's personal constructivism)

> **Theory** · [All theories](index.md)
> **Evidence** · no claims cited

## Description
The article presents trial constructivism, attributed to von Glasersfeld's account of Piaget, as the origin of constructivism, also called personal constructivism. Its core is that the learner actively constructs knowledge rather than passively receiving it, contradicting transmission models. The authors note it leaves open how prior knowledge connects to the environment, calling these "the shortcomings of this rudimental of view."

## Design Implications

### Context
#### Requirements
- Learners must actively construct knowledge rather than passively receive it from the environment
#### Constraints
- The article states the rudimental view leaves unanswered how knowledge connects to prior knowledge and the environment

### Target Learners
- elementary mathematics learners (as in the Taiwan reform context)

### Target Learning Objectives
- active construction of knowledge rather than passive reception

### Claims
- 

## Related Theories

- [Piaget's model of a conceptual scaffolding of objects, space, time, and causality frames the construction of experiential reality](piaget-scaffolding-experiential-reality-construction.md)
- [Radical constructivism: knowing as dynamic adaptation of interpretations of experience](radical-constructivism-von-glasersfeld.md)
- [Piaget's genetic epistemology studies how the human mind moves from less sufficient to higher knowledge](piaget-genetic-epistemology-constructivist-knowing.md)
- [Vico's principle that we can rationally know only what we ourselves have made as an early formulation of constructivism](vico-constructivist-epistemology.md)
- [Stages of Cognitive Development](stages-of-cognitive-development.md)

## Examples
-

## Key Sources
- Liu, C. C., & Chen, I. J. (2010). Evolution Of Constructivism. Contemporary Issues In Education Research, 3(4). https://www.cluteinstitute.com/ojs/index.php/cier
