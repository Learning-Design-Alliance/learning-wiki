---
type: theory
title: "Transspection: imagining oneself in a role within a foreign culture, a step beyond empathy"
description: "Drawing on Lerner's account of empathy and Maruyama's concept of transspection, the essay distinguishes empathy, the capacity to imagine oneself in another role within one's own culture, from transspection, the capaci..."
status: draft
generated:
  by: claude/unspecified
  at: 2026-09-25
sources:
  - id: hanvey-1975
    resource: "https://eric.ed.gov/?id=ED116993"
    title: "Hanvey, Robert G. (1975). An Attainable Global Perspective. Center for War/Peace Studies. https://eric.ed.gov/?id=ED116993"
    author: Hanvey, Robert G
---

# Transspection: imagining oneself in a role within a foreign culture, a step beyond empathy

> **Theory** · [All theories](index.md)

## Description
Drawing on Lerner's account of empathy and Maruyama's concept of transspection, the essay distinguishes empathy, the capacity to imagine oneself in another role within one's own culture, from transspection, the capacity to imagine oneself in a role within the context of a foreign culture. Transspection is described as a trans-epistemological process in which a person temporarily believes what the other person believes, an understanding by practice. The essay charts a development from traditional through modern to postmodern psychic capacity and urges educators to provide students with maximum experience in transspection within a facilitating, reinforcing climate.

## Design Implications

### Context
#### Requirements
- A climate in which transspection is facilitated and expected and in which the expectations are reinforced, not merely allotted time
#### Constraints
- The essay holds the postmodern personality type is not likely to be produced by educational strategies alone, but may emerge from broad intensification of human interaction, with educators in minor supporting roles

### Target Learners
- Elementary and secondary students

### Target Learning Objectives
- Imagining viewpoints and roles within foreign cultures and seeing one's own culture from outside

### Claims
- 

## Related Theories
- 

## Examples
-

## Key Sources
- Hanvey, Robert G. (1975). An Attainable Global Perspective. Center for War/Peace Studies. https://eric.ed.gov/?id=ED116993
