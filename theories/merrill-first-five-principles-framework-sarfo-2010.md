---
type: theory
title: "Merrill's first five principles of instruction as the framework for quality teaching and learning"
description: "The article adopts Merrill's first principles of instruction as its organizing framework: \"1) task centeredness, 2) demonstration, 3) activation, 4) application, and 5) integration\"."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: sarfo-2010
    resource: "https://www.tojet.net"
    title: "Sarfo, F. K., & Ansong-Gyimah, K. (2010). The perceptions of students, teachers, and educational officers in Ghana on the role of computer and the teacher in promoting the first five principles of instruction. The Turkish Online Journal of Educational Technology, 9(3). https://www.tojet.net"
    author: "Sarfo, F. K., & Ansong-Gyimah, K"
---

# Merrill's first five principles of instruction as the framework for quality teaching and learning

> **Theory** · [All theories](index.md)

## Description
The article adopts Merrill's first principles of instruction as its organizing framework: "1) task centeredness, 2) demonstration, 3) activation, 4) application, and 5) integration". Each principle is elaborated: task centeredness engages learners in real-life problems; demonstration provides specific examples; activation stimulates relevant prior knowledge; application requires learners to use new knowledge to solve problems; integration encourages learners to incorporate new knowledge into everyday life. The authors use these principles to construct their 15-item perception scale and argue any quality instruction should meet all five.

## Design Implications

### Context
#### Requirements
- Any quality instruction should meet all the five principles, per Merrill as cited by the article
#### Constraints
- 

### Target Learners
- Senior High School students
- teachers
- education officers in Ghana

### Target Learning Objectives
- quality teaching and learning through effective instructional design

### Claims
- [Ghanaian Students Favour Computer Over Teacher First Principles](../claims/ghanaian-students-favour-computer-over-teacher-first-principles.md) [+M]

## Related Theories
- 

## Examples
-

## Key Sources
- Sarfo, F. K., & Ansong-Gyimah, K. (2010). The perceptions of students, teachers, and educational officers in Ghana on the role of computer and the teacher in promoting the first five principles of instruction. The Turkish Online Journal of Educational Technology, 9(3). https://www.tojet.net
