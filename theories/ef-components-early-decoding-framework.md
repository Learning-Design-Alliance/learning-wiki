---
type: theory
title: "Executive function components in early decoding: working memory, inhibitory control, and cognitive flexibility"
description: "The article frames early decoding as drawing on executive function, defined as \"cognitive resources to control attention to the print, recall the relevant phonological codes, ignore irrelevant codes, and flexibly shif..."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-25
sources:
  - id: vadasy-2023
    resource: "https://doi.org/10.1080/02702711.2023.2166636"
    title: "Vadasy, P. F., & Sanders, E. A. (2023). Cognitive flexibility + phonics intervention effects on reading gains. Reading Psychology. https://doi.org/10.1080/02702711.2023.2166636"
    author: "Vadasy, P. F., & Sanders, E. A"
---

# Executive function components in early decoding: working memory, inhibitory control, and cognitive flexibility

> **Theory** · [All theories](index.md)
> **Evidence** · 2 claims (1 for, 1 mixed) · 1 study, `q3` · 0 of 1 report an effect size · 2 claims rest on one study

## Description
The article frames early decoding as drawing on executive function, defined as "cognitive resources to control attention to the print, recall the relevant phonological codes, ignore irrelevant codes, and flexibly shift attention from one grapheme and sound to the next." It states that "Executive function (EF) comprises three primary and related cognitive processes: working memory, inhibitory control, and cognitive flexibility," each positively related to academic performance and still developing during the early school years. The study uses this framework to motivate embedding cognitive flexibility practice in phonics and to interpret why pretest flexibility predicted decoding gains.

## Design Implications

### Context
#### Requirements
- The framework assumes children are at the initial stage of decoding, when decoding is still effortful and draws heavily on executive functions before it becomes automatic.
#### Constraints
- The authors note challenges in measuring EF components in young children because individual components are less differentiated, and cite the task impurity problem.

### Target Learners
- Kindergarten children with low alphabet knowledge at the cusp of decoding

### Target Learning Objectives
- Grapheme-phoneme correspondences, decoding, and encoding of taught letters and words

### Claims
- [Pretest Cognitive Flexibility Predicts Decoding Gains Compensates Alphabet](../claims/pretest-cognitive-flexibility-predicts-decoding-gains-compensates-alphabet.md) [+M]
- [Embedded Cognitive Flexibility No Overall Advantage Winter Kindergarten](../claims/embedded-cognitive-flexibility-no-overall-advantage-winter-kindergarten.md) [~M]

## Related Theories

- [Five-dimension framework of preschool executive function with three core components advancing ages 3 to 5](preschool-ef-five-dimension-framework.md)

## Examples
-

## Key Sources
- Vadasy, P. F., & Sanders, E. A. (2023). Cognitive flexibility + phonics intervention effects on reading gains. Reading Psychology. https://doi.org/10.1080/02702711.2023.2166636
