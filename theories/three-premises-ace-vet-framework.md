---
type: theory
title: Three-premises framework for viewing ACE within the national VET system
description: "The report imposes order on wide-ranging views about ACE's role by working from three premises that form \"the lens or framework through whichwe have written this Report\": ACE is a provider; ACE providers are community..."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: schofield-1996
    resource: "https://eric.ed.gov/?id=ED420782"
    title: "Schofield, Kaye. (1996). Think Local and Compete. An Analysis of the Role of Adult and Community Education in the Implementation of a National System for Vocational Education and Training. A Report to the MCEETYA ACE Taskforce. Australian National Training Authority, Brisbane. https://eric.ed.gov/?id=ED420782"
    author: Schofield, Kaye
---

# Three-premises framework for viewing ACE within the national VET system

> **Theory** · [All theories](index.md)

## Description
The report imposes order on wide-ranging views about ACE's role by working from three premises that form "the lens or framework through whichwe have written this Report": ACE is a provider; ACE providers are community-based providers distinct from public and private providers; and the ACE sector is a network of community-based ACE providers. The framework's purpose is described as pragmatic and context specific, providing common boundaries around the analysis of ACE's role in the national VET system.

## Design Implications

### Context
#### Requirements
- The framework is explicitly pragmatic and context specific to the Australian national VET system of the mid-1990s
#### Constraints
- The report acknowledges it takes the existing objectives of the national VET system as given, running the risk of presenting the national VET system in its current form as immutable

### Target Learners
- adult learners in Australian community-based education settings

### Target Learning Objectives
- vocational education and training outcomes within nationally agreed frameworks

### Claims
- 

## Related Theories
- 

## Examples
-

## Key Sources
- Schofield, Kaye. (1996). Think Local and Compete. An Analysis of the Role of Adult and Community Education in the Implementation of a National System for Vocational Education and Training. A Report to the MCEETYA ACE Taskforce. Australian National Training Authority, Brisbane. https://eric.ed.gov/?id=ED420782
