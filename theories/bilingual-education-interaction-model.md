---
type: theory
title: "Model of bilingual education: outcomes as interaction of background, child input, and educational treatment factors"
description: "The paper integrates its two hypotheses into an overall model of bilingual education in which \"educational outcomes are explained as a function of the interaction between background, child input, and educational treat..."
status: draft
generated:
  by: claude/unspecified
  at: 2026-09-25
sources:
  - id: cummins-1979
    resource: "https://eric.ed.gov/?id=ED257312"
    title: "Cummins, James. (1979). Linguistic Interdependence and the Educational Development of Bilingual Children. Bilingual Education Paper Series, Vol. 3 No. 2. https://eric.ed.gov/?id=ED257312"
    author: Cummins, James
---

# Model of bilingual education: outcomes as interaction of background, child input, and educational treatment factors

> **Theory** · [All theories](index.md)

## Description
The paper integrates its two hypotheses into an overall model of bilingual education in which "educational outcomes are explained as a function of the interaction between background, child input, and educational treatment factors." Two child input factors are examined as intervening variables that interact with school program factors and mediate sociocultural background effects: conceptual-linguistic knowledge and motivation to learn L2 and maintain L1. The model assigns a central role to the interaction between sociocultural, linguistic, and school program factors, and is offered as the framework the paper finds missing from existing bilingual education research.

## Design Implications

### Context
#### Requirements
- Evaluations must incorporate the possibility of interaction between child input and educational treatment variables into their experimental designs for the model to be tested
#### Constraints
- The paper states the links in the causal chain from sociocultural background factors to academic outcomes, and the mechanisms of many school program variables, are not yet known

### Target Learners
- Minority-language and majority-language bilingual children

### Target Learning Objectives
- Academic achievement
- Cognitive development in bilingual education programs

### Claims
- [Bilingual Evaluations Ignore Interactions](../claims/bilingual-evaluations-ignore-interactions.md) [+M]

## Related Theories
- 

## Examples
-

## Key Sources
- Cummins, James. (1979). Linguistic Interdependence and the Educational Development of Bilingual Children. Bilingual Education Paper Series, Vol. 3 No. 2. https://eric.ed.gov/?id=ED257312. [doi:10.3102/00346543049002222](https://doi.org/10.3102/00346543049002222)
