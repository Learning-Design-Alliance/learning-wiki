---
type: theory
title: "ICAP theory: a taxonomy of four cognitive engagement modes with a hierarchical learning prediction"
description: "ICAP defines four modes of cognitive engagement — Interactive, Constructive, Active, and Passive — operationally differentiated by students' overt behaviors and products."
status: draft
generated:
  by: claude/unspecified
  at: 2026-09-25
sources:
  - id: chi-2018
    resource: "https://doi.org/10.1111/cogs.12626"
    title: "Chi, M. T. H., Adams, J., Bogusch, E. B., Bruchok, C., Kang, S., Lancaster, M., Levy, R., Li, N., McEldoon, K. L., Stump, G. S., Wylie, R., Xu, D., & Yaghmourian, D. L. (2018). Translating the ICAP Theory of Cognitive Engagement Into Practice. Cognitive Science, 42. https://doi.org/10.1111/cogs.12626"
    author: "Chi, M. T. H., Adams, J., Bogusch, E. B., Bruchok, C., Kang, S., Lancaster, M., Levy, R., Li, N., McEldoon, K. L., Stump, G. S., Wylie, R., Xu, D., & Yaghmourian, D. L"
---

# ICAP theory: a taxonomy of four cognitive engagement modes with a hierarchical learning prediction

> **Theory** · [All theories](index.md)
> **Evidence** · no claims cited

## Description
ICAP defines four modes of cognitive engagement — Interactive, Constructive, Active, and Passive — operationally differentiated by students' overt behaviors and products. The article states that "ICAP encompasses three components: a taxonomy of four engagement modes and the operational definition of each mode, a metric that can define the degree of engagement based on the cognitive processes corresponding to the four behavioral modes, and a hypothesis that can predict the hierarchical levels of student learning as a function of the mode of engagement." Each mode is mapped to underlying knowledge-change processes (storing, activating, linking, inferring), and the modes are hierarchically ordered so that Interactive subsumes Constructive, which subsumes Active, which subsumes Passive.

## Design Implications

### Context
#### Requirements
- Activities must be classifiable by comparing students' overt behaviors and products against the instructional materials presented
#### Constraints
- The taxonomy cannot predict which activities are better within the same mode; it only differentiates across modes
- The correspondence between overt behaviors and underlying cognitive processes is not perfect, only good enough by assumption

### Target Learners
- K-12 students
- college students
- teachers in professional development
- toddlers

### Target Learning Objectives
- Deeper learning through higher modes of cognitive engagement with instructional materials

### Claims
- 

## Related Theories
- [Active Learning](../principles/active-learning.md)

## Examples
-

## Key Sources
- Chi, M. T. H., Adams, J., Bogusch, E. B., Bruchok, C., Kang, S., Lancaster, M., Levy, R., Li, N., McEldoon, K. L., Stump, G. S., Wylie, R., Xu, D., & Yaghmourian, D. L. (2018). Translating the ICAP Theory of Cognitive Engagement Into Practice. Cognitive Science, 42. https://doi.org/10.1111/cogs.12626
