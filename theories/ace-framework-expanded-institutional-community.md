---
type: theory
title: Academic Communities of Engagement (ACE) framework, expanded to include an institutional/school community of support
description: The ACE framework explains how supports help online learners engage affectively, behaviorally, and cognitively.
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: graham-2023
    resource: "https://doi.org/10.24059/olj.v27i3.4001"
    title: "Graham, C., Borup, J., Tuiloma, S., Martínez Arias, A., Parra Caicedo, D., Larson, R. (2023). Institutional support for Academic Engagement in online and blended learning environments: Exploring affective, behavioral, and cognitive dimensions. Online Learning, 27(3), 4-40. https://doi.org/10.24059/olj.v27i3.4001"
    author: Graham, C., Borup, J., Tuiloma, S., Martínez Arias, A., Parra Caicedo, D., Larson, R
---

# Academic Communities of Engagement (ACE) framework, expanded to include an institutional/school community of support

> **Theory** · [All theories](index.md)
> **Evidence** · 2 claims (1 for, 1 against) · 1 study, `q3` · 1 of 1 report an effect size · 2 claims rest on one study

## Description
The ACE framework explains how supports help online learners engage affectively, behaviorally, and cognitively. It groups support actors into a course community and a personal community, hypothesizing that independent engagement "is limited and likely insufficient for academic success and requires support from their personal and course communities." This study expands it with a school/institutional community of support, arguing the distinction "provides a broader and more nuanced understanding of both the communities of support." It also aligns specific support elements (e.g., instructing and collaborating with cognitive engagement; facilitating communication and developing relationships with affective engagement) to engagement dimensions.

## Design Implications

### Context
#### Requirements
- Support actors within the course and personal communities whose relationships and support efforts can be distinguished and measured
#### Constraints
- The original framework focused on engagement at the course level rather than the institutional/school level; the study was conducted at a single university in Colombia

### Target Learners
- higher education students in online and blended courses

### Target Learning Objectives
- affective, behavioral, and cognitive engagement in online and blended learning

### Claims
- [Affective Support Predicts All Engagement Dimensions](../claims/affective-support-predicts-all-engagement-dimensions.md) [-M]
- [Overall Support Strongly Predicts Engagement](../claims/overall-support-strongly-predicts-engagement.md) [+M]

## Related Theories
- 

## Examples
-

## Key Sources
- Graham, C., Borup, J., Tuiloma, S., Martínez Arias, A., Parra Caicedo, D., Larson, R. (2023). Institutional support for Academic Engagement in online and blended learning environments: Exploring affective, behavioral, and cognitive dimensions. Online Learning, 27(3), 4-40. https://doi.org/10.24059/olj.v27i3.4001
