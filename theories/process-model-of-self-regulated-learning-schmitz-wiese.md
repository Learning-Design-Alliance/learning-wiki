---
type: theory
title: Process Model of Self-Regulated Learning (Schmitz and Wiese)
description: "The study is built on the process model of self-regulated learning by Schmitz and Wiese, an adaptation of Zimmerman's conception."
status: draft
generated:
  by: claude/unspecified
  at: 2026-09-24
sources:
  - id: bellhäuser-2022
    resource: "https://doi.org/10.3389/fpsyg.2022.813381"
    title: "Bellhäuser, H., Liborius, P., & Schmitz, B. (2022). Fostering Self-Regulated Learning in Online Environments: Positive Effects of a Web-Based Training With Peer Feedback on Learning Behavior. Frontiers in Psychology, 13, 813381. https://doi.org/10.3389/fpsyg.2022.813381"
    author: "Bellhäuser, H., Liborius, P., & Schmitz, B"
---

# Process Model of Self-Regulated Learning (Schmitz and Wiese)

> **Theory** · [All theories](index.md)
> **Evidence** · 1 claim (1 for) · 1 study, `q3` · 0 of 1 report an effect size · 1 claim rests on one study

## Description
The study is built on the process model of self-regulated learning by Schmitz and Wiese, an adaptation of Zimmerman's conception. "According to this model, learning is a process that can be divided into three phases: pre-action, action, and post-action." Phases recur cyclically across learning episodes through a feedback loop, and each phase needs different strategies; the web-based training devotes one lesson to each phase.

## Design Implications

### Context
#### Requirements
- Pre-action phase: learners set goals, deduce a plan and use self-motivation strategies where motivation is insufficient
- Action phase: cognitive, metacognitive and volitional strategies while working on the learning content
- Post-action phase: reflection comparing learning goals to actual achievement, which triggers the next pre-action phase
#### Constraints
- 

### Target Learners
- Prospective university students in mathematically oriented fields (computer science, civil engineering, mechanical engineering, mathematics) taking a voluntary online mathematics preparation course

### Target Learning Objectives
- Self-regulated learning across daily learning episodes: goal setting, planning, strategy use, volition and reflection

### Claims
- [Web Based Srl Training With Diary Raises Srl Knowledge And Self Efficacy Not Math](../claims/web-based-srl-training-with-diary-raises-srl-knowledge-and-self-efficacy-not-math.md) [+M]

## Related Theories
- [Self Regulated Learning](self-regulated-learning.md)

## Examples
-

## Key Sources
- Bellhäuser, H., Liborius, P., & Schmitz, B. (2022). Fostering Self-Regulated Learning in Online Environments: Positive Effects of a Web-Based Training With Peer Feedback on Learning Behavior. Frontiers in Psychology, 13, 813381. https://doi.org/10.3389/fpsyg.2022.813381
