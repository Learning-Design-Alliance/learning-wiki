---
type: element
id: cid-interdisciplinary-design-studio-library
title: CID interdisciplinary design studio in a university library
description: The Creativity, Innovation, and Design (CID) studio was a space created in a university library in 2014 to support interdisciplinary project-based learning, built quickly from surplus furniture, removed book rows, and...
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: warr-2020
    resource: "https://doi.org/10.14434/ijpbl.v14i1.28590"
    title: "Warr, M. & West, R. E. (2020). Bridging Academic Disciplines with Interdisciplinary Project-based Learning: Challenges and Opportunities. The Interdisciplinary Journal of Problem-based Learning. https://doi.org/10.14434/ijpbl.v14i1.28590"
    author: "Warr, M. & West, R. E"
---

# CID interdisciplinary design studio in a university library

> **Element** · [All elements](index.md)
> **Evidence** · no claims cited

## Description
The Creativity, Innovation, and Design (CID) studio was a space created in a university library in 2014 to support interdisciplinary project-based learning, built quickly from surplus furniture, removed book rows, and temporary walls with nearby group study rooms for teams. Over four semesters it hosted courses spanning business, education, engineering, fine arts, and other disciplines (196 participants tabulated), including multi-semester projects such as Fundación Paraguaya, DUST, and social innovation design. The studio "was to serve as a prototype for the kind of interdisciplinary collaboration that could exist if space and opportunity were provided."

## Design Implications

### Context
#### Requirements
- Faculty from many departments with training in design thinking pedagogy, supported by monthly CID faculty meetings as professional development
#### Constraints
- Implementation of studio pedagogy differed across courses; the disciplines of Winter Year 1 participants were not recorded

### Target Learners
- university students across disciplines

### Target Learning Goals
- creative problem-solving
- interdisciplinary collaboration
- design skills

### Affordances
- [Thick Authenticity Four Types Framework](../theories/thick-authenticity-four-types-framework.md)

## Related Elements
- 

## Examples
-

## Key Sources
- Warr, M. & West, R. E. (2020). Bridging Academic Disciplines with Interdisciplinary Project-based Learning: Challenges and Opportunities. The Interdisciplinary Journal of Problem-based Learning. https://doi.org/10.14434/ijpbl.v14i1.28590
