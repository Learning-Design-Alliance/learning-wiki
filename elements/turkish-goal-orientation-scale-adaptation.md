---
type: element
id: turkish-goal-orientation-scale-adaptation
title: "Turkish adaptation of Dweck's Goal Orientation Scale"
description: "This article adapts Dweck's four-item Goal Orientation Scale into Turkish, with two sub-dimensions: \"learning goal orientation\" and \"performance goal orientation\"."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: beyaztaş-2017
    resource: "https://doi.org/10.13189/ujer.2017.050909"
    title: "Beyaztaş, D. İ., Kaptı, S. B., & Hymer, B. (2017). The Relationship between Student Teachers' Perception of Intelligence and Their Goal Orientation. Universal Journal of Educational Research 5(9): 1519-1528. https://doi.org/10.13189/ujer.2017.050909"
    author: "Beyaztaş, D. İ., Kaptı, S. B., & Hymer, B"
---

# Turkish adaptation of Dweck's Goal Orientation Scale

> **Element** · [All elements](index.md)
> **Evidence** · no claims cited

## Description
This article adapts Dweck's four-item Goal Orientation Scale into Turkish, with two sub-dimensions: "learning goal orientation" and "performance goal orientation". Adaptation followed Hambleton and Patsula's steps: four independent translations revised via the Delphi technique, back-translation, expert grammar review, and permission from Dweck. The fourth item was converted to a 6-point Likert format, yielding Cronbach's Alpha of .81 for learning orientation and .61 for performance orientation, with mean inter-item correlations of 0,31 and 0,38, and CFA fit indices of RMSEA= 0.00, CFI= 1 and χ2/sd=0.26.

## Design Implications

### Context
#### Requirements
- Used with permission from Dweck and following recommended scale-adaptation steps including translation, back-translation and expert review
#### Constraints
- The performance orientation subscale's Cronbach's Alpha (.61) fell below the .7 threshold the article cites, so reliability for that dimension rests on mean inter-item correlation

### Target Learners
- university student teachers in Turkey

### Target Learning Goals
- measuring achievement goal orientation (learning vs performance)

## Related Elements
- 

## Examples

- [Introduce student teachers to mindset theory early and foster a learning orientation through classroom language, feedback and attitude to challenge](../strategies/mindset-theory-early-in-teacher-education.md)

## Key Sources
- Beyaztaş, D. İ., Kaptı, S. B., & Hymer, B. (2017). The Relationship between Student Teachers' Perception of Intelligence and Their Goal Orientation. Universal Journal of Educational Research 5(9): 1519-1528. https://doi.org/10.13189/ujer.2017.050909
