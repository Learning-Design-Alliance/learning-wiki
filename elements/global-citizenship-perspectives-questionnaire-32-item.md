---
type: element
id: global-citizenship-perspectives-questionnaire-32-item
title: 32-item global citizenship perspectives questionnaire adapted from Morais and Ogden (2011) and Roberts and Wilson (2016)
description: "A questionnaire 'adapted from the validated questionnaire from Morais and Ogden (2011) and Roberts and Wilson (2016)' consisting of 32 items with modifications such as country names and political policies."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: nguyen-2021
    resource: "https://doi.org/10.31014/aior.1993.04.03.362"
    title: "Nguyen, M. T. (2021). Validating a Scale for Measuring Students' Perspectives toward Global Citizenship: A Case at a Private University in Vietnam. Education Quarterly Reviews, 4(3), 575-587. https://doi.org/10.31014/aior.1993.04.03.362"
    author: Nguyen, M. T
---

# 32-item global citizenship perspectives questionnaire adapted from Morais and Ogden (2011) and Roberts and Wilson (2016)

> **Element** · [All elements](index.md)
> **Evidence** · no claims cited

## Description
A questionnaire 'adapted from the validated questionnaire from Morais and Ogden (2011) and Roberts and Wilson (2016)' consisting of 32 items with modifications such as country names and political policies. Items cover demographic features, social responsibility (GJD, AE, GIPR), global competence (SA, IC, GK), and global civic engagement (ICO, PV, GCA), all rated on a 5-point Likert scale from 1 (totally disagree) to 5 (totally agree).

## Design Implications

### Context
#### Requirements
- Pilot testing before distribution to allow adjustments; PLS-SEM analysis for validation
#### Constraints
- Some items required modification for the Vietnamese context, and several indicators were removed during validation

### Target Learners
- higher education students

### Target Learning Goals
- measuring student perspectives toward global citizenship

## Related Elements
- 

## Examples
-

## Key Sources
- Nguyen, M. T. (2021). Validating a Scale for Measuring Students' Perspectives toward Global Citizenship: A Case at a Private University in Vietnam. Education Quarterly Reviews, 4(3), 575-587. https://doi.org/10.31014/aior.1993.04.03.362
