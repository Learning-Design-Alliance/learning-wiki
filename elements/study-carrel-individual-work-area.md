---
type: element
id: study-carrel-individual-work-area
title: Study carrel as an individual work area shielding a child from disturbance
description: "The booklet defines and details the study carrel as a behaviour-support fixture: \"A carrel is a work area for individual study."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-25
sources:
  - id: special-education-facilities-for-emotionally-disturbed-children-1968
    resource: "https://eric.ed.gov/?id=ED085907"
    title: "Special Education Facilities for Emotionally Disturbed Children. (1968). Ontario Department of Education. https://eric.ed.gov/?id=ED085907"
    author: Special Education Facilities for Emotionally Disturbed Children
---

# Study carrel as an individual work area shielding a child from disturbance

> **Element** · [All elements](index.md)
> **Evidence** · no claims cited

## Description
The booklet defines and details the study carrel as a behaviour-support fixture: "A carrel is a work area for individual study. Itis intended as a place where a child may work alone, relatively undisturbed by other children." Drawing 10 shows a folding-panel type with chalkboard and tackboard on each side that folds against the wall when unused, and two permanent types with built-in desk top, shelving, chalkboard, tackboard, and a light. Carrels need electrical outlets for teaching machines and warm-white fluorescent lighting with diffusers.

## Design Implications

### Context
#### Requirements
- Each carrel should have a light and electrical outlets for teaching machines
#### Constraints
- In mild-disorder classrooms there is less need for carrels; folding panels allow removal when not in use

### Target Learners
- emotionally disturbed children needing individual study space

### Target Learning Goals
- sustained individual work relatively undisturbed by other children

## Related Elements
- 

## Examples
-

## Key Sources
- Special Education Facilities for Emotionally Disturbed Children. (1968). Ontario Department of Education. https://eric.ed.gov/?id=ED085907
