---
type: element
id: evidence-synthesis-education-scoping-review-protocol
title: Registered scoping review protocol on evidence synthesis education for pharmacy students
description: "A protocol for a scoping review registered in the Open Science Framework (registration DOI:10.17605/OSF.IO/ZQVGY), designed per PRISMA-P and to be reported per PRISMA-ScR."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-25
sources:
  - id: kapu-h-2026
    resource: "https://doi.org/10.1136/bmjopen-2026-119283"
    title: "Kapu H, Shetty Veekshitha S, Rajendran Rajalakshmi, Sekhar M Sonal, Kunhikatta Vijayanarayana, Unnikrishnan B, Khan Sohil, Gunning Tiffany K, Athan Eugene, Saha Sajal K, Thunga Girish. (2026). Exploring educational approaches and strategies for developing evidence synthesis skills among pharmacy students: a scoping review protocol. BMJ Open. https://doi.org/10.1136/bmjopen-2026-119283"
    author: Kapu H, Shetty Veekshitha S, Rajendran Rajalakshmi, Sekhar M Sonal, Kunhikatta Vijayanarayana, Unnikrishnan B, Khan Sohil, Gunning Tiffany K, Athan Eugene, Saha Sajal K, Thunga Girish
---

# Registered scoping review protocol on evidence synthesis education for pharmacy students

> **Element** · [All elements](index.md)
> **Evidence** · no claims cited

## Description
A protocol for a scoping review registered in the Open Science Framework (registration DOI:10.17605/OSF.IO/ZQVGY), designed per PRISMA-P and to be reported per PRISMA-ScR. It will search PubMed, Scopus, Embase, CINAHL Ultimate and Web of Science from 2010, screen in Covidence with two independent reviewers and Cohen's Kappa, extract data on training programme, delivery method, duration and outcome assessment, and compile results with descriptive statistics and narrative synthesis.

## Design Implications

### Context
#### Requirements
- Two reviewers screen and extract independently, with a third reviewer consulted to resolve disagreements; inter-rater agreement for pilot extraction uses Cohen's Kappa with a coefficient (k) value >0.70 considered strong
#### Constraints
- The scoping review has not yet commenced; formal screening is anticipated to begin in June 2026
- Included studies were not subjected to critical appraisal, given that the review's purpose is to map and explore available evidence rather than assess methodological quality

### Target Learners
- pharmacy students

### Target Learning Goals
- evidence synthesis skills development

## Related Elements
- 

## Examples

- [Map educational approaches onto predefined evidence synthesis skill components, delivery modes and outcome-measure categories when charting the literature](../strategies/chart-approaches-by-skill-component-delivery-and-outcome-type.md)

## Key Sources
- Kapu H, Shetty Veekshitha S, Rajendran Rajalakshmi, Sekhar M Sonal, Kunhikatta Vijayanarayana, Unnikrishnan B, Khan Sohil, Gunning Tiffany K, Athan Eugene, Saha Sajal K, Thunga Girish. (2026). Exploring educational approaches and strategies for developing evidence synthesis skills among pharmacy students: a scoping review protocol. BMJ Open. https://doi.org/10.1136/bmjopen-2026-119283
