---
type: element
title: Justification
description: Justification is the element in which learners explain why a claim, choice, or solution is warranted using reasons, criteria, or evidence.
status: review
generated:
  by: codex/unspecified
  at: 2026-04-08
---

# Justification

## Description
Justification is the element in which learners explain why a claim, choice, or solution is warranted using reasons, criteria, or evidence. It is useful when the instructional goal is to strengthen explanation quality rather than only reach an answer.

## Design Implications

### Context
#### Requirements
- **A claim, decision, or solution to defend**
- **Criteria or evidence that can support the defense**
#### Constraints
- **Learners may state conclusions without explaining the warrant unless prompted**

### Target Learning Goals
- Improve reasoning quality, evidence use, and explanation of decisions.

### Affordances
- [Epistemic Cognition](../principles/epistemic-cognition.md)
- [Explaining Their Thinking](../principles/explaining-their-thinking.md)

## Related Elements
- [Justification & Argumentation](justification-argumentation.md)
- [Argumentation](argumentation.md)
- [Evidence-Based Reasoning](evidence-based-reasoning.md)

## Patterns That Use This Element
- [MEL Reasoning Pattern](../patterns/mel-reasoning-pattern.md)
- [Discussion-Based Learning](../patterns/discussion-based-learning.md)

## Examples
- Learners explain why a solution method fits the problem better than another.

## Key Sources
- Toulmin, S. (2003). *The uses of argument* (Updated ed.). Cambridge University Press.
