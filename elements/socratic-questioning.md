---
type: element
title: Socratic Questioning
description: Socratic questioning is the element in which prompts are used to probe assumptions, clarify reasoning, test evidence, and extend implications.
status: review
generated:
  by: codex/unspecified
  at: 2026-04-07
sources:
  - id: chin-2007
    resource: "https://doi.org/10.1002/tea.20171"
    title: "Chin, C. (2007). Teacher questioning in science classrooms: Approaches that stimulate productive thinking. *Journal of Research in Science Teaching, 44*(6), 815-843"
    author: Chin, C
---

# Socratic Questioning

## Description
Socratic questioning is the element in which prompts are used to probe assumptions, clarify reasoning, test evidence, and extend implications. It is useful when the goal is not simply to elicit an answer, but to strengthen the quality of explanation and judgment behind the answer.

## Design Implications

### Context
#### Requirements
- **A discussable claim or interpretation**
- **Questions aimed at reasoning, evidence, and implication**
- **Time for learners to respond and reconsider**
#### Constraints
- **Interrogation feel**: Poorly paced questioning can feel punitive rather than exploratory.
- **Surface participation**: Rapid-fire questioning can privilege quick responders over thoughtful ones.

### Target Learners
- Learners developing explanation, argumentation, and conceptual precision.

### Target Learning Goals
- Clarify assumptions, test evidence, and deepen reasoning.

### Affordances
- [Explaining Their Thinking](../principles/explaining-their-thinking.md)
- [Social Learning](../principles/social-learning.md)
- [Constructivism](../principles/constructivism.md)

## Related Elements
- [Conceptual Questioning](conceptual-questioning.md)
- [Class Discussion](class-discussion.md)
- [Peer Discussion](peer-discussion.md)

## Patterns That Use This Element
- [Socratic Seminar](../patterns/socratic-seminar.md)
- [Discussion-Based Learning](../patterns/discussion-based-learning.md)

## Examples
- Asking learners what evidence supports a claim and what would count against it.
- Probing a proposed solution with follow-up questions about assumptions and consequences.

## Key Sources
- Paul, R., & Elder, L. (2006). *The art of Socratic questioning*. Foundation for Critical Thinking.
- Chin, C. (2007). Teacher questioning in science classrooms: Approaches that stimulate productive thinking. *Journal of Research in Science Teaching, 44*(6), 815-843. [https://doi.org/10.1002/tea.20171](https://doi.org/10.1002/tea.20171)
