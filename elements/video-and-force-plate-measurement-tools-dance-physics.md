---
type: element
id: video-and-force-plate-measurement-tools-dance-physics
title: Digital video and Vernier force plates as low-setup measurement tools for dance motion
description: "The course uses two measurement tools: digital video cameras, from which position vs."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-25
sources:
  - id: richard-p-barber-2006
    resource: "https://scholar.google.com"
    title: "Richard P. Barber, Jr., David J. Popalisky, Rose Hacking and Kristina Chiapella. (2006). Newton's 2nd Law and the Physics of Dance. https://scholar.google.com"
    author: Richard P. Barber, Jr., David J. Popalisky, Rose Hacking and Kristina Chiapella
---

# Digital video and Vernier force plates as low-setup measurement tools for dance motion

> **Element** · [All elements](index.md)
> **Evidence** · no claims cited

## Description
The course uses two measurement tools: digital video cameras, from which position vs. time data are extracted by hand against a calibrated background, and "force plates from Vernier Software & Technology that are similar to bathroom scales but read force vs. time at 50 readings/second", recorded with a handheld interface. Two plates, one under each foot, provide a safer jumping target and require only adding two data columns.

## Design Implications

### Context
#### Requirements
- Calibrated background or meter stick in the video field-of-view
- zeroing the force plates before each run
- Vernier interface and LoggerPro software
#### Constraints
- Video data are extracted by hand frame-by-frame using a ruler on a computer screen

### Target Learners
- non-science-major undergraduates

### Target Learning Goals
- measuring force and motion
- data analysis skills

## Related Elements
- 

## Examples
-

## Key Sources
- Richard P. Barber, Jr., David J. Popalisky, Rose Hacking and Kristina Chiapella. (2006). Newton's 2nd Law and the Physics of Dance. https://scholar.google.com
