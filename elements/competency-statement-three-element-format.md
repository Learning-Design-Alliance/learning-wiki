---
type: element
id: competency-statement-three-element-format
title: "Competency statement format: behavior, conditions, and minimal performance level"
description: "A competency statement is the outcome objective for staff development: it specifies what educational personnel must know, do, or value to implement the educational improvement program."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: huenecke-1975
    resource: "https://eric.ed.gov/?id=ED123751"
    title: "Huenecke, Dorothy M.; Stansbury, George W. (1975). A Systems Approach to Staff Development. Georgia State Dept. of Education. https://eric.ed.gov/?id=ED123751"
    author: Huenecke, Dorothy M.; Stansbury, George W
---

# Competency statement format: behavior, conditions, and minimal performance level

> **Element** · [All elements](index.md)
> **Evidence** · no claims cited

## Description
A competency statement is the outcome objective for staff development: it specifies what educational personnel must know, do, or value to implement the educational improvement program. The workbook teaches two ways of defining competency statements — adopting existing statements from published lists, or writing original ones — and specifies that "Each competency statement should have the minimal following element": a statement of what is to occur, a statement of the conditions under which it is to occur, and a statement of minimal performance level. Competencies are educational-personnel-centered, in contrast to student-centered objectives.

## Design Implications

### Context
#### Requirements
- An educational improvement program already designed, from which needed staff competencies are derived
- an assessment of which competencies the staff already possess
#### Constraints
- Competencies needed but found lacking form the basis of the staff development program; inadequate assessment of professional competencies can be as hazardous as no assessment at all

### Target Learners
- educational personnel and staff development planners

### Target Learning Goals
- specify professional competencies needed to achieve student objectives

## Related Elements
- 

## Examples
-

## Key Sources
- Huenecke, Dorothy M.; Stansbury, George W. (1975). A Systems Approach to Staff Development. Georgia State Dept. of Education. https://eric.ed.gov/?id=ED123751
