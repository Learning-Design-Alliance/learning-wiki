---
type: element
id: voicethread-interpersonal-audio-discussions
title: VoiceThread interpersonal audio discussion activities for L2 pronunciation practice
description: VoiceThread is a web-based multimedia tool used in this study to deliver three collaborative interpersonal audio discussion activities in introductory French.
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: lepore-2014
    resource: "http://www.voicethread.com"
    title: "Lepore, C. E. (2014). Influencing Students' Pronunciation and Willingness to Communicate through Interpersonal Audio Discussions. Dimension 2014. http://www.voicethread.com"
    author: Lepore, C. E
---

# VoiceThread interpersonal audio discussion activities for L2 pronunciation practice

> **Element** · [All elements](index.md)

## Description
VoiceThread is a web-based multimedia tool used in this study to deliver three collaborative interpersonal audio discussion activities in introductory French. Creators upload content as images, text, or video that operates as a point of departure for asynchronous discussions where users add their own content or comment on the existing conversation. Activities followed three phases (brainstorming, initial participation, interaction with classmates) and were paired with instructor pronunciation feedback and student self-assessment forms. The tool was selected for its multimedia criteria and its ability to accommodate communicatively based activities.

## Design Implications

### Context
#### Requirements
- Access to VoiceThread accounts via web browser or mobile devices, instructor guidance on technical matters, and activity guidelines with checklists listing required tasks before each deadline
#### Constraints
- The article states the practicality and effectiveness of using interpersonal audio discussions as casual supplements without feedback or assessment is uncertain

### Target Learners
- introductory second-language learners (second-semester university French students)

### Target Learning Goals
- L2 pronunciation development
- willingness to communicate
- spoken interaction in the L2

## Related Elements
- 

## Examples
-

## Key Sources
- Lepore, C. E. (2014). Influencing Students' Pronunciation and Willingness to Communicate through Interpersonal Audio Discussions. Dimension 2014. http://www.voicethread.com
