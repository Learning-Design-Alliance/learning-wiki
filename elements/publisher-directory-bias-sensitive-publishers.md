---
type: element
id: publisher-directory-bias-sensitive-publishers
title: Publisher Directory listing publishers sensitive to treatment of minorities and females
description: "Part III's Publisher Directory (Section 11) opens with the Association of American Publishers' \"Statement on Bias-free Materials\" reproduced in its entirety, then presents an alphabetical listing of publishers of book..."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: alspektor-1979
    resource: "https://eric.ed.gov/?id=ED020373"
    title: "Alspektor, Rose Ann, and Wirtenberg, Jeana. (1979). Fair Textbooks: A Resource Guide. U.S. Commission on Civil Rights, Clearinghouse Publication 61. https://eric.ed.gov/?id=ED020373"
    author: Alspektor, Rose Ann, and Wirtenberg, Jeana
---

# Publisher Directory listing publishers sensitive to treatment of minorities and females

> **Element** · [All elements](index.md)
> **Evidence** · no claims cited

## Description
Part III's Publisher Directory (Section 11) opens with the Association of American Publishers' "Statement on Bias-free Materials" reproduced in its entirety, then presents an alphabetical listing of publishers of books and other educational materials that, in the estimation of the publishers themselves, contribute favorably to or are particularly sensitive to the treatment of minorities and females. It also includes publishers who have developed or are using guidelines in an attempt to help administrators avoid textbook bias. Each publisher entry lists titles of catalogs, guidelines, and other resources the publisher provided, coded by group, grade level, and subject area.

## Design Implications

### Context
#### Requirements
- Publisher listings reflect publishers' own estimations; each resource is coded for the group(s), grade level(s), and subject area(s) for which it applies
#### Constraints
- The directory is self-reported by publishers, and inclusion in the guide is not an endorsement by the Commission

### Target Learners
- Educators, administrators, and textbook selection committees seeking bias-free materials

### Target Learning Goals
- Identifying publishers and materials sensitive to the treatment of minorities and females

## Related Elements

- [Fair Textbooks: A Resource Guide, a centralized catalog of bias-free education resources](fair-textbooks-resource-guide-catalog.md)

## Examples
-

## Key Sources
- Alspektor, Rose Ann, and Wirtenberg, Jeana. (1979). Fair Textbooks: A Resource Guide. U.S. Commission on Civil Rights, Clearinghouse Publication 61. https://eric.ed.gov/?id=ED020373
