---
type: element
title: Pre-Reading Activities
description: Pre-reading activities are tasks completed before reading in order to activate prior knowledge, establish purpose, surface key vocabulary, or prepare attention for important ideas.
status: review
generated:
  by: codex/unspecified
  at: 2026-04-08
---

# Pre-Reading Activities

## Description
Pre-reading activities are tasks completed before reading in order to activate prior knowledge, establish purpose, surface key vocabulary, or prepare attention for important ideas. They are useful when learners need orientation before engaging the text itself.

## Design Implications

### Context
#### Requirements
- **A specific purpose tied to the upcoming text**
- **Prompts that prepare attention rather than duplicate the reading**
#### Constraints
- **Overlong pre-reading can delay the actual text unnecessarily**

### Target Learning Goals
- Prepare comprehension by activating knowledge and setting expectations.

### Affordances
- [Activation](/principles/activation.md)
- [Retrieval Practice](/principles/retrieval-practice.md)
- [Pre-Reading Questioning](/principles/pre-reading-questioning.md)

## Related Elements
- [Prior Knowledge Activation](/elements/prior-knowledge-activation.md)
- [Assigned Readings](/elements/assigned-readings.md)
- [Discussion Prompt](/elements/discussion-prompt.md)

## Patterns That Use This Element
- [Flipped Learning](/patterns/flipped-learning.md)
- [Discussion-Based Learning](/patterns/discussion-based-learning.md)

## Examples
- Prediction prompts, quick recalls, vocabulary previews, and question generation before reading.

## Key Sources
- Graves, M. F., Juel, C., & Graves, B. B. (2007). *Teaching reading in the 21st century*. Allyn & Bacon.
