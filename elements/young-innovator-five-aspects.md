---
type: element
id: young-innovator-five-aspects
title: "Young innovator attribute framework: five synthesized aspects"
description: The article synthesizes desired characteristics of young innovators from seven prior sources (Jeffrey et al., 2009; Wongtienlai et al., 2019; Tonghom et al., 2017; National Innovation Agency, 2022; Phatrawat, 2020; Ts...
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-25
sources:
  - id: sapliyan-2023
    resource: "https://doi.org/10.5539/jel.v12n4p81"
    title: "Sapliyan, S., Chatwattana, P., & Nilsook, P. (2023). Constructionism Imagineering Learning Model via Metaverse to Enhance Young Innovators. Journal of Education and Learning, 12(4). https://doi.org/10.5539/jel.v12n4p81"
    author: "Sapliyan, S., Chatwattana, P., & Nilsook, P"
---

# Young innovator attribute framework: five synthesized aspects

> **Element** · [All elements](index.md)

## Description
The article synthesizes desired characteristics of young innovators from seven prior sources (Jeffrey et al., 2009; Wongtienlai et al., 2019; Tonghom et al., 2017; National Innovation Agency, 2022; Phatrawat, 2020; Tsou et al., 2014; Yams, 2017) into five aspects. As printed in Table 1, "young innovators consist of five aspects as follows. Linking problems by learners using problems as a catalyst for problem-solving; questioning or defining a hypothesis by allowing learners to ask a series of questions systematically; observing details or collecting insights data on new methods to predict the possibility of innovation; experiment, daring to make decisions, daring trial and error; and create networking, expanding innovation to social media." This framework serves as the output element of the developed model.

## Design Implications

### Context
#### Requirements
- Serves as an assessment target: a young innovator assessment form is used in the summarise and post-test stage
#### Constraints
- The synthesis is based on document analysis of prior sources rather than empirical testing of the attribute framework itself

### Target Learners
- children and young people being developed as innovators

### Target Learning Goals
- innovator attributes: linking problems, questioning, observing, experimenting, creating networks

### Affordances
- [Constructionism Imagineering Learning Model Metaverse](../theories/constructionism-imagineering-learning-model-metaverse.md)

## Related Elements
- 

## Examples
-

## Key Sources
- Sapliyan, S., Chatwattana, P., & Nilsook, P. (2023). Constructionism Imagineering Learning Model via Metaverse to Enhance Young Innovators. Journal of Education and Learning, 12(4). https://doi.org/10.5539/jel.v12n4p81
