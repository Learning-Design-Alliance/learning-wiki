---
type: element
id: alternate-portfolio-assessment-multidomain-rubric
title: Standards-based alternate portfolio assessment with multi-domain scoring rubric
description: An alternate assessment in which student portfolios—a purposeful and systematic collection of student work evaluated against predetermined scoring criteria—document progress toward the same content standards as all st...
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: olson-2002
    resource: "https://eric.ed.gov/?id=ED472306"
    title: "Olson, B., Mead, R., & Payne, D. (2002). A report of a standard setting method for alternate assessments for students with significant disabilities (Synthesis Report 47). https://eric.ed.gov/?id=ED472306"
    author: "Olson, B., Mead, R., & Payne, D"
---

# Standards-based alternate portfolio assessment with multi-domain scoring rubric

> **Element** · [All elements](index.md)

## Description
An alternate assessment in which student portfolios—a purposeful and systematic collection of student work evaluated against predetermined scoring criteria—document progress toward the same content standards as all students, using alternate learning expectations. Entries span audio and videotapes, photographs, checklists, interviews, surveys, rating scales, and existing records. A focused, holistic, domain scoring rubric scored each entry in performance, appropriateness, and level of assistance domains, with a settings domain scored once per subject area.

## Design Implications

### Context
#### Requirements
- A pre-defined portfolio structure specifying the number and type of entries
- A stakeholder-developed scoring rubric and trained, qualified readers
#### Constraints
- Non-scoreable entries were common, especially at the lower end of the score range

### Target Learners
- Students with significant disabilities in grades 4, 6, 8, and 11

### Target Learning Goals
- Progress toward state content standards in literacy and mathematics

### Affordances
- [Body Of Work Standard Setting Approach](../theories/body-of-work-standard-setting-approach.md)

## Related Elements
- 

## Examples
-

## Key Sources
- Olson, B., Mead, R., & Payne, D. (2002). A report of a standard setting method for alternate assessments for students with significant disabilities (Synthesis Report 47). https://eric.ed.gov/?id=ED472306
