---
type: element
id: grasha-riechmann-inventory-turkish-adaptation
title: "Grasha-Riechmann Learning Style Inventory (Turkish adaptation) as the study's learning style measure"
description: The study used the Grasha-Riechmann Learning Style Inventory, a 60-item five-point Likert scale adapted by Sarıtaş and Süral (2010) with six dimensions of ten items each, with a printed Cronbach Alpha of .802.
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: balta-2018
    resource: "https://doi.org/10.5430/ijhe.v7n6p106"
    title: "Balta, E. E. (2018). Reflective Thinking Tendencies and Epistemological Beliefs in Terms of Learning Styles. International Journal of Higher Education, 7(6). https://doi.org/10.5430/ijhe.v7n6p106"
    author: Balta, E. E
---

# Grasha-Riechmann Learning Style Inventory (Turkish adaptation) as the study's learning style measure

> **Element** · [All elements](index.md)

## Description
The study used the Grasha-Riechmann Learning Style Inventory, a 60-item five-point Likert scale adapted by Sarıtaş and Süral (2010) with six dimensions of ten items each, with a printed Cronbach Alpha of .802. The article describes it as "a social interaction scale" positioning styles as independent-dependent, avoidant-participant, and collaborative-competitive on three major lines, each representing two opposite directions. Styles are scored at low, medium, and high levels using printed cut-off ranges (Table 2).

## Design Implications

### Context
#### Requirements
- Administered as a 60-item five-point Likert instrument; each learning style is determined at low, medium, and high levels via the printed degree ranges
#### Constraints
- The article reports dimension reliabilities only for the other scales; the inventory's printed alpha of .802 is the overall coefficient

### Target Learners
- preservice teachers in higher education

### Target Learning Goals
- identifying individual differences in how learners interact with teachers, classmates, and lesson content

## Related Elements
- 

## Examples
-

## Key Sources
- Balta, E. E. (2018). Reflective Thinking Tendencies and Epistemological Beliefs in Terms of Learning Styles. International Journal of Higher Education, 7(6). https://doi.org/10.5430/ijhe.v7n6p106
