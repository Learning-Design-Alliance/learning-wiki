---
type: element
title: Seminar Format
description: Seminar format is the element in which learning is organized around sustained discussion of a shared text, question, or issue with facilitator support but strong learner participation.
status: review
generated:
  by: codex/unspecified
  at: 2026-04-08
---

# Seminar Format

> **Element** · [All elements](index.md)

## Description
Seminar format is the element in which learning is organized around sustained discussion of a shared text, question, or issue with facilitator support but strong learner participation. It is useful when interpretation, reasoning, and dialogue are central.

## Design Implications

### Context
#### Requirements
- **A common text, question, or topic**
- **Participation norms and discussion prompts**
#### Constraints
- **Weak preparation can flatten discussion**

### Target Learning Goals
- Support interpretive dialogue, critical reasoning, and sustained discussion.

### Affordances
- [Peer Discussion](../principles/peer-discussion.md)
- [Social Constructivism](../principles/social-constructivism.md)

## Related Elements
- [Open-Ended Discussion](open-ended-discussion.md)
- [Structured Discussion](structured-discussion.md)
- [Pre-Seminar Preparation](pre-seminar-preparation.md)

## Patterns That Use This Element
- [Socratic Seminar](../patterns/socratic-seminar.md)
- [Discussion-Based Learning](../patterns/discussion-based-learning.md)

## Examples
- Text-centered seminar where learners respond to each other around a common prompt.

## Key Sources
- Adler, M. J. (1982). *The Paideia proposal*. Macmillan.
