---
type: element
id: intellectual-styles-inventory
title: "Sternberg's intellectual styles inventory under validation"
description: Sternberg describes an inventory under validation designed to measure intellectual styles, in which students rate statements on a 1-to-9 scale for how well each describes them.
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: presseisen-1990
    resource: "https://eric.ed.gov/?id=ED327322"
    title: "Presseisen, Barbara Z.; Sternberg, Robert J.; Fischer, Kurt W.; Knight, Catharine C.; Feuerstein, Reuven. (1990). Learning and Thinking Styles: Classroom Interaction. National Education Association. https://eric.ed.gov/?id=ED327322"
    author: Presseisen, Barbara Z.; Sternberg, Robert J.; Fischer, Kurt W.; Knight, Catharine C.; Feuerstein, Reuven
---

# Sternberg's intellectual styles inventory under validation

> **Element** · [All elements](index.md)

## Description
Sternberg describes an inventory under validation designed to measure intellectual styles, in which students rate statements on a 1-to-9 scale for how well each describes them. Legislative, executive, and judicial preferences are indexed by agreement with items about planning one's own way, following instructions, and evaluating others' work. The article states "The inventory consists of a series of statements, which students rate on a 1-to-9 scale, depending on the extent to which each statement is viewed as describing the rater."

## Design Implications

### Context
#### Requirements
- Measurement should capture both predilections toward styles and abilities to implement them, per the chapter's discussion.
#### Constraints
- 

### Target Learners
- Students whose style preferences are being assessed

### Target Learning Goals
- Identifying preferences for ways of using intelligence

### Affordances
- [Mental Self Government Intellectual Styles](../theories/mental-self-government-intellectual-styles.md)

## Related Elements
- 

## Examples
-

## Key Sources
- Presseisen, Barbara Z.; Sternberg, Robert J.; Fischer, Kurt W.; Knight, Catharine C.; Feuerstein, Reuven. (1990). Learning and Thinking Styles: Classroom Interaction. National Education Association. https://eric.ed.gov/?id=ED327322
