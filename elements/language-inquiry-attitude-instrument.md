---
type: element
id: language-inquiry-attitude-instrument
title: "The Language Inquiry: a 100-item language-attitude questionnaire benchmarked against ten linguists"
description: "The Language Inquiry is a collection of 100 statements about attitudes toward language, devised to help teachers evaluate themselves and determine their students' ideas about language."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: frogner-1969
    resource: "https://eric.ed.gov/?id=ED034768"
    title: "Frogner, Ellen A. (1969). Using the Language Inquiry as a Teaching Device. Illinois State-Wide Curriculum Study Center in the Preparation of Secondary English Teachers (ISCPET). https://eric.ed.gov/?id=ED034768"
    author: Frogner, Ellen A
---

# The Language Inquiry: a 100-item language-attitude questionnaire benchmarked against ten linguists

> **Element** · [All elements](index.md)
> **Evidence** · no claims cited

## Description
The Language Inquiry is a collection of 100 statements about attitudes toward language, devised to help teachers evaluate themselves and determine their students' ideas about language. Respondents indicate agreement with each statement and compare their responses with those of ten linguists who evaluated each item. An additional section lets respondents record three items they would like discussed and the reasons for their choice.

## Design Implications

### Context
#### Requirements
- Respondents need access to the linguists' votes for each item, since interpretation is by item-by-item comparison with the ten experts
#### Constraints
- The instrument focuses on attitudes in relation to language and does not aim to test information for the sake of the information itself

### Target Learners
- prospective secondary English teachers
- in-service high school English teachers
- elementary language arts majors

### Target Learning Goals
- awareness of attitudes and concepts about language
- alignment of language attitudes with current expert thought

## Related Elements
- 

## Examples

- [Administer the Inquiry with classroom test procedures and interpret by item-by-item comparison with expert votes](../strategies/inquiry-item-by-item-expert-comparison-strategy.md)

## Key Sources
- Frogner, Ellen A. (1969). Using the Language Inquiry as a Teaching Device. Illinois State-Wide Curriculum Study Center in the Preparation of Secondary English Teachers (ISCPET). https://eric.ed.gov/?id=ED034768
