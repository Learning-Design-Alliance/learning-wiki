---
type: element
id: teacher-language-and-gesture-codes-for-mathematical-equivalence
title: Teacher Language and Gesture Coding Scheme for Mathematical Equivalence
description: "A coding scheme with four language codes (ISA, \"is the same as\"; DEF, definition; BAL, balance; FORM, equation form) and two gesture codes (GBAL, balance gesture; GSIDES, equation sides gesture) for a teacher's talk a..."
status: draft
generated:
  by: claude/unspecified
  at: 2026-09-24
sources:
  - id: sung-2022
    resource: "https://eric.ed.gov/?id=ED630361"
    title: "Sung, Y., Stephens, A., Veltri Torres, R., Strachota, S., Blanton, M., Gardiner, A., Stroud, R., & Knuth, E. (2022). Teacher Language and Gesture in an Intervention Focused on Developing Kindergarteners' Understandings of the Equal Sign. Proceedings of the forty-fourth annual meeting of the North American Chapter of the International Group for the Psychology of Mathematics Education. https://eric.ed.gov/?id=ED630361"
    author: "Sung, Y., Stephens, A., Veltri Torres, R., Strachota, S., Blanton, M., Gardiner, A., Stroud, R., & Knuth, E"
---

# Teacher Language and Gesture Coding Scheme for Mathematical Equivalence

> **Element** · [All elements](index.md)
> **Evidence** · no claims cited

## Description
A coding scheme with four language codes (ISA, "is the same as"; DEF, definition; BAL, balance; FORM, equation form) and two gesture codes (GBAL, balance gesture; GSIDES, equation sides gesture) for a teacher's talk about equations. The authors developed it iteratively from classroom video, settling on a scheme "that focused on the teacher’s use of language around equations and the meaning of the equal sign during whole-group discussion."

## Design Implications

### Context
#### Requirements
- Transcribed classroom video of the lessons that address mathematical equivalence
- Iterative, collaborative coding by several team members with group consensus, following Syed and Nelson's (2015) recommendations as the article describes
#### Constraints
- Applied to whole-group discussion only
- The analysis covered one of the two intervention classrooms and seven of its eighteen lessons

### Target Learners
- Kindergarten students in the Northeastern U.S. taking part in an early algebra intervention

### Target Learning Goals
- A relational understanding of the equal sign as indicating the equivalence of two amounts
- Acceptance of equations in forms other than a + b = c

## Related Elements
- [Gestures](../strategies/gestures.md)
- [Multimodal Instruction](../principles/multimodal-instruction.md)
- [Math Talks](../strategies/math-talks.md)

## Examples
-

## Key Sources
- Sung, Y., Stephens, A., Veltri Torres, R., Strachota, S., Blanton, M., Gardiner, A., Stroud, R., & Knuth, E. (2022). Teacher Language and Gesture in an Intervention Focused on Developing Kindergarteners' Understandings of the Equal Sign. Proceedings of the forty-fourth annual meeting of the North American Chapter of the International Group for the Psychology of Mathematics Education. https://eric.ed.gov/?id=ED630361
