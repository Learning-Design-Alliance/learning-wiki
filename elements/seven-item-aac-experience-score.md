---
type: element
id: seven-item-aac-experience-score
title: Seven-item parent-reported AAC implementation experience score
description: A parent-reported questionnaire score measuring experiences during AAC implementation, covering professional support, follow-up, confidence using AAC, perceived individual fit, team cooperation, and consideration of e...
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: jarrar-hala-2026
    resource: "https://doi.org/10.1186/s12887-026-07161-2"
    title: "Jarrar Hala, Malhis Lana, Munawer Alaa, Omareya Moamen, Deek Nagham, Abutair Razan, Dodeen Saja, Khadeje Sara Majed, Abu-Shamat Zaynab. (2026). Parent-reported benefits, barriers, and experiences of augmentative and alternative communication use among children with autism in Nablus, Palestine: a cross-sectional study. BMC Pediatrics. https://doi.org/10.1186/s12887-026-07161-2"
    author: Jarrar Hala, Malhis Lana, Munawer Alaa, Omareya Moamen, Deek Nagham, Abutair Razan, Dodeen Saja, Khadeje Sara Majed, Abu-Shamat Zaynab
---

# Seven-item parent-reported AAC implementation experience score

> **Element** · [All elements](index.md)

## Description
A parent-reported questionnaire score measuring experiences during AAC implementation, covering professional support, follow-up, confidence using AAC, perceived individual fit, team cooperation, and consideration of emotional and psychological needs. Items are rated on four ordered options (never, sometimes, mostly, always) scored 0–3, giving a total range of 0–21. The article reports that "The final AAC experience score was calculated from seven positively worded experience items and showed good internal consistency (Cronbach’s α = 0.841)"; the observed median was 16.0 (IQR 12.75–19.0).

## Design Implications

### Context
#### Requirements
- Administration to parents or caregivers with at least six months of AAC exposure; scoring excludes negatively worded acceptance-difficulty items, which are analyzed separately
#### Constraints
- Developed for this study via literature review, expert panel review, and pilot testing with 10 parents; the article states it was not a formally validated psychometric instrument

### Target Learners
- Parents and primary caregivers of children with autism using AAC

### Target Learning Goals
- Assessing parent-reported quality of AAC implementation experiences, including professional support and family-centered service dimensions

## Related Elements
- 

## Examples
-

## Key Sources
- Jarrar Hala, Malhis Lana, Munawer Alaa, Omareya Moamen, Deek Nagham, Abutair Razan, Dodeen Saja, Khadeje Sara Majed, Abu-Shamat Zaynab. (2026). Parent-reported benefits, barriers, and experiences of augmentative and alternative communication use among children with autism in Nablus, Palestine: a cross-sectional study. BMC Pediatrics. https://doi.org/10.1186/s12887-026-07161-2
