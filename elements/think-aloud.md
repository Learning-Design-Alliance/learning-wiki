---
type: element
title: Think-Aloud
description: Think-aloud is the element in which an instructor, peer, or learner verbalizes reasoning while performing a task.
status: review
generated:
  by: codex/unspecified
  at: 2026-04-07
---

# Think-Aloud

## Description
Think-aloud is the element in which an instructor, peer, or learner verbalizes reasoning while performing a task. It makes otherwise hidden judgment, strategy, and error-checking processes visible so learners can observe not just what to do, but how to decide.

## Design Implications

### Context
#### Requirements
- **A meaningful task**: There needs to be a visible decision process worth narrating.
- **Reasoning visibility**: The speaker should explain choices, checks, and uncertainties, not just steps.
- **Connection to later learner use**: Learners should apply, imitate, or critique the reasoning afterward.
#### Constraints
- **Expert blind spots**: Skilled performers may omit tacit decisions unless prompted.
- **Cognitive overload**: Dense narration can overwhelm novices if too many ideas are packed together.

### Target Learners
- Learners who need help seeing how experts monitor, choose, and revise during performance.

### Target Learning Goals
- Surface strategies, criteria, and self-monitoring moves.

### Affordances
- [Modeling](/principles/modeling.md)
- [Scaffolding and Fading](/principles/scaffolding-and-fading.md)
- [Explaining Their Thinking](/principles/explaining-their-thinking.md)

## Related Elements
- [Demonstration](/elements/demonstration.md)
- [Coaching](/elements/coaching.md)
- [Feedback](/elements/feedback.md)

## Patterns That Use This Element
- [Cognitive Apprenticeship](/patterns/cognitive-apprenticeship.md)
- [Gagné's 9 Events](/patterns/gagnés-9-events.md)

## Examples
- An instructor solves a math problem while narrating why each step is chosen.
- A reader pauses during annotation to explain how they infer meaning from a difficult sentence.

## Key Sources
- Collins, A., Brown, J. S., & Newman, S. E. (1989). Cognitive apprenticeship: Teaching the crafts of reading, writing, and mathematics. In L. B. Resnick (Ed.), *Knowing, learning, and instruction*. Lawrence Erlbaum.
- Chi, M. T. H. (1996). Constructing self-explanations and scaffolded explanations in tutoring. *Applied Cognitive Psychology, 10*(7), S33-S49. [https://doi.org/10.1002/(SICI)1099-0720(199612)10:7<S33::AID-ACP436>3.0.CO;2-2](https://doi.org/10.1002/(SICI)1099-0720(199612)10:7<S33::AID-ACP436>3.0.CO;2-2)
