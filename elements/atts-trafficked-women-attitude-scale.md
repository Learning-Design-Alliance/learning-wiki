---
type: element
id: atts-trafficked-women-attitude-scale
title: Attitudes Toward Trafficked Women and Sex Trafficking Scale (ATTS)
description: "The ATTS is a modified version of the 29-item APPS in which the first author collaborated with the original developers to reword items with person-first language such as 'human trafficking survivor' and 'sex trafficki..."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: litam-2019
    resource: "https://doi.org/10.15241/sdal.9.4.396"
    title: "Litam, S. D. A. (2019). She's Just a Prostitute: The Effects of Labels on Counselor Attitudes, Empathy, and Rape Myth Acceptance. The Professional Counselor, 9(4), 396-415. https://doi.org/10.15241/sdal.9.4.396"
    author: Litam, S. D. A
---

# Attitudes Toward Trafficked Women and Sex Trafficking Scale (ATTS)

> **Element** · [All elements](index.md)

## Description
The ATTS is a modified version of the 29-item APPS in which the first author collaborated with the original developers to reword items with person-first language such as 'human trafficking survivor' and 'sex trafficking'. It retains the APPS's four subscales (PSCV, PSND, PNND, PNCV) measuring attitudes on normative/deviant and choosing/victimized axes. In this study it served as the 'sex trafficking' label condition instrument, completed by 203 licensed counselors.

## Design Implications

### Context
#### Requirements
- Developed with input from the original APPS developers to minimize interference with the APPS's sound psychometric properties
#### Constraints
- The reliability and validity information pertaining to the ATTS is unknown, as this study was the first to use it

### Target Learners
- licensed professional counselors
- researchers studying attitudes toward trafficking

### Target Learning Goals
- measuring attitudes toward trafficked women and sex trafficking

## Related Elements
- 

## Examples
-

## Key Sources
- Litam, S. D. A. (2019). She's Just a Prostitute: The Effects of Labels on Counselor Attitudes, Empathy, and Rape Myth Acceptance. The Professional Counselor, 9(4), 396-415. https://doi.org/10.15241/sdal.9.4.396
