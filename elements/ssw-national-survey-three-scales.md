---
type: element
id: ssw-national-survey-three-scales
title: National survey of school social workers with three reliability-tested scales
description: An anonymous online Qualtrics survey administered to a national sample of school social workers between November 2019 and June 2020 (N = 517), with 11 open-ended questions yielding 1,738 comments.
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: rodriguez-2022
    resource: "https://doi.org/10.1177/23328584211073170"
    title: "Rodriguez, Roth, & Villarreal Sosa. (2022). \"Immigration Enforcement Is a Daily Part of Our Students' Lives\": School Social Workers' Perceptions of Racialized Nested Contexts of Reception for Immigrant Students. AERA Open. https://doi.org/10.1177/23328584211073170"
    author: "Rodriguez, Roth, & Villarreal Sosa"
---

# National survey of school social workers with three reliability-tested scales

> **Element** · [All elements](index.md)
> **Evidence** · no claims cited

## Description
An anonymous online Qualtrics survey administered to a national sample of school social workers between November 2019 and June 2020 (N = 517), with 11 open-ended questions yielding 1,738 comments. The instrument included three scales: Social Workers' Extent of Action Scale (16 items, Cronbach's α of .878), Social Workers' Perception of the Impact of Immigration Enforcement Scale (nine items, α of .901), and Social Workers' Perception of Local Support Scale (six items, α of .839). Sampling targeted 67 districts where immigrants represented at least 40% of residents.

## Design Implications

### Context
#### Requirements
- Multiple imputation was used to handle missing data for valid scale cases
#### Constraints
- The sample is not representative of all SSWs, and findings are not generalizable per the authors' limitations section

### Target Learners
- School social workers in immigrant-serving K-12 schools

### Target Learning Goals
- Understanding SSW perceptions and actions supporting immigrant students

## Related Elements
- 

## Examples
-

## Key Sources
- Rodriguez, Roth, & Villarreal Sosa. (2022). "Immigration Enforcement Is a Daily Part of Our Students' Lives": School Social Workers' Perceptions of Racialized Nested Contexts of Reception for Immigrant Students. AERA Open. https://doi.org/10.1177/23328584211073170
