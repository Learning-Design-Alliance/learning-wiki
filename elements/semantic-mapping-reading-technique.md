---
type: element
id: semantic-mapping-reading-technique
title: Semantic mapping as a pre- and post-reading comprehension technique
description: Semantic mapping is a classroom technique for teaching reading comprehension that integrates new information with prior knowledge.
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-25
sources:
  - id: el-koumy-1999
    resource: "https://eric.ed.gov/?id=ED435193"
    title: "El-Koumy, Abdel Salam A. (1999). Effects of Three Semantic Mapping Strategies on EFL Students' Reading Comprehension. https://eric.ed.gov/?id=ED435193"
    author: El-Koumy, Abdel Salam A
---

# Semantic mapping as a pre- and post-reading comprehension technique

> **Element** · [All elements](index.md)

## Description
Semantic mapping is a classroom technique for teaching reading comprehension that integrates new information with prior knowledge. Citing Heimlich and Pittelman, the article lists advantages: it "appears to motivate students of all age levels and to involve them actively in the thinking-reading process," integrates thinking with reading, integrates assessment with teaching, and helps teachers judge needed instruction based on what students demonstrate they know. Prior studies cited found semantic mapping groups outscored no-map controls on recall and comprehension tests.

## Design Implications

### Context
#### Requirements
- Used as a strategy to activate, assess and embellish students' prior knowledge of a topic before reading
#### Constraints
- The article's focus is expository passages of about 800 words with EFL college readers

### Target Learners
- Students of all grade levels, including EFL college freshmen

### Target Learning Goals
- Reading comprehension of expository and narrative text

### Affordances
- [Three Semantic Mapping Strategy Variants](../theories/three-semantic-mapping-strategy-variants.md)

## Related Elements
- 

## Examples
-

## Key Sources
- El-Koumy, Abdel Salam A. (1999). Effects of Three Semantic Mapping Strategies on EFL Students' Reading Comprehension. https://eric.ed.gov/?id=ED435193
