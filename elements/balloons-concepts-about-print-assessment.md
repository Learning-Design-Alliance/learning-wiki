---
type: element
id: balloons-concepts-about-print-assessment
title: "Balloons: A Concepts About Print Assessment, a criterion-referenced 17-item measure from the Kindergarten Assessment Portfolio"
description: "The Balloons test, locally constructed in 1991, is \"a criterion-referenced measure from the Kindergarten Assessment Portfolio\" used to assess kindergarten pupils' concepts about print."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: johnson-1994
    resource: "https://eric.ed.gov/?id=ED379098"
    title: "Johnson, Jessie; And Others. (1994). Language Development Component: All Day Kindergarten Program 1992-1993. Ohio Disadvantaged Pupil Program Fund. Final Evaluation Report. https://eric.ed.gov/?id=ED379098"
    author: Johnson, Jessie; And Others
---

# Balloons: A Concepts About Print Assessment, a criterion-referenced 17-item measure from the Kindergarten Assessment Portfolio

> **Element** · [All elements](index.md)

## Description
The Balloons test, locally constructed in 1991, is "a criterion-referenced measure from the Kindergarten Assessment Portfolio" used to assess kindergarten pupils' concepts about print. Its 17 scored objectives include knowing the front of the book, that print rather than pictures carries the message, left-to-right movement, one-to-one word match, and concepts of letter, word, and capital letter. Program teachers administered it the week of March 29, 1993; successful completion of at least 12 of 17 items was considered appropriate for promotion to grade 1.

## Design Implications

### Context
#### Requirements
- Administered individually with scripted directions and scoring criteria for each item
#### Constraints
- Locally constructed for Columbus Public Schools; the report notes results preclude comparisons across projects using different tests

### Target Learners
- Kindergarten pupils

### Target Learning Goals
- Concepts about print, including book orientation, directionality, one-to-one matching, and letter/word concepts

## Related Elements
- 

## Examples
-

## Key Sources
- Johnson, Jessie; And Others. (1994). Language Development Component: All Day Kindergarten Program 1992-1993. Ohio Disadvantaged Pupil Program Fund. Final Evaluation Report. https://eric.ed.gov/?id=ED379098
