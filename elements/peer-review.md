---
type: element
title: Peer Review
description: "Peer review is the element in which learners examine and respond to one another's work using shared criteria, prompts, or examples."
status: review
generated:
  by: codex/unspecified
  at: 2026-04-07
sources:
  - id: topping-1998
    resource: "https://doi.org/10.3102/00346543068003249"
    title: "Topping, K. J. (1998). Peer assessment between students in colleges and universities. *Review of Educational Research, 68*(3), 249-276"
    author: Topping, K. J
  - id: huisman-2019
    resource: "https://doi.org/10.1080/02602938.2018.1545896"
    title: "Huisman, B., Saab, N., van den Broek, P., & van Driel, J. (2019). The impact of formative peer feedback on higher education students' academic writing: A meta-analysis. *Assessment & Evaluation in Higher Education, 44*(6), 863-880"
    author: "Huisman, B., Saab, N., van den Broek, P., & van Driel, J"
---

# Peer Review

> **Element** · [All elements](index.md)

## Description
Peer review is the element in which learners examine and respond to one another's work using shared criteria, prompts, or examples. It can improve drafts directly, but it is also valuable because evaluating peer work often sharpens learners' own judgment about quality.

## Design Implications

### Context
#### Requirements
- **Clear criteria**: Reviewers need guidance on what to notice and comment on.
- **Structured prompts**: Specific feedback stems or rubrics improve consistency and usefulness.
- **Revision opportunity**: The reviewed work should be revisable after feedback.
#### Constraints
- **Low trust or weak norms**: Learners may avoid candor or overcorrect each other.
- **Uneven expertise**: Peers need support to give accurate, actionable feedback.
- **Social risk**: Public criticism can suppress participation if norms are weak.

### Target Learners
- Learners revising visible work products and developing evaluative judgment.

### Target Learning Goals
- Improve drafts, strengthen criteria use, and develop feedback literacy.

### Affordances
- [Peer Feedback/Peer Review](../principles/peer-feedbackpeer-review.md)
- [Formative Assessment](../principles/formative-assessment.md)
- [Feedback Loops](../principles/feedback-loops.md)

## Related Elements
- [Feedback](feedback.md)
- [Revision](revision.md)
- [Rubrics](rubrics.md)

## Patterns That Use This Element
- [Structured Peer Review](../patterns/structured-peer-review.md)
- [Formative Assessment](../patterns/formative-assessment.md)

## Examples
- Learners exchange drafts and leave rubric-based comments before revision.
- Students review a worked solution from a peer and identify strengths, errors, and next steps.

## Key Sources
- Topping, K. J. (1998). Peer assessment between students in colleges and universities. *Review of Educational Research, 68*(3), 249-276. [https://doi.org/10.3102/00346543068003249](https://doi.org/10.3102/00346543068003249)
- Huisman, B., Saab, N., van den Broek, P., & van Driel, J. (2019). The impact of formative peer feedback on higher education students' academic writing: A meta-analysis. *Assessment & Evaluation in Higher Education, 44*(6), 863-880. [https://doi.org/10.1080/02602938.2018.1545896](https://doi.org/10.1080/02602938.2018.1545896)
