---
type: element
title: Prior Knowledge Activation
description: Prior knowledge activation is the element in which learners recall, surface, or organize what they already know before engaging with new material.
status: review
generated:
  by: codex/unspecified
  at: 2026-04-08
---

# Prior Knowledge Activation

## Description
Prior knowledge activation is the element in which learners recall, surface, or organize what they already know before engaging with new material. It is useful when new learning depends on connection to existing ideas or experiences.

## Design Implications

### Context
#### Requirements
- **A prompt that surfaces relevant prior knowledge**
- **A connection between recalled knowledge and the new task**
#### Constraints
- **Activated misconceptions may need correction, not just recall**

### Target Learning Goals
- Prepare learners to connect new material to what they already know.

### Affordances
- [Activation](../principles/activation.md)
- [Metacognition](../principles/metacognition.md)
- [Retrieval Practice](../principles/retrieval-practice.md)

## Related Elements
- [Activation](activation.md)
- [Concept Mapping](concept-mapping.md)
- [Pre-Reading Activities](pre-reading-activities.md)

## Patterns That Use This Element
- [Merrill's First Principles](../patterns/merrills-first-principles.md)
- [Gagné's 9 Events](../patterns/gagnés-9-events.md)

## Examples
- Quick-write prompts about what learners already know before a new unit.

## Key Sources
- Bransford, J. D., Brown, A. L., & Cocking, R. R. (Eds.). (2000). *How people learn*. National Academies Press.
