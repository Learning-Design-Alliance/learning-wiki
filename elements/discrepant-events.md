---
type: element
title: Discrepant Events
description: Discrepant events are demonstrations, observations, or scenarios that violate learner expectations in a noticeable way.
status: review
generated:
  by: codex/unspecified
  at: 2026-04-08
---

# Discrepant Events

> **Element** · [All elements](index.md)

## Description
Discrepant events are demonstrations, observations, or scenarios that violate learner expectations in a noticeable way. They are useful when the instructional goal is to provoke rethinking, explanation, and conceptual change.

## Design Implications

### Context
#### Requirements
- **An event that clearly contradicts an expected outcome**
- **A follow-up prompt that asks learners to explain the mismatch**
#### Constraints
- **Without debrief, surprise alone may entertain more than teach**

### Target Learning Goals
- Surface misconceptions and create readiness for explanation revision.

### Affordances
- [Cognitive Disequilibrium](../principles/cognitive-disequilibrium.md)
- [Cognitive Flexibility](../principles/cognitive-flexibility.md)
- [Error Analysis](../principles/error-analysis.md)

## Related Elements
- [Cognitive Conflict](cognitive-conflict.md)
- [Demonstration](demonstration.md)
- [Inquiry-Based Learning](inquiry-based-learning.md)

## Patterns That Use This Element
- [Guided Discovery Learning](../patterns/guided-discovery-learning.md)
- [Cognitive Flexibility Theory](../patterns/cognitive-flexibility-theory.md)

## Examples
- A science demonstration that produces a result opposite to what learners predicted.

## Key Sources
- Limon, M. (2001). On the cognitive conflict as an instructional strategy for conceptual change. *Learning and Instruction, 11*(4-5), 357-380. [https://doi.org/10.1016/S0959-4752(00)00037-2](https://doi.org/10.1016/S0959-4752(00)00037-2)
