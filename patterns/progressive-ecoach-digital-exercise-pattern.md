---
type: pattern
id: progressive-ecoach-digital-exercise-pattern
title: Progressive home-based digital exercise pattern with eCoach guidance, reminders and self-paced progression
description: "A lesson/programme-level design in which learners follow a progressive strength-and-balance curriculum at home at their own pace and convenience, guided by animated eCoaches providing \"instructions, guidance, safety i..."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: french-chloe-2026
    resource: "https://doi.org/10.1093/ageing/afag291"
    title: "French Chloe, Parchment Amelia, Odebiyi Bolanle, Shi Chunhu, Bashir Saima, Dowding Dawn, Kislov Roman, Thompson Alexander, Skelton Dawn A, Clarke Margaret, Sylvestre Garcia Yvonne, Ahmed Saima S, Todd Chris J, Bower Peter, Stanmore Emma. (2026). Effectiveness and cost-effectiveness of the Keep-On-Keep-Up digital falls prevention programme in community-dwelling older adults: results of a randomised controlled trial. Age and Ageing. https://doi.org/10.1093/ageing/afag291"
    author: French Chloe, Parchment Amelia, Odebiyi Bolanle, Shi Chunhu, Bashir Saima, Dowding Dawn, Kislov Roman, Thompson Alexander, Skelton Dawn A, Clarke Margaret, Sylvestre Garcia Yvonne, Ahmed Saima S, Todd Chris J, Bower Peter, Stanmore Emma
---

# Progressive home-based digital exercise pattern with eCoach guidance, reminders and self-paced progression

> **Pattern** · [All patterns](index.md)
> **Evidence** · 2 claims (2 for) · 1 study, `q2` · 1 of 1 report an effect size · 2 claims rest on one study

## Description
A lesson/programme-level design in which learners follow a progressive strength-and-balance curriculum at home at their own pace and convenience, guided by animated eCoaches providing "instructions, guidance, safety information, automated reminders and in-app progress tracking". The structure was considered simple to follow, with clear and accessible audio and visual guidance, and some participants came to engage daily by the end of the programme. The pattern embeds behaviour change techniques within a self-directed digital setting.

## Design Implications

### Context
#### Requirements
- Clear and accessible audio and visual guidance, embedded regular safety instructions, and initial training so learners can use the programme independently
#### Constraints
- Counters, timers or guidance on required repetitions were felt to be needed; challenge must be tailorable to ability, and stopping use at the end of the intervention period was felt to lead to a decline in physical functioning for some

### Target Learners
- community-dwelling older adults aged 60 and over, including those with lower digital literacy

### Target Goals
- progressive strength and balance exercise
- independent sustained engagement with exercise at home

### Claims
- [Koku Improves Balance 12 Weeks Older Adults](../claims/koku-improves-balance-12-weeks-older-adults.md) [+M]
- [Koku Qualitative Acceptability Themes](../claims/koku-qualitative-acceptability-themes.md) [+M]

## Related Patterns
- 

## Examples
-

## Key Sources
- French Chloe, Parchment Amelia, Odebiyi Bolanle, Shi Chunhu, Bashir Saima, Dowding Dawn, Kislov Roman, Thompson Alexander, Skelton Dawn A, Clarke Margaret, Sylvestre Garcia Yvonne, Ahmed Saima S, Todd Chris J, Bower Peter, Stanmore Emma. (2026). Effectiveness and cost-effectiveness of the Keep-On-Keep-Up digital falls prevention programme in community-dwelling older adults: results of a randomised controlled trial. Age and Ageing. https://doi.org/10.1093/ageing/afag291
