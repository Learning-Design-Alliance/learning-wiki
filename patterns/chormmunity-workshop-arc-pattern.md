---
type: pattern
id: chormmunity-workshop-arc-pattern
title: "Workshop arc: oral conversation, then imagistic, then somatic, ending in ritual performance"
description: Chormmunity sessions follow a modal sequence across the presentational-propositional continuum.
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: loper-2000
    resource: "https://eric.ed.gov/?id=ED443009"
    title: "Loper, Paul. (2000). Chormmunity: Co-Creating Embodied Community. ERIC Document ED443009. https://eric.ed.gov/?id=ED443009"
    author: Loper, Paul
---

# Workshop arc: oral conversation, then imagistic, then somatic, ending in ritual performance

> **Pattern** · [All patterns](index.md)
> **Evidence** · no claims cited

## Description
Chormmunity sessions follow a modal sequence across the presentational-propositional continuum. As printed: "The process of a Chormmunity workshop begins verbally, but in conversation, not lecture, definitely not in printed text", then "quickly "moves back" into the imagistic, then the somatic". The co-created movement text is ritually enacted at the end when the group formally shifts from a rehearsal mode to a performance mode.

## Design Implications

### Context
#### Requirements
- Participants remain fully present as rational, cogent selves throughout
- A closing performance-mode enactment of the co-created text
#### Constraints
- The verbal-rational modality cannot be skipped, since adult participants operate in Philosophic and Ironic Understandings as a given

### Target Learners
- adult workshop participants

### Target Goals
- embodied meaning-making
- transformative learning through collective creation

### Claims
- 

## Related Patterns
- 

## Examples
-

## Key Sources
- Loper, Paul. (2000). Chormmunity: Co-Creating Embodied Community. ERIC Document ED443009. https://eric.ed.gov/?id=ED443009
