---
type: pattern
id: five-activity-shadow-to-sundial-sequence
title: Secuencia de cinco actividades que va del problema cotidiano a la construcción de un instrumento de medición
description: "El patrón organiza la enseñanza en cinco actividades encadenadas: indagación introductoria sobre las sombras y la hora sin relojes; diseño de una maqueta de los arcos solares; debate histórico sobre modelos geocéntric..."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: gangui-2009
    resource: "https://cms.iafe.uba.ar/gangui"
    title: "Gangui, Alejandro; Iglesias, María C.; Quinteros, Cynthia P. (2009). El movimiento de las sombras: Una propuesta de trabajo para la escuela secundaria. Ciencia Hoy. https://cms.iafe.uba.ar/gangui"
    author: Gangui, Alejandro; Iglesias, María C.; Quinteros, Cynthia P
---

# Secuencia de cinco actividades que va del problema cotidiano a la construcción de un instrumento de medición

> **Pattern** · [All patterns](index.md)

## Description
El patrón organiza la enseñanza en cinco actividades encadenadas: indagación introductoria sobre las sombras y la hora sin relojes; diseño de una maqueta de los arcos solares; debate histórico sobre modelos geocéntrico y heliocéntrico; sombras y latitud con modelos externos e internos; y construcción y orientación de un reloj de Sol horizontal. Cada actividad se apoya en los conocimientos adquiridos en la anterior y registra las respuestas de los alumnos en un cartel colectivo para revisarlas más adelante.

## Design Implications

### Context
#### Requirements
- Materiales de bajo costo listados por los autores
- registro colectivo de respuestas en un cartel para volver sobre ellas
- guiado docente mediante preguntas
#### Constraints
- El reloj propuesto indica la hora solar y no la hora civil; el tablero mostrado fue confeccionado para la latitud de Buenos Aires y sirve para localidades de latitud aproximadamente igual

### Target Learners
- estudiantes de escuela secundaria

### Target Goals
- movimiento aparente del Sol
- estaciones
- relación sombra-latitud
- construcción de un reloj de Sol

### Claims
- 

## Related Patterns
- 

## Examples
-

## Key Sources
- Gangui, Alejandro; Iglesias, María C.; Quinteros, Cynthia P. (2009). El movimiento de las sombras: Una propuesta de trabajo para la escuela secundaria. Ciencia Hoy. https://cms.iafe.uba.ar/gangui
