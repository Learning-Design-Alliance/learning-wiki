---
type: pattern
id: three-language-knowledge-aspects-for-reading
title: "Three aspects of children's language knowledge hypothesized to support fluent reading acquisition"
description: "In analyzing how home-school language switching affects reading development, the paper distinguishes three general aspects of children's knowledge of language hypothesized as important for acquiring fluent reading ski..."
status: draft
generated:
  by: claude/unspecified
  at: 2026-09-25
sources:
  - id: cummins-1979
    resource: "https://eric.ed.gov/?id=ED257312"
    title: "Cummins, James. (1979). Linguistic Interdependence and the Educational Development of Bilingual Children. Bilingual Education Paper Series, Vol. 3 No. 2. https://eric.ed.gov/?id=ED257312"
    author: Cummins, James
---

# Three aspects of children's language knowledge hypothesized to support fluent reading acquisition

> **Pattern** · [All patterns](index.md)

## Description
In analyzing how home-school language switching affects reading development, the paper distinguishes three general aspects of children's knowledge of language hypothesized as important for acquiring fluent reading skills: "vocabulary-concept knowledge" of the child, metalinguistic insights regarding the nature of printed text, and facility in processing decontextualized language. These aspects are conceptually distinguishable but are all determined by the child's linguistic experiences prior to school and are likely strongly related empirically. The pattern frames the primary academic task of the early school years as learning to extract information efficiently from printed text.

## Design Implications

### Context
#### Requirements
- Fluent reading requires that the reader's knowledge of language be used to make inferences or predictions about information in the text
#### Constraints
- The three aspects are distinguished conceptually only; the paper states they are likely strongly related to one another empirically

### Target Learners
- Bilingual children entering school, particularly minority-language children

### Target Goals
- Fluent reading comprehension
- Literacy development in L1 and L2

### Claims
- [Developmental Interdependence Hypothesis](../theories/developmental-interdependence-hypothesis.md) [+M]

## Related Patterns
- 

## Examples
-

## Key Sources
- Cummins, James. (1979). Linguistic Interdependence and the Educational Development of Bilingual Children. Bilingual Education Paper Series, Vol. 3 No. 2. https://eric.ed.gov/?id=ED257312. [doi:10.3102/00346543049002222](https://doi.org/10.3102/00346543049002222)
