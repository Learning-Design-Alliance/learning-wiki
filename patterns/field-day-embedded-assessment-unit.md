---
type: pattern
id: field-day-embedded-assessment-unit
title: Authentic project unit (Field Day) with embedded assessment tasks mapped to critical objectives
description: The Type 3 assessment is organized as a project unit (a class Field Day, including a cookie-selling sub-project) whose everyday activities double as assessment occasions.
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-25
sources:
  - id: kasten-1997
    resource: "https://eric.ed.gov/?id=ED417935"
    title: "Kasten, Margaret; Meiring, Steve; Mikesell, Anne. (1997). Mathematics. Competency-Based Education Assessment Series. https://eric.ed.gov/?id=ED417935"
    author: Kasten, Margaret; Meiring, Steve; Mikesell, Anne
---

# Authentic project unit (Field Day) with embedded assessment tasks mapped to critical objectives

> **Pattern** · [All patterns](index.md)
> **Evidence** · no claims cited

## Description
The Type 3 assessment is organized as a project unit (a class Field Day, including a cookie-selling sub-project) whose everyday activities double as assessment occasions. The guide specifies that "The instructional tasks within which the district-wide, grade levelassessment will occur should involve situations that will reflect ways in which mathematics is learnedand applied." A checklist maps each activity number to the critical objectives it assesses, and student pages are used at relevant times throughout the project rather than as a single test session.

## Design Implications

### Context
#### Requirements
- Every student must be assessed on each activity; using the questions for whole-class discussion with a different child answering each one is not acceptable
#### Constraints
- If the school's actual field day plans differ, tasks may be changed, or another activity such as a holiday program substituted

### Target Learners
- First-grade students

### Target Goals
- Identifying needed information, using problem-solving strategies, skip counting, and developing addition and subtraction concepts from environmental situations

### Claims
- [Embed Assessment In Ongoing Instruction](../principles/embed-assessment-in-ongoing-instruction.md) [+M]

## Related Patterns
- 

## Examples
-

## Key Sources
- Kasten, Margaret; Meiring, Steve; Mikesell, Anne. (1997). Mathematics. Competency-Based Education Assessment Series. https://eric.ed.gov/?id=ED417935
