---
type: pattern
id: flexible-individualistic-pjbl-course-e
title: "Flexible-Individualistic PjBL: students design their own projects with separate final presentations"
description: Quadrant 3 (Flexible-Individualistic) PjBL, exemplified by Course E, lets each student design an individual project using emerging technologies in a shared campus production space.
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: son-2022
    resource: "https://doi.org/10.5278/ojs.jpblhe.v10i1.6821"
    title: "Son, E. H., & Penry, T. (2022). Variations in Project-Based Course Design. Journal of Problem Based Learning in Higher Education, Vol. 10, No. 1. https://doi.org/10.5278/ojs.jpblhe.v10i1.6821"
    author: "Son, E. H., & Penry, T"
---

# Flexible-Individualistic PjBL: students design their own projects with separate final presentations

> **Pattern** · [All patterns](index.md)

## Description
Quadrant 3 (Flexible-Individualistic) PjBL, exemplified by Course E, lets each student design an individual project using emerging technologies in a shared campus production space. "In this highly Flexible and Individual course, students designed their own individual projects, including goals, timetables, and deliverables." Students were not required to attend at the same time as long as they made weekly progress; a common presentation template was a rare Fixed attribute, and each student ended with a separate final project and presentation. The authors note this combination was unique both in their sample and in PjBL scholarship.

## Design Implications

### Context
#### Requirements
- A shared production space with innovative tools and emerging technology; weekly progress monitoring
#### Constraints
- Students give feedback and encouragement in a common space but do not work toward a common goal; no collaboration on final presentations

### Target Learners
- undergraduate and graduate students in interdisciplinary electives

### Target Goals
- developing expertise in emerging technologies through self-directed projects

### Claims
- [Pjbl Courses Span Multiple Quadrants](../claims/pjbl-courses-span-multiple-quadrants.md) [+M]

## Related Patterns
- 

## Examples
-

## Key Sources
- Son, E. H., & Penry, T. (2022). Variations in Project-Based Course Design. Journal of Problem Based Learning in Higher Education, Vol. 10, No. 1. https://doi.org/10.5278/ojs.jpblhe.v10i1.6821
