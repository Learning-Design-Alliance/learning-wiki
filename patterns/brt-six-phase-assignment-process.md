---
type: pattern
id: brt-six-phase-assignment-process
title: Six-phase assignment process mapped to BRT cognitive steps
description: "Each weekly assignment is structured under six phases corresponding to BRT's cognitive process dimensions."
status: draft
generated:
  by: "process:wiki-ingest"
  at: 2026-09-26
sources:
  - id: sözen-2024
    resource: "https://doi.org/10.19128/turje.1495965"
    title: "Sözen, G., & Özen-Yavuz, A. (2024). Proposing a course schedule for architectural basic design studio guided by Bloom's Revised Taxonomy. Turkish Journal of Education, 13(4), 379-409. https://doi.org/10.19128/turje.1495965"
    author: "Sözen, G., & Özen-Yavuz, A"
---

# Six-phase assignment process mapped to BRT cognitive steps

> **Pattern** · [All patterns](index.md)
> **Evidence** · no claims cited

## Description
Each weekly assignment is structured under six phases corresponding to BRT's cognitive process dimensions. The article states: "the process of the assignments in each week is designed under the cognitive learning steps of remember, understand, apply, analyze, evaluate and create referring to BRT." The phases progress from retrieving relevant knowledge from long-term memory through to creating an original design each week.

## Design Implications

### Context
#### Requirements
- Weekly clues from different disciplines
- Instructor critiques rather than theoretical lectures
#### Constraints
- Clues are designed to foster the analyze and evaluate steps

### Target Learners
- first-year architecture students

### Target Goals
- cognitive processing of design concepts
- original design production

### Claims
- 

## Related Patterns
- 

## Examples
-

## Key Sources
- Sözen, G., & Özen-Yavuz, A. (2024). Proposing a course schedule for architectural basic design studio guided by Bloom's Revised Taxonomy. Turkish Journal of Education, 13(4), 379-409. https://doi.org/10.19128/turje.1495965
