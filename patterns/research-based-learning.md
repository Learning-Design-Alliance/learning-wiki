---
type: pattern
title: Research-Based Learning
description: Research-Based Learning is a pattern in which learners investigate questions, gather and evaluate evidence, and build explanations or products from that inquiry.
status: review
generated:
  by: codex/unspecified
  at: 2026-04-07
sources:
  - id: brew-2013
    resource: "https://doi.org/10.1007/s10734-013-9624-x"
    title: "Brew, A. (2013). Understanding the scope of undergraduate research: A framework for curricular and pedagogical decision-making. *Higher Education, 66*(5), 603-618"
    author: Brew, A
author: research-centered instruction tradition
grain_size: unit
---

# Research-Based Learning

## Description
Research-Based Learning is a pattern in which learners investigate questions, gather and evaluate evidence, and build explanations or products from that inquiry. It is useful when learning goals include information literacy, inquiry, and evidence-based reasoning.

## Implications

### Context
#### Requirements
- **A meaningful research question**
- **Access to sources and evidence**
- **Evaluation, synthesis, and interpretation tasks**
#### Constraints
- **Weak questions lead to shallow research**
- **Learners often need support for source evaluation**
#### Grain Size
- Unit
- Course

### Target Goals
- Build inquiry, evidence evaluation, and synthesis.

### Target Learners
- Learners developing research and evidence-based reasoning skills.

### Theory
#### Supporting
- [Inquiry-Based Learning](../principles/inquiry-based-learning.md)
- [Constructivism](../principles/constructivism.md)

## Design

### Elements Used
- [Information Literacy](../elements/information-literacy.md)
- [Research](../elements/research.md)
- [Resource Evaluation](../elements/resource-evaluation.md)

### Affordances
- [Evaluating Sources](../principles/evaluating-sources.md)
- [Explicit Instruction in Internet Search](../principles/explicit-instruction-internet-search.md)

## Examples
- Students research a local issue and build a recommendation supported by sources.

## Key Sources
- Brew, A. (2013). Understanding the scope of undergraduate research: A framework for curricular and pedagogical decision-making. *Higher Education, 66*(5), 603-618. [https://doi.org/10.1007/s10734-013-9624-x](https://doi.org/10.1007/s10734-013-9624-x)
